...
import { queueDelta, getBalance } from "./vaultAdapter";
...
export async function settlePots(sessionId, board, players){
  ...
  for(const prizePool of pots){
    const eligible = players.filter(p=> prizePool.eligible.includes(p.seat_index));
    const winners = determineWinnersAuto(eligible, board);
    const share = Math.floor(prizePool.amount / winners.length);
    for(const seat of winners){
      ...
      if(pl){
        ...
        if (typeof window !== 'undefined' && pl.client_id) {
          ...
          if (rushData?.vault != null) queueDelta(0);
        }
      }
    }
  }
...
export async function startHand(sessionId) {
  ...
  // הגנה מפני לחיצה כפולה - בדיקה שהמשחק לא בתהליך
  if (ses.stage !== 'lobby' && ses.stage !== 'showdown') {
    return { ok:false, reason:'hand in progress' };
  }
  
  // בדיקה נוספת - אם יש deadline עתידי, המשחק עדיין פעיל
  if (ses.turn_deadline && new Date(ses.turn_deadline) > new Date()) {
    return { ok:false, reason:'hand in progress' };
  }
  ...
  await supabase.from('poker_players')
    .update({ 
      bet_street: 0, 
      total_bet: 0,      // ✅ חשוב: מאפס את הסכום הכולל של ההימורים לכל יד
      acted: false, 
      folded: false, 
      all_in: false, 
      hole_cards: [] 
  ...
  await supabase.from('poker_sessions').update({
    ...
    pot_total: Number(ses.pot_total||0),           // ✅ לא מוסיפים שוב – takeChips כבר הוסיף
    board: [],
    deck_remaining: deck,
    min_bet: bb,
