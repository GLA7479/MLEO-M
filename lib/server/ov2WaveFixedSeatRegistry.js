...
/** Case-insensitive UUID string compare (client `?room=` casing can differ from DB/registry). */
export function ov2WaveFixedTableRoomIdEq(a, b) {
...
export async function assertWaveFixedNoForeignActiveSeat(admin, participantKey, allowOnlyRoomId) {
  ...
  if (allowOnlyRoomId != null && ov2WaveFixedTableRoomIdEq(data.room_id, allowOnlyRoomId)) return { ok: true };
  return {
    ok: false,
...
export async function assertWaveFixedSeatFree(admin, participantKey, targetRoomId) {
  ...
  if (!data) return { ok: true };
  if (ov2WaveFixedTableRoomIdEq(data.room_id, targetRoomId)) return { ok: true };
  return {
    ok: false,
    code: "ALREADY_SEATED_ELSEWHERE",
    message: "You already have a seat at another table. Open that table or leave it before joining a new one.",
