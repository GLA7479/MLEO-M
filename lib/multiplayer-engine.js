...
export class MultiplayerEngine {
  ...
  async createRoom(playerName, maxPlayers = 4, startingChips = 10000) {
    ...
    conn.on('data', (data) => {
      if (data.type === 'join') {
        ...
        const newPlayer = {
          ...
          isHost: false,
          chips: this.gameState.startingChips || 10000,
          status: 'ready'
        };
  ...
  handleGameMessage(data, conn) {
    ...
    if (data.type === 'state_update' && this.isHost) {
      ...
      // These are now handled through gameState synchronization
