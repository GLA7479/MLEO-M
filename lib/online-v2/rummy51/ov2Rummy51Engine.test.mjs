...
test("sortHandCards: rank groups values; suit uses SHDC not alphabetical CDHS", () => {
  const deck = buildRummy51Deck();
  const s7 = pick(deck, 0, "S", 7);
  const h3 = pick(deck, 0, "H", 3);
  const d3 = pick(deck, 0, "D", 3);
  const cA = pick(deck, 0, "C", 1);
  const sA = pick(deck, 0, "S", 1);
  const byRank = sortHandCards([s7, h3, d3, cA, sA], "rank").map(c => c.id);
  assert.ok(byRank.indexOf(h3.id) < byRank.indexOf(s7.id), "3s before 7");
  assert.ok(byRank.indexOf(d3.id) - byRank.indexOf(h3.id) === 1, "both 3s adjacent ♥ then ♦");
  const bySuit = sortHandCards([cA, sA, h3], "suit").map(c => c.id);
  assert.equal(bySuit[0], sA.id);
  assert.equal(bySuit[1], h3.id);
  assert.equal(bySuit[2], cA.id);
});
