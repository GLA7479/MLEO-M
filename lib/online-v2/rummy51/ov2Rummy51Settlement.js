/**
 * Rummy51 settlement — claim RPC + vault apply (parity with Ludo / Board Path).
 */
 ...
 */
export async function requestOv2Rummy51ClaimSettlement(roomId, participantKey) {
  ...
  try {
    const { data, error } = await supabaseMP.rpc("ov2_rummy51_claim_settlement", {
      p_room_id: rid,
