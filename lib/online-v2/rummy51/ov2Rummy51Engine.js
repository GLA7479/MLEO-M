...
/**
 * Hand strip: **rank** = group by value (A high), then ♠♥♦♣. **suit** = ♠♥♦♣ then value (A high).
 * Jokers last; tie-break by card id.
 * @param {Rummy51Card[]} cards
 * @param {"rank"|"suit"} mode
 * @returns {Rummy51Card[]}
 */
export function sortHandCards(cards, mode) {
  return [...cards].sort((a, b) => {
    if (a.isJoker !== b.isJoker) return a.isJoker ? 1 : -1;
    if (a.isJoker) return a.id.localeCompare(b.id);
    const rva = a.rank === 1 ? 14 : a.rank;
    const rvb = b.rank === 1 ? 14 : b.rank;
    const ia = SUITS.indexOf(/** @type {"S"|"H"|"D"|"C"} */ (a.suit));
    const ib = SUITS.indexOf(/** @type {"S"|"H"|"D"|"C"} */ (b.suit));
    if (mode === "rank") {
...
export function sortCardsForHand(cards) {
  return sortHandCards(cards, "suit");
}
...
/**
 * Add a natural to a meld that may contain jokers: prefer swapping out a joker when the card
 * completes the meld without needing that joker (joker returns to hand on the server).
 * @param {Rummy51Card[]} meld
 * @param {Rummy51Card} card
 * @param {{ minSize?: number, maxSetSize?: number }} [options]
 * @returns {{ meld: Rummy51Card[], returnedJoker: Rummy51Card|null }|null}
 */
export function tryAddCardToMeldWithJokerSwap(meld, card, options = {}) {
  if (card.isJoker) {
    const app = applyCardToMeld(card, meld, options);
    return app ? { meld: app, returnedJoker: null } : null;
  }
  const hasJoker = meld.some(c => c.isJoker);
  if (hasJoker) {
    for (let ji = 0; ji < meld.length; ji++) {
      if (!meld[ji].isJoker) continue;
      const jokerCard = meld[ji];
      const replaced = [...meld.slice(0, ji), ...meld.slice(ji + 1), card];
      if (classifyMeld(replaced, options) !== "invalid") {
        return { meld: replaced, returnedJoker: jokerCard };
      }
    }
  }
  const appended = applyCardToMeld(card, meld, options);
  if (appended) return { meld: appended, returnedJoker: null };
  return null;
}

/**
 * @param {{ existing: Rummy51Card[], cardsFromHand: Rummy51Card[] }[]} adds
 * @param {{ minSize?: number, maxSetSize?: number }} [options]
 * @returns {{ ok: true, returnedJokers: Rummy51Card[] } | { ok: false }}
 */
export function simulateTableAddsJokerReturns(adds, options = {}) {
  /** @type {Rummy51Card[]} */
  const returnedJokers = [];
  for (const a of adds) {
    let cur = [...a.existing];
    for (const c of a.cardsFromHand) {
      const res = tryAddCardToMeldWithJokerSwap(cur, c, options);
      if (!res) return { ok: false };
      cur = res.meld;
      if (res.returnedJoker) returnedJokers.push(res.returnedJoker);
    }
  }
  return { ok: true, returnedJokers };
}
