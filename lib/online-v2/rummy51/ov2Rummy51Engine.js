...
function isConsecutiveRunWithJokers(cards, mode) {
  ...
  if (vals.length !== new Set(vals).size) return false;
  if (vals.length === 0) {
    return jokers.length >= RUMMY51_MIN_MELD_SIZE;
  }
  const min = Math.min(...vals);
  const max = Math.max(...vals);
  const span = max - min + 1;
  if (cards.length !== span) return false;
  return vals.length + jokers.length === span;
}
...
export function scoreMeld(cards, options = {}) {
  ...
  const mode = isRunInMode(cards, "low") ? "low" : "high";
  const vals = naturalRunValues(cards, mode);
  ...
  /** @type {number[]} */
  const jokerRanks = [];
  for (let v = min; v <= max && jokerRanks.length < jokerCount; v++) {
    if (!used.has(v)) jokerRanks.push(runValueToRank(v, mode));
  }
  let ji = 0;
  let sum = 0;
  for (const c of cards) {
