/**
 * Backgammon settlement delivery — claim RPC + vault apply (shared bridge).
 */
 ...
 */
export async function requestOv2BackgammonClaimSettlement(roomId, participantKey) {
  ...
  try {
    const { data, error } = await supabaseMP.rpc("ov2_backgammon_claim_settlement", {
      p_room_id: rid,
