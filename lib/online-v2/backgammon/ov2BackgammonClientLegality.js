 ...
 *
 * Legal first moves are derived from ALL complete turn paths: repeatedly apply
 * legal steps until no dice remain or no legal move exists (same terminal rule
 * as the server between RPCs). No maximal-dice or larger-die heuristics.
 */
...
export function ov2BgClientLegalFirstMoves(board) {
  const seqs = ov2BgClientAllLegalTurnSequences(board);
  const map = new Map();
  for (const seq of seqs) {
    if (seq.length === 0) continue;
    const f = seq[0];
    const k = `${f.from},${f.to},${f.die}`;
 ...
 * Union of every destination that appears as a legal first step in some full turn sequence.
