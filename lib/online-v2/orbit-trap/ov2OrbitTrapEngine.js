...
/** @param {number[]} [activeSeats] sorted unique seat indices 0–3, length 2–4 */
function normalizeActiveSeats(activeSeats) {
  const raw = Array.isArray(activeSeats) ? activeSeats.map(s => Math.floor(Number(s))).filter(s => s >= 0 && s <= 3) : [];
  const uniq = [...new Set(raw)].sort((a, b) => a - b);
  if (uniq.length >= 2 && uniq.length <= 4) return uniq;
  return [0, 1, 2, 3];
}
 ...
 * @returns {number[]}
 */
export function otActiveRoster(st) {
  return normalizeActiveSeats(st.activeSeats);
}
 ...
 */
function playerOnBoard(p) {
  return p.inPlay !== false;
}
