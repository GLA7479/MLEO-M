/**
 * OV2 Ludo — **pure domain engine** (rules only; no I/O).
 * Source of truth for rules: original `lib/ludoEngine.js` (read-only reference).
 *
 * Layering: `ov2LudoBoardProjection.js` (geometry) → `ov2LudoBoardView.js` (React) →
 * `ov2LudoSessionAdapter.js` (live state boundary) → `ov2LudoLocalPreview.js` (client sandbox only).
 */
...
/**
 * Authoritative and preview boards share this shape (see `Ov2LudoBoardState` in `ov2LudoSessionAdapter.js`).
