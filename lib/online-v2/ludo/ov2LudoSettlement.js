/**
 * Ludo settlement delivery — claim RPC mirrors Board Path; vault apply uses shared bridge helpers.
 */
 ...
 */
export async function requestOv2LudoClaimSettlement(roomId, participantKey) {
  ...
  try {
    const { data, error } = await supabaseMP.rpc("ov2_ludo_claim_settlement", {
      p_room_id: rid,
