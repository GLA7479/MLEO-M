...
/**
 * Host-only rematch entrypoint. Backend may return a fresh session snapshot when prior match is finished.
 * @param {string} roomId
 * @param {string} participantKey
 * @param {{ presenceLeaderKey?: string|null }} [_opts]
 * @returns {Promise<Ov2LudoRpcResult>}
 */
export async function requestOv2LudoRematch(roomId, participantKey, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "room_id and participant_key required" };
  try {
    const { data, error } = await supabase.rpc("ov2_ludo_rematch", {
      p_room_id: roomId,
      p_participant_key: pk,
    });
    if (error) {
