...
/**
 * Pre-session seat strip: committed members in the same order as `ov2_ludo_open_session`
 * (`seat_index ASC NULLS LAST`, then `participant_key`). Up to four slots; empty slots show "—".
 *
...
export function buildLudoLobbySeatStripFromMembers(members, selfParticipantKey) {
  const pk = selfParticipantKey != null ? String(selfParticipantKey).trim() : "";
  const committed = (Array.isArray(members) ? members : [])
    .filter(m => m && typeof m === "object" && m.wallet_state === "committed")
    .sort((a, b) => {
  ...
  if (pk) {
    const idx = committed.findIndex(m => String(m.participant_key) === pk);
...
/**
 * Host opens first session (mirrors Board Path `open_session`). Optional until UI calls it.
 * @param {string} roomId
 * @param {string} participantKey
 * @returns {Promise<Ov2LudoRpcResult>}
 */
export async function requestOv2LudoOpenSession(roomId, participantKey) {
