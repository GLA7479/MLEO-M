 ...
 * @property {Record<string, unknown>} [doubleInitiations] — per-seat counts for offers started this match (server `__double_initiations`)
...
export function normalizeAuthoritativeSnapshot(raw) {
  ...
  return {
    ...
    doubleCycleUsedSeats: Array.isArray(raw.doubleCycleUsedSeats)
      ...
      : [],
    doubleInitiations:
      raw.doubleInitiations && typeof raw.doubleInitiations === "object" && !Array.isArray(raw.doubleInitiations)
        ? /** @type {Record<string, unknown>} */ (raw.doubleInitiations)
        : {},
