 ...
 * Parent-driven: `board` snapshot, optional `mySeat` for highlights, `readOnlyPresentation` for room/no-match UX.
 */
...
export default function Ov2LudoBoardView({
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (containerRect.height === 0 || containerRect.width === 0) {
        ...
        setBoardSize(Math.max(220, Math.min(maxSize, 820)));
        return;
      }
      const availableH = containerRect.height - 8;
      const availableW = containerRect.width - 8;
  ...
  const highlightNumbers = useMemo(() => {
    ...
    return numbers;
  }, [readOnlyPresentation, disableHighlights, effectiveHighlights, board, pieces]);
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col gap-0" ref={containerRef}>
      <div className="flex min-h-0 min-w-0 flex-1 items-center justify-center overflow-hidden">
        <div
          className={`relative aspect-square shrink-0 overflow-hidden rounded-2xl border-2 bg-black shadow-2xl ${
            readOnlyPresentation ? "border-amber-500/50 opacity-85" : "border-white/30"
          }`}
          style={{
            width: boardSize ? `${boardSize}px` : "min(98vmin, 100%)",
            height: boardSize ? `${boardSize}px` : "min(98vmin, 100%)",
            maxWidth: "100%",
            maxHeight: "100%",
          }}
        >
          {readOnlyPresentation ? (
            <div className="pointer-events-none absolute inset-x-0 top-1 z-40 flex justify-center">
              <span className="rounded bg-amber-950/90 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-amber-100">
                Read-only
              </span>
            </div>
          ) : null}
          ...
          {(diceValue != null || diceInteractive) && (
            <div
              ...
              aria-label={diceInteractive ? "Roll dice" : "Dice"}
              onClick={() => {
