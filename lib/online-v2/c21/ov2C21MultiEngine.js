...
import { OV2_C21_BETTING_PRE_LOCK_FREEZE_MS } from "./ov2C21ClientConstants";
...
function emptySeat(i) {
  return {
    ...
    intendedBet: 0,
    /** True after set_bet secured main stake for the upcoming round (server debited). */
    ...
    handStakes: [],
    handMeta: [],
    /** True after this seat has performed a split on a pair of Aces this round (blocks further Ace splits). */
    didSplitAces: false,
    ...
    /** Server-authoritative auto re-entry for the next betting round (separate from missed-bet cleanup). */
    autoNextEnabled: false,
    autoNextStake: 0,
    /** Consecutive completed rounds with no real manual action while seated in-round (cap at 3 disables auto-next). */
    autoNextInactiveRounds: 0,
    /** Set true on manual ops (hit/stand/…); cleared when entering a new in-round deal. */
    c21HadManualActionThisRound: false,
  };
}
...
export function buildFreshEngine(tableStakeUnits) {
  const now = Date.now();
  const stake = Math.max(10, Math.floor(Number(tableStakeUnits) || 10));
  return {
    v: 1,
    ...
    /** When true, skip auto-next inactivity increments (dealer opening natural — no acting phase). */
    c21DealerOpeningBlackjackRound: false,
...
export function normalizeEngine(raw, tableStakeUnits) {
  ...
  e.v = 1;
  if (typeof e.c21DealerOpeningBlackjackRound !== "boolean") e.c21DealerOpeningBlackjackRound = false;
  if (!Array.isArray(e.seats) || e.seats.length !== 6) {
    e.seats = Array.from({ length: 6 }, (_, i) => emptySeat(i));
  } else {
    e.seats = e.seats.map((s, i) => {
      const merged = { ...emptySeat(i), ...s, seatIndex: i };
      merged.didSplitAces =
        Boolean(s.didSplitAces) ||
        (Array.isArray(s.handMeta) && s.handMeta.some(m => m && m.splitAces));
      if ("splitUsed" in merged) delete merged.splitUsed;
      return merged;
    });
  }
  return e;
...
function clearAutoNextPrefs(seat) {
  if (!seat) return;
  seat.autoNextEnabled = false;
  seat.autoNextStake = 0;
}

function vacateAutoNextState(seat) {
  if (!seat) return;
  clearAutoNextPrefs(seat);
  seat.autoNextInactiveRounds = 0;
  seat.c21HadManualActionThisRound = false;
}

function markManualC21Round(seat) {
  if (!seat) return;
  seat.c21HadManualActionThisRound = true;
}

/**
 * After betting reset, auto-commit seats with server-side auto-next (economy ops + vault applied in API).
 * @param {object} engine
 * @param {any[]} economyOps
 */
function applyAutoNextBettingCommits(engine, economyOps) {
  ...
  for (const seat of engine.seats) {
    ...
    if (!seat.autoNextEnabled) continue;
    if ((seat.autoNextInactiveRounds || 0) >= 3) {
      clearAutoNextPrefs(seat);
      continue;
    }
    const amt = Math.floor(Number(seat.autoNextStake) || 0);
    ...
    economyOps.push({
      ...
      amount: clamped,
      suffix: `main_r${nextRid}_s${seat.seatIndex}_a${clamped}_auto`,
      c21AutoNextSeat: seat.seatIndex,
    });
    seat.intendedBet = clamped;
    seat.betCommitRecorded = true;
  }
}

function phaseEndTimeMs(v) {
  if (v == null) return 0;
...
function isBettingPreRoundFreeze(engine, t) {
  if (engine.phase !== "betting") return false;
  const end = phaseEndTimeMs(engine.phaseEndsAt);
  if (end <= 0) return false;
  return t >= end - OV2_C21_BETTING_PRE_LOCK_FREEZE_MS;
}
...
function applyAfkAndCollectCommits(engine, economyOps) {
  ...
  for (const seat of engine.seats) {
    ...
    /** Main stake must already be debited via set_bet (betCommitRecorded). No tick-time commits. */
    if (valid && seat.betCommitRecorded) {
      anyValidBet = true;
      seat.consecutiveMissRounds = 0;
      seat.roundBet = bet;
      seat.inRound = true;
      ...
      if (seat.consecutiveMissRounds >= 2) {
        vacateAutoNextState(seat);
        seat.participantKey = null;
        seat.displayName = null;
        seat.consecutiveMissRounds = 0;
        seat.intendedBet = 0;
        seat.betCommitRecorded = false;
      }
    }
    seat.intendedBet = 0;
    seat.betCommitRecorded = false;
...
function finalizeDealerAndSettle(engine, now, economyOps) {
  ...
  if (engine.c21DealerOpeningBlackjackRound) {
    engine.c21DealerOpeningBlackjackRound = false;
  } else {
    for (const seat of engine.seats) {
      if (!seat.participantKey || !seat.inRound) continue;
      if (!seat.c21HadManualActionThisRound) {
        seat.autoNextInactiveRounds = Math.min(99, (seat.autoNextInactiveRounds || 0) + 1);
      } else {
        seat.autoNextInactiveRounds = 0;
      }
      if ((seat.autoNextInactiveRounds || 0) >= 3) {
...
function startNextBetting(engine, now, economyOps) {
  ...
  applyAutoNextBettingCommits(engine, economyOps);
}
...
export function mutateEngine(prev, { op, participantKey, payload, now }) {
  ...
  if (op === "tick") {
    if (engine.phase === "betting" && t >= engine.phaseEndsAt) {
      ...
      if (!hasPlayers) {
        startNextBetting(engine, t, economyOps);
        return { engine, economyOps };
      }
      const branch = runDeal(engine);
      if (branch === "settle_early") {
        engine.c21DealerOpeningBlackjackRound = true;
        finalizeDealerAndSettle(engine, t, economyOps);
        return { engine, economyOps };
      }
      if (branch === "insurance") {
  ...
  if (op === "sit") {
    ...
    for (const s of engine.seats) {
      if (s.participantKey === pk && s.seatIndex !== seatIndex) {
        ...
        clearAutoNextPrefs(s);
        s.autoNextInactiveRounds = 0;
        s.c21HadManualActionThisRound = false;
      }
    }
  ...
  if (op === "leave_seat") {
    ...
    if (engine.phase === "betting") {
      if (isBettingPreRoundFreeze(engine, t)) {
        return { engine: prev, economyOps, error: "pre_round_freeze" };
      }
      if (seat.betCommitRecorded) {
        const ref = Math.floor(Number(seat.intendedBet) || 0);
        if (ref > 0) {
          economyOps.push({
            type: "credit",
            participantKey: pk,
            amount: ref,
            suffix: `betting_leave_r${engine.roundSeq}_s${idx}`,
            lineKind: "REFUND",
          });
        }
      }
  ...
  if (op === "set_auto_next") {
    ...
    if (!enabled) {
      clearAutoNextPrefs(seat);
      markManualC21Round(seat);
      return { engine, economyOps };
    }
    const minStake = engine.tableStakeUnits;
    const maxStake = maxBetForTable(minStake);
    const stake = Math.floor(Number(payload?.stake) || 0);
    const clamped = Math.max(0, Math.min(maxStake, stake));
    if (clamped < minStake) {
      return { engine: prev, economyOps, error: "invalid_auto_next_stake" };
    }
    seat.autoNextEnabled = true;
    seat.autoNextStake = clamped;
    seat.autoNextInactiveRounds = 0;
    markManualC21Round(seat);
    return { engine, economyOps };
  }
  ...
  if (op === "set_bet") {
    ...
    const clamped = Math.max(0, Math.min(maxStake, amt));
    const nextRid = Math.max(1, Math.floor(Number(engine.roundSeq) || 0) + 1);
    const prevSecured = seat.betCommitRecorded ? Math.floor(Number(seat.intendedBet) || 0) : 0;

    if (clamped < minStake) {
      if (
        isBettingPreRoundFreeze(engine, t) &&
        seat.betCommitRecorded &&
        prevSecured > 0
      ) {
        return { engine: prev, economyOps, error: "pre_round_freeze" };
      }
      if (seat.betCommitRecorded && prevSecured > 0) {
        economyOps.push({
          type: "credit",
          participantKey: pk,
          amount: prevSecured,
          suffix: `main_clear_r${nextRid}_s${seat.seatIndex}`,
          lineKind: "REFUND",
        });
      }
      seat.intendedBet = 0;
    ...
    if (seat.betCommitRecorded && prevSecured > 0 && prevSecured !== clamped) {
      economyOps.push({
        type: "credit",
        participantKey: pk,
        amount: prevSecured,
        suffix: `main_swapout_r${nextRid}_s${seat.seatIndex}_p${prevSecured}`,
        lineKind: "REFUND",
      });
    }
    ...
    economyOps.push({
      ...
      participantKey: pk,
      amount: clamped,
      suffix: `main_r${nextRid}_s${seat.seatIndex}_a${clamped}`,
    });
    seat.intendedBet = clamped;
    seat.betCommitRecorded = true;
    markManualC21Round(seat);
    if (seat.autoNextEnabled) {
      seat.autoNextStake = clamped;
    }
    return { engine, economyOps };
  }
  ...
  if (op === "clear_bet") {
    ...
    const prevSecured = Math.floor(Number(seat.intendedBet) || 0);
    const nextRid = Math.max(1, Math.floor(Number(engine.roundSeq) || 0) + 1);
    if (prevSecured > 0) {
      economyOps.push({
        type: "credit",
        participantKey: pk,
        amount: prevSecured,
        suffix: `main_uncommit_r${nextRid}_s${seat.seatIndex}_a${prevSecured}`,
        lineKind: "REFUND",
      });
    }
    seat.intendedBet = 0;
  ...
  if (actionOps.includes(op)) {
    ...
    if (op === "split") {
      if (seat.hands.length >= 4) return { engine: prev, economyOps, error: "split_illegal" };
      if (!m.firstAction || cards.length !== 2) {
      ...
      const pairOfAces = splitRankKey(c0) === "A" && splitRankKey(c1) === "A";
      if (pairOfAces && seat.didSplitAces) {
        return { engine: prev, economyOps, error: "split_illegal" };
      }
      markManualC21Round(seat);
      const stake = seat.handStakes[hi];
      economyOps.push({
        ...
        participantKey: pk,
        amount: stake,
        suffix: `split_r${engine.roundSeq}_s${seat.seatIndex}_h${hi}`,
      });
      ...
      const newHands = [...seat.hands.slice(0, hi), [c0, d0], [c1, d1], ...seat.hands.slice(hi + 1)];
      const newStakes = [...seat.handStakes.slice(0, hi), stake, stake, ...seat.handStakes.slice(hi + 1)];
      const newMeta = pairOfAces
        ? [...seat.handMeta.slice(0, hi), aceMeta, aceMeta, ...seat.handMeta.slice(hi + 1)]
        : [...seat.handMeta.slice(0, hi), splitPairMeta(), splitPairMeta(), ...seat.handMeta.slice(hi + 1)];

      seat.hands = newHands;
      seat.handStakes = newStakes;
      seat.handMeta = newMeta;
      if (pairOfAces) seat.didSplitAces = true;

      engine.currentTurn = { seatIndex: ct.seatIndex, handIndex: hi };
      afterPlayerHandFinished(engine, t, economyOps);
      return { engine, economyOps };
    }
  }
