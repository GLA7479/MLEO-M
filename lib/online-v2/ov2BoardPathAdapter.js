...
function sessionSelfIsActiveSeat(session, seats, selfKey) {
  const ai = session?.turnMeta?.activeSeatIndex;
  if (ai == null || Number.isNaN(Number(ai))) return undefined;
  if (!Array.isArray(seats) || !selfKey) return undefined;
...
/** @param {Ov2BoardPathSessionLike|null} session */
function sessionRowEnded(session) {
  if (!session) return false;
  if (session.you_won || session.opponent_won) return true;
  if (session.engine_state === "ended") return true;
  if (session.engine_phase === "ended") return true;
  return false;
}

/** Active seat index for session-driven UI — `turnMeta` only. */
function adapterPrimaryActiveSeatIndex(session) {
  return session?.turnMeta?.activeSeatIndex ?? null;
}

/** @param {Ov2BoardPathSessionLike|null} session */
function isTurnDataPartial(session) {
  if (!session) return true;
  if (sessionRowEnded(session)) return false;
  if (session.engine_phase === "pregame") return true;
  if (session.engine_phase === "playing") {
    const a = adapterPrimaryActiveSeatIndex(session);
 ...
 */
export function deriveBoardPathViewModel(raw) {
  ...
  if (phase === ONLINE_V2_ROOM_PHASE.CLOSED || phase === ONLINE_V2_ROOM_PHASE.ABORTED) {
    return vmFinished(meta, selfMember, opp, session, members, selfKey, tier);
  }
  ...
  if (phase === ONLINE_V2_ROOM_PHASE.ACTIVE) {
    ...
    if (sessionRowEnded(session)) {
      return vmFinished(meta, selfMember, opp, session, members, selfKey, tier);
    }
    const seatsComplete = seats == null ? undefined : boardPathSeatsLookComplete(seats, count);
    const turnPartial = isTurnDataPartial(session);
    if (seatsComplete === false || turnPartial) {
      return vmSessionReady(meta, selfMember, opp, session, members, selfKey, tier, seatsComplete, turnPartial);
    }
...
/** @returns {BoardPathViewModel} */
function vmSessionReady(meta, selfMember, opp, session, members, selfKey, tier, seatsComplete, turnPartial) {
  const oppConnected = Boolean(opp);
  ...
  return withStake(
    {
      ...
      statusLine: `Session loaded — ${detail}. Controls stay off until engine is ready.`,
      turnLine: undefined,
      primary: ctrl(BOARD_PATH_CONTROL_INTENT.SYNC_SESSION, true, true),
      secondary: ctrl(BOARD_PATH_CONTROL_INTENT.STAND_BY, true, true),
      tokenSlots: { you: yourSlot, opp: oppSlot },
      playerBadges: { you: "Loading", opp: oppConnected ? "Loading" : "—" },
      youConnected: true,
      oppConnected,
      meta,
    },
...
function vmSessionActive(meta, selfMember, opp, session, members, selfKey, tier, seatsComplete, seats) {
  ...
  /** `END_TURN` is for your turn after a roll (future); opponent turn uses stand-by. */
  const secondary =
    selfActive === true
      ? ctrl(BOARD_PATH_CONTROL_INTENT.CHOOSE_TOKEN, true, true)
      : ctrl(BOARD_PATH_CONTROL_INTENT.STAND_BY, true, true);

  const statusLine =
    selfActive === true
      ? `Your move — engine not wired (${seatHint}).`
      : selfActive === false
        ? `Opponent's move — waiting. (${seatHint})`
        : `Session active — pass seats[] to resolve turn (${seatHint}).`;
  const turnLine =
    selfActive === true ? "Your move" : selfActive === false ? "Their move" : "Turn";
  ...
  return withStake(
    {
      ...
      secondary,
      tokenSlots: { you: yourSlot, opp: oppSlot },
      playerBadges: { you: "In play", opp: oppConnected ? "In play" : "—" },
      youConnected: true,
      oppConnected,
      meta,
    },
...
/** @returns {BoardPathViewModel} */
function vmFinished(meta, selfMember, opp, session, members, selfKey, tier) {
  const oppConnected = Boolean(opp);
  const youWon = Boolean(session?.you_won);
  const oppWon = Boolean(session?.opponent_won);
  ...
  return withStake(
    {
      ...
      tokenSlots: { you: PATH_SLOTS - 1, opp: oppConnected ? PATH_SLOTS - 1 : null },
