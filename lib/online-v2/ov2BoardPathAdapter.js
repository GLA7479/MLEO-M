...
import {
  buildBoardPathRoomSettlementSummary,
  canBoardPathFinalizeRoom,
  getBoardPathRoomFinalizationLabel,
  getBoardPathRoomSettlementStatus,
} from "./board-path/ov2BoardPathRoomFinalize";
import {
  BOARD_PATH_SETTLEMENT_DELIVERY_UI_PHASE,
  canBoardPathClaimSettlementForVault,
  deriveBoardPathSettlementDeliveryUiPhase,
  getBoardPathSettlementClaimStatusLabel,
  getBoardPathSettlementDeliveryHintLine,
  settlementDeliveryUiPhaseAllowsClaimButton,
} from "./board-path/ov2BoardPathSettlementDelivery";
/**
 * -----------------------------------------------------------------------------
...
/**
 * Canonical session + seats + board pipeline (single source of truth for UI gating).
 * @readonly
 */
export const BOARD_PATH_SESSION_STATE = Object.freeze({
  MISSING: "session_missing",
  OPENING: "session_opening",
  NO_SEATS: "session_no_seats",
  HYDRATING: "session_hydrating",
  READY: "session_ready",
 ...
 * @property {number|null} [finalized_match_seq]
 */
/**
 ...
 * @property {Record<string, unknown>[]|null} [roomSettlementLines] all settlement lines for the room (Phase 6)
 * @property {Record<string, unknown>[]|null} [boardPathSessions] compact session rows for the room (Phase 6)
 */
/**
 ...
 * @property {(typeof BOARD_PATH_SESSION_STATE)[keyof typeof BOARD_PATH_SESSION_STATE]} sessionState
 * @property {unknown|null} [lastEvent]
 ...
 * @property {string|null} [liveRealtimeChannelId]
 * @property {string|null} [liveRealtimeSubscribeStatus]
 * @property {number|null} [liveRealtimeLastPostgresChangeAt]
 ...
 * @property {boolean} [roomFinalized]
 * @property {string} [roomSettlementStatus]
 * @property {number} [roomSettlementRevision]
 * @property {string|null} [roomFinalizedAt]
 * @property {boolean} [roomCanFinalize]
 * @property {boolean} [hostCanFinalizeRoom]
 * @property {boolean} [roomFinalizeBusy]
 * @property {{ code?: string, message?: string }|null} [roomFinalizeError]
 * @property {{ participantKey: string, totalAmount: number }[]} [roomSettlementSummary]
 * @property {string} [roomStatusLabel]
 * @property {boolean} [selfCanClaimSettlement]
 * @property {string} [settlementClaimStatusLabel]
 * @property {boolean} [settlementClaimBusy]
 * @property {{ code?: string, message?: string }|null} [settlementClaimError]
 * @property {string} [settlementDeliveryUiPhase]
 * @property {string} [settlementDeliveryHintLine]
 * @property {boolean} [settlementDeliveryClaimButtonEnabled]
 * @property {boolean} [settlementVaultReliabilityGapVisible]
 * @property {boolean} [commitStakeBusy] — hook: stake RPC in flight (CTA pending).
 */
/**
 ...
 * @property {null|"roll"|"move"|"end_turn"|"commit_stake"} actionPending
 ...
 * @param {{ actionPending?: null|"roll"|"move"|"end_turn"|"commit_stake", actionError?: { code?: string, message?: string }|null }|null|undefined} actionOpts
 ...
 * @param {{ finalizeBusy?: boolean, finalizeError?: { code?: string, message?: string }|null, liveDbBoardPath?: boolean, roomFinalizeBusy?: boolean, roomFinalizeError?: { code?: string, message?: string }|null }} [hookFlags]
 */
...
/**
 * Room-level finalization slice (Phase 6).
 * @param {Ov2BoardPathContext|null|undefined} rawContext
 * @param {string|null|undefined} hostKey
 * @param {{ liveDbBoardPath?: boolean, roomFinalizeBusy?: boolean, roomFinalizeError?: { code?: string, message?: string }|null }} [hookFlags]
 */
function buildBoardPathRoomViewSlice(rawContext, hostKey, hookFlags) {
  const ctx = rawContext && typeof rawContext === "object" ? rawContext : createDisconnectedBoardPathContext();
  const room = ctx.room && typeof ctx.room === "object" ? ctx.room : null;
  const sessions = Array.isArray(ctx.boardPathSessions) ? ctx.boardPathSessions : [];
  const roomLines = Array.isArray(ctx.roomSettlementLines) ? ctx.roomSettlementLines : [];
  const selfKey = ctx.self?.participant_key?.trim() || null;
  const hk = (hostKey && String(hostKey).trim()) || (room?.host_participant_key && String(room.host_participant_key).trim()) || null;
  const liveDb = Boolean(hookFlags?.liveDbBoardPath);
  const z = {
    roomFinalized: false,
    roomSettlementStatus: "pending",
    roomSettlementRevision: 0,
    roomFinalizedAt: null,
    roomCanFinalize: false,
    hostCanFinalizeRoom: false,
    roomFinalizeBusy: Boolean(hookFlags?.roomFinalizeBusy),
    roomFinalizeError: hookFlags?.roomFinalizeError ?? null,
    roomSettlementSummary: /** @type {{ participantKey: string, totalAmount: number }[]} */ ([]),
    roomStatusLabel: "",
  };
  if (!room || !selfKey) return z;
  const rss = getBoardPathRoomSettlementStatus(room);
  const canR = canBoardPathFinalizeRoom(room, sessions, selfKey, hk, liveDb);
  return {
    roomFinalized: rss === "finalized",
    roomSettlementStatus: rss,
    roomSettlementRevision: Math.floor(Number(room.settlement_revision ?? room.settlementRevision) || 0),
    roomFinalizedAt: room.finalized_at != null ? String(room.finalized_at) : room.finalizedAt != null ? String(room.finalizedAt) : null,
    roomCanFinalize: canR,
    hostCanFinalizeRoom: canR,
    roomFinalizeBusy: Boolean(hookFlags?.roomFinalizeBusy),
    roomFinalizeError: hookFlags?.roomFinalizeError ?? null,
    roomSettlementSummary: buildBoardPathRoomSettlementSummary(roomLines),
    roomStatusLabel: getBoardPathRoomFinalizationLabel(room, sessions),
  };
}
/**
 * @param {Ov2BoardPathContext|null|undefined} rawContext
 ...
 *     rpcIdempotentEmpty: boolean,
 *     vaultCreditableAttempted: number,
 *     vaultCreditedCount: number,
 *     vaultFailedCount: number,
 *     vaultSkippedLocalIdemCount: number,
 *     vaultGapAfterDbMark: boolean,
 *     vaultSuccessAll: boolean,
 *   }|null,
 * }} [hookFlags]
 */
function buildBoardPathSettlementDeliveryVmSlice(rawContext, hookFlags) {
  const ctx = rawContext && typeof rawContext === "object" ? rawContext : createDisconnectedBoardPathContext();
  const room = ctx.room && typeof ctx.room === "object" ? ctx.room : null;
  const selfKey = ctx.self?.participant_key?.trim() || null;
  const roomLines = Array.isArray(ctx.roomSettlementLines) ? ctx.roomSettlementLines : [];
  const liveDb = Boolean(hookFlags?.liveDbBoardPath);
  const roomFinalized = room ? getBoardPathRoomSettlementStatus(room) === "finalized" : false;
  const lastTouch = hookFlags?.settlementClaimLastTouch ?? null;
  const z = {
    selfCanClaimSettlement: false,
    settlementClaimStatusLabel: "",
    settlementClaimBusy: Boolean(hookFlags?.settlementClaimBusy),
    settlementClaimError: hookFlags?.settlementClaimError ?? null,
    settlementDeliveryUiPhase: BOARD_PATH_SETTLEMENT_DELIVERY_UI_PHASE.INACTIVE,
    settlementDeliveryHintLine: "",
    settlementDeliveryClaimButtonEnabled: false,
    settlementVaultReliabilityGapVisible: false,
  };
  if (!room || !selfKey) return z;
  const selfCanClaimSettlement = canBoardPathClaimSettlementForVault(room, selfKey, roomLines, liveDb);
  const settlementDeliveryUiPhase = deriveBoardPathSettlementDeliveryUiPhase({
    liveDbBoardPath: liveDb,
    roomFinalized,
    selfKey,
    roomSettlementLines: roomLines,
    selfCanClaimSettlement,
    settlementClaimBusy: Boolean(hookFlags?.settlementClaimBusy),
    settlementClaimError: hookFlags?.settlementClaimError ?? null,
    settlementClaimLastTouch: lastTouch,
  });
  const settlementDeliveryHintLine = getBoardPathSettlementDeliveryHintLine(settlementDeliveryUiPhase);
  const settlementDeliveryClaimButtonEnabled = settlementDeliveryUiPhaseAllowsClaimButton(settlementDeliveryUiPhase);
  const settlementVaultReliabilityGapVisible =
    settlementDeliveryUiPhase === BOARD_PATH_SETTLEMENT_DELIVERY_UI_PHASE.VAULT_GAP_NO_DB_RETRY ||
    settlementDeliveryUiPhase === BOARD_PATH_SETTLEMENT_DELIVERY_UI_PHASE.VAULT_GAP_PARTIAL;
  return {
    selfCanClaimSettlement,
    settlementClaimStatusLabel: getBoardPathSettlementClaimStatusLabel(room, selfKey, roomLines, liveDb),
    settlementClaimBusy: Boolean(hookFlags?.settlementClaimBusy),
    settlementClaimError: hookFlags?.settlementClaimError ?? null,
    settlementDeliveryUiPhase,
    settlementDeliveryHintLine,
    settlementDeliveryClaimButtonEnabled,
    settlementVaultReliabilityGapVisible,
  };
}
/**
 ...
 *   liveDbBoardPath?: boolean,
 *   roomFinalizeBusy?: boolean,
 *   roomFinalizeError?: { code?: string, message?: string }|null,
 *   settlementClaimBusy?: boolean,
 *   settlementClaimError?: { code?: string, message?: string }|null,
 *   settlementClaimLastTouch?: import("./board-path/ov2BoardPathSettlementDelivery").SettlementClaimLastTouch | null,
 * }} [hookFlags]
 */
export function buildBoardPathPostFinishSlice(rawContext, hostKey, hookFlags) {
  ...
  const roomVm = buildBoardPathRoomViewSlice(rawContext, hostKey, hookFlags);
  const deliveryVm = buildBoardPathSettlementDeliveryVmSlice(rawContext, hookFlags);
  const busy =
    Boolean(hookFlags?.rematchBusy) ||
    Boolean(hookFlags?.finalizeBusy) ||
    Boolean(hookFlags?.roomFinalizeBusy) ||
    Boolean(hookFlags?.settlementClaimBusy);
  ...
  const empty = {
    ...
    postFinishStatusLabel: [
      settlementVm.settlementStatusLabel,
      roomVm.roomStatusLabel,
      deliveryVm.settlementClaimStatusLabel,
      deliveryVm.settlementDeliveryHintLine,
    ]
      .filter(Boolean)
      .join(" · "),
  };
...
/**
 * Board payload present (path length + positions object). No synthetic defaults.
 * @param {Ov2BoardPathSessionLike|null|undefined} session
 */
export function boardPathSessionHasBoard(session) {
  const b = session?.board_state ?? session?.boardState ?? session?.board;
  if (!b || typeof b !== "object") return false;
  const pl = /** @type {{ pathLength?: number, path_length?: number }} */ (b).pathLength ?? /** @type {{ path_length?: number }} */ (b).path_length;
  if (typeof pl !== "number" || Number.isNaN(pl) || pl < 1) return false;
  const pos = /** @type {{ positions?: unknown }} */ (b).positions;
  return pos != null && typeof pos === "object";
}
/**
 * @param {Ov2BoardPathRoomLike|null|undefined} room
 * @param {Ov2BoardPathSessionLike|null|undefined} session
 * @param {Ov2BoardPathSeatRowLike[]|null|undefined} seats
 * @returns {keyof typeof BOARD_PATH_SESSION_STATE}
 */
export function deriveBoardPathSessionState(room, session, seats) {
  const seatsArr = Array.isArray(seats) ? seats : [];
  if (!room || typeof room !== "object") {
    return BOARD_PATH_SESSION_STATE.MISSING;
  }
  const lifecycle = room.lifecycle_phase;

  if (lifecycle === ONLINE_V2_ROOM_PHASE.ACTIVE) {
    const aid = room.active_session_id != null && String(room.active_session_id).trim() !== "";
    if (!aid) {
      return BOARD_PATH_SESSION_STATE.MISSING;
    }
    if (!boardPathSessionIdMatchesRoom(session, room)) {
      return BOARD_PATH_SESSION_STATE.OPENING;
    }
    if (sessionRowEnded(session, seatsArr)) {
      return BOARD_PATH_SESSION_STATE.FINISHED;
    }
    if (seatsArr.length === 0) {
      return BOARD_PATH_SESSION_STATE.NO_SEATS;
    }
    if (!boardPathSessionHasBoard(session)) {
      return BOARD_PATH_SESSION_STATE.HYDRATING;
    }
    const enginePhase = session?.phase ?? session?.engine_phase;
    if (enginePhase === "playing") {
      return BOARD_PATH_SESSION_STATE.ACTIVE;
    }
    return BOARD_PATH_SESSION_STATE.READY;
  }

  if (session && sessionRowEnded(session, seatsArr)) {
    return BOARD_PATH_SESSION_STATE.FINISHED;
  }
  return BOARD_PATH_SESSION_STATE.MISSING;
}
 ...
 * @param {Ov2BoardPathContext|null|undefined} raw
 * @param {{
 *   actionPending?: null|"roll"|"move"|"end_turn"|"commit_stake",
 ...
 *     liveRealtimeChannelId: string|null,
 *     liveRealtimeSubscribeStatus: string|null,
 *     liveRealtimeLastPostgresChangeAt: number|null,
 *   }>,
...
export function deriveBoardPathViewModel(raw, opts) {
  ...
  const session = ctx.session && typeof ctx.session === "object" ? ctx.session : null;
  const seats = Array.isArray(ctx.seats) ? ctx.seats : null;
  const sessionState = deriveBoardPathSessionState(room, session, seats);
