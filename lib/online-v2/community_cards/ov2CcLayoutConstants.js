 ...
 * Must match action dock `min-h-[134px]` in Ov2CcScreen.js (no “Actions” label — tighter strip).
 */
export const OV2_CC_MOBILE_ACTION_RESERVE_PX = 134;

/** Mobile (max-sm): felt fills viewport up to reserved action strip (78% × app height − reserve; no “Actions” label). */
...
/** Mobile: hero / hole-card strip — centered, no negative translate (avoids clipping). */
export const OV2_CC_MOBILE_HERO_ZONE_CLASSES =
  "max-sm:h-[9.75rem] max-sm:min-h-[9.75rem] max-sm:items-center max-sm:overflow-visible max-sm:pb-2 max-sm:pt-1";
