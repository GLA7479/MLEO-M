...
test("three-handed SB and BTN fold BB wins hand settles to between_hands", () => {
  const er = { e: buildFreshEngine(cfg9) };
  const pr = { p: normalizePrivatePayload({}) };
  let t = 570_000;
  for (let i = 0; i < 3; i++) {
  ...
  const pkBtn = e.seats[e.buttonSeat].participantKey;
  assert.ok(!apply(er, pr, { op: "fold", participantKey: pkSb, payload: {}, now: t++ }).error);
  e = er.e;
  assert.ok(!apply(er, pr, { op: "fold", participantKey: pkBtn, payload: {}, now: t++ }).error);
  e = er.e;
  assert.equal(e.phase, "showdown");
