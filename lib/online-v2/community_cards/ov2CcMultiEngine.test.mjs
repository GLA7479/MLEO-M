...
test("heads-up: call vs shorter all-in auto-runs board (no postflop check prompts)", () => {
  const er = { e: buildFreshEngine(cfg9) };
  const pr = { p: normalizePrivatePayload({}) };
  let t = 11_000;
  apply(er, pr, {
    op: "sit",
    participantKey: "p0",
    payload: { seatIndex: 0, buyIn: 200, displayName: "A" },
    now: t++,
  });
  apply(er, pr, {
    op: "sit",
    participantKey: "p1",
    payload: { seatIndex: 1, buyIn: 600, displayName: "B" },
    now: t++,
  });
  apply(er, pr, { op: "tick", participantKey: "", payload: {}, now: t++ });
  apply(er, pr, { op: "tick", participantKey: "", payload: {}, now: t++ });
  ...
  assert.ok(!apply(er, pr, { op: "all_in", participantKey: pkSb, payload: {}, now: t++ }).error);
  e = er.e;
  assert.equal(e.seats[e.sbSeat].allIn, true);
  assert.equal(e.actionSeat, e.bbSeat);
  assert.ok(!apply(er, pr, { op: "call", participantKey: pkBb, payload: {}, now: t++ }).error);
  e = er.e;
  assert.equal(e.phase, "showdown");
  assert.equal(e.communityCards.length, 5);
  assert.equal(e.actionSeat, null);
  assert.equal(e.seats[e.bbSeat].allIn, false);
  assert.ok(String(e.tableNotice || "").includes("All-in"));
  assert.equal(validateCcEngineInvariants(e), null);
});

test("heads-up: preflop call-down with nobody all-in reaches flop with action (no premature showdown)", () => {
  const er = { e: buildFreshEngine(cfg9) };
  const pr = { p: normalizePrivatePayload({}) };
  let t = 11_500;
  apply(er, pr, {
  ...
  assert.equal(e.phase, "preflop");
  assert.notEqual(e.phase, "showdown");
  assert.equal(e.seats[0].allIn, false);
  assert.equal(e.seats[1].allIn, false);
...
test("action timeout sets tableNotice and advances", () => {
  ...
  apply(er, pr, { op: "tick", participantKey: "", payload: {}, now: dl + OV2_CC_ACTION_MS + OV2_CC_ACTION_SERVER_GRACE_MS + 50 });
  e = er.e;
...
test("distributeSidePotsToWinners even chops when holes/revealed missing (stuck-showdown recovery)", () => {
  const e = buildFreshEngine(cfg9);
  e.pot = 200;
  e.communityCards = ["3s", "Jd", "8s"];
  e.street = "river";
  e.phase = "showdown";
  e.handSeq = 35;
  for (const s of e.seats) {
  ...
  const priv = normalizePrivatePayload({ holes: {}, revealed: {}, deck: [] });
  const d = distributeSidePotsToWinners(e, priv);
  assert.equal(d.error, null);
  assert.equal(d.stackAdds[0] + d.stackAdds[1], 200);
  assert.equal(d.stackAdds[0], 100);
...
test("leave_seat when caller has no seat is idempotent noop", () => {
  const e = buildFreshEngine(cfg9);
  const priv = normalizePrivatePayload({});
  const r = mutateEngine(e, priv, {
    op: "leave_seat",
    participantKey: "00000000-0000-0000-0000-00000000dead",
    payload: {},
    now: 1,
    config: cfg9,
  });
  assert.equal(r.error, null);
  assert.equal(r.leaveNotSeatedNoop, true);
});

test("orbitEstablished is false before any hand is dealt", () => {
  const e = buildFreshEngine(cfg9);
  assert.equal(orbitEstablished(e), false);
  e.handSeq = 1;
  assert.equal(orbitEstablished(e), true);
});

test("fresh 5-max: five seats before first tick — all play hand 1, no waitBb", () => {
  const er = { e: buildFreshEngine(cfg5) };
  const pr = { p: normalizePrivatePayload({}) };
  let t = 2_000_000;
  for (let i = 0; i < 5; i++) {
    const r = mutateEngine(er.e, pr.p, {
      op: "sit",
      participantKey: `p${i}`,
      payload: { seatIndex: i, buyIn: 500, displayName: `P${i}` },
      now: t++,
    ...
    assert.ok(!r.error);
    er.e = r.engine;
    pr.p = r.privatePayload;
    assert.equal(er.e.seats[i].waitBb, false);
  }
  assert.equal(orbitEstablished(er.e), false);
  {
    const t1 = mutateEngine(er.e, pr.p, { op: "tick", participantKey: "", payload: {}, now: t++, config: cfg5 });
    assert.ok(!t1.error);
    er.e = t1.engine;
    pr.p = t1.privatePayload;
  }
  {
    const t2 = mutateEngine(er.e, pr.p, { op: "tick", participantKey: "", payload: {}, now: t++, config: cfg5 });
    assert.ok(!t2.error);
    er.e = t2.engine;
    pr.p = t2.privatePayload;
  }
  const e = er.e;
  assert.equal(e.phase, "preflop");
  assert.equal(e.handSeq, 1);
  let inHand = 0;
  for (let i = 0; i < 5; i++) {
    if (e.seats[i].inCurrentHand) inHand += 1;
    assert.equal(e.seats[i].waitBb, false);
  }
  assert.equal(inHand, 5);
  assert.equal(Object.keys(pr.p.holes || {}).length, 5);
});

test("fresh 5-max: four seats then fifth before first deal — all five in hand 1", () => {
  const er = { e: buildFreshEngine(cfg5) };
  const pr = { p: normalizePrivatePayload({}) };
  let t = 2_010_000;
  for (let i = 0; i < 4; i++) {
  ...
  {
    const r4 = mutateEngine(er.e, pr.p, {
      op: "sit",
      participantKey: "p4",
      payload: { seatIndex: 4, buyIn: 500, displayName: "P4" },
      now: t++,
      config: cfg5,
    });
    assert.ok(!r4.error);
    er.e = r4.engine;
    pr.p = r4.privatePayload;
  }
  assert.equal(er.e.seats[4].waitBb, false);
  {
    const t1 = mutateEngine(er.e, pr.p, { op: "tick", participantKey: "", payload: {}, now: t++, config: cfg5 });
    assert.ok(!t1.error);
    er.e = t1.engine;
...
test("mid-orbit join: new seat gets waitBb and is out until next hand", () => {
  const er = { e: buildFreshEngine(cfg9) };
  const pr = { p: normalizePrivatePayload({}) };
  let t = 2_020_000;
  for (let i = 0; i < 2; i++) {
    assert.ok(!apply(er, pr, { op: "sit", participantKey: `p${i}`, payload: { seatIndex: i, buyIn: 500, displayName: `P${i}` }, now: t++ }).error);
  }
  apply(er, pr, { op: "tick", participantKey: "", payload: {}, now: t++ });
  apply(er, pr, { op: "tick", participantKey: "", payload: {}, now: t++ });
  let e = er.e;
  assert.equal(orbitEstablished(e), true);
  assert.ok(!apply(er, pr, { op: "sit", participantKey: "p2", payload: { seatIndex: 2, buyIn: 500, displayName: "P2" }, now: t++ }).error);
  e = er.e;
  assert.equal(e.seats[2].waitBb, true);
  assert.equal(e.seats[2].inCurrentHand, false);
});

test("waitBb: every dealt seat clears waitBb when BB waiter is SB in shorthanded lineup (no rotate-miss next hand)", () => {
  const e = buildFreshEngine(cfg5);
  const priv = normalizePrivatePayload({});
  const t = 2_030_000;
  for (let i = 0; i < 5; i++) {
    e.seats[i].participantKey = `p${i}`;
  ...
  e.pot = 0;
  e.actionSeat = null;
  const r = mutateEngine(e, priv, { op: "tick", participantKey: "", payload: {}, now: t + 1, config: cfg5 });
  assert.equal(r.error, null);
  const e2 = r.engine;
  assert.equal(e2.phase, "preflop");
  for (const i of [0, 2, 4]) {
    assert.equal(e2.seats[i].inCurrentHand, true, `seat ${i} in hand`);
    assert.equal(e2.seats[i].waitBb, false, `seat ${i} waitBb cleared`);
  }
  assert.equal(e2.seats[1].inCurrentHand, false);
  assert.equal(e2.seats[1].waitBb, true);
  assert.equal(e2.seats[3].inCurrentHand, false);
  assert.equal(e2.seats[3].waitBb, true);
  assert.equal(validateCcEngineInvariants(e2), null);
});
