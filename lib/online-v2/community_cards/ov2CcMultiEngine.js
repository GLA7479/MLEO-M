...
/**
 * When phase is idle/between_hands, repair persisted inconsistencies and waitBb deadlocks
 * so reused tables can start hands again. Does not run during live betting/showdown.
 */
function repairStaleNonBettingTable(engine, privatePayload) {
...
function runTick(engine, privatePayload, now, economyOps) {
  if (engine.phase === "idle" || engine.phase === "between_hands") {
    repairStaleNonBettingTable(engine, privatePayload);
  }
...
export function mutateEngine(engine, privatePayload, ctx) {
  ...
  if (e.phase === "idle" || e.phase === "between_hands") {
    repairStaleNonBettingTable(e, priv);
  }
