...
import { bestHandScoreFrom7 } from "./ov2CcPokerEval";
...
function tryStartNewHand(engine, privatePayload, now) {
  ...
  const base = occupiedSeatIndexes(engine).filter(i => {
...
function advanceStreet(engine, privatePayload) {
  resetStreet(engine);
  ...
  if (live.length <= 1) {
    ...
    engine.showdownRevealIdx = 0;
    engine.lastShowdownAdvanceAt = Date.now();
    return;
  }
  ...
  engine.actionDeadline = Date.now() + OV2_CC_ACTION_MS;
}
 ...
 */
function settleHand(engine, privatePayload, economyOps) {
  ...
  engine.phaseEndsAt = Date.now() + OV2_CC_BETWEEN_HANDS_MS;
  engine.actionSeat = null;
  engine.actionDeadline = null;
...
function applyAutoFoldOrCheck(engine, privatePayload, seatIdx) {
  ...
  advanceAfterAction(engine, privatePayload);
}

function advanceAfterAction(engine, privatePayload) {
  if (everyoneMatchedOrFolded(engine)) {
    ...
    advanceStreet(engine, privatePayload);
    return;
  }
...
function runTick(engine, privatePayload, now, economyOps) {
  ...
  if (engine.phase === "showdown") {
    ...
    if (total === 0) {
      return settleHand(engine, privatePayload, economyOps);
    }
  ...
  if (engine.actionDeadline && now > engine.actionDeadline && engine.actionSeat != null) {
    applyAutoFoldOrCheck(engine, privatePayload, engine.actionSeat);
  }
  return null;
}
...
function applyPlayerAction(engine, privatePayload, seatIdx, kind, payload) {
  const s = engine.seats[seatIdx];
...
export function mutateEngine(engine, privatePayload, ctx) {
  ...
  if (op === "leave_seat" || op === "leave_table") {
    ...
    const s = seatByPk(e, pk);
    if (!s) return { engine: e, privatePayload: priv, economyOps, error: "not_seated" };
    const activeBetting = ["preflop", "flop", "turn", "river"].includes(e.phase);
    const inHand = s.inCurrentHand && activeBetting;
    if (inHand) {
      s.pendingLeaveAfterHand = true;
      if (e.actionSeat === s.seatIndex) {
        applyAutoFoldOrCheck(e, priv, s.seatIndex);
      } else {
        s.folded = true;
      }
      return { engine: e, privatePayload: priv, economyOps, error: null };
  ...
  if (["fold", "check", "call", "bet", "raise", "all_in"].includes(op)) {
    ...
    const r = applyPlayerAction(e, priv, s.seatIndex, op === "all_in" ? "all_in" : op, payload);
