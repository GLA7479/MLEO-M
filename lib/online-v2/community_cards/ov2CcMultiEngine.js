...
export function distributeSidePotsToWinners(engine, privatePayload) {
  ...
  for (const pot of pots) {
    ...
    for (const i of activeElig) {
      const fromHoles = privatePayload.holes[String(i)];
      const fromRev = privatePayload.revealed[String(i)];
      const hole =
        Array.isArray(fromHoles) && fromHoles.length >= 2
          ? fromHoles
          : Array.isArray(fromRev) && fromRev.length >= 2
            ? fromRev
            : Array.isArray(fromHoles)
              ? fromHoles
              : Array.isArray(fromRev)
                ? fromRev
                : [];
