...
function ov2LeaveTrace(event, payload = {}) {
  if (typeof console === "undefined" || typeof console.info !== "function") return;
  try {
    console.info("[OV2][leave-trace]", event, payload);
  } catch {
...
export async function leaveOv2RoomUnified(params) {
  ...
  if (
    roomRow != null &&
    typeof roomRow === "object" &&
    (roomRow.shared_schema_version == null || roomRow.shared_schema_version === "") &&
    String(roomRow.product_game_id || "").startsWith("ov2_")
  ) {
    roomRow = { ...roomRow, shared_schema_version: 1 };
  }
  const shared = roomRow != null && typeof roomRow === "object" && Number(roomRow.shared_schema_version) === 1;
  ov2LeaveTrace("leave_unified_select_path", {
    room_id,
    participant_key,
    shared,
    shared_schema_version:
      roomRow && typeof roomRow === "object" ? Number(roomRow.shared_schema_version) || null : null,
    status: roomRow && typeof roomRow === "object" ? String(roomRow.status || "") : "",
    forfeit_game: Boolean(forfeit_game),
    rpc: shared ? "ov2_shared_leave_room" : "ov2_leave_room",
  });
  if (shared) {
    return leaveOv2SharedRoom({ room_id, participant_key, forfeit_game });
  }
  const legacy = await leaveOv2Room({ room_id, participant_key, forfeit_game });
  return { closed: legacy.closed, stake_refund_vault_apply_error: undefined };
}
...
export async function leaveOv2RoomWithForfeitRetry(params) {
  ...
  ov2LeaveTrace("leave_with_retry_begin", {
    room_id,
    participant_key,
    status: room && typeof room === "object" ? String(room.status || "") : "",
    lifecycle_phase: room && typeof room === "object" ? String(room.lifecycle_phase || "") : "",
    shared_schema_version: room && typeof room === "object" ? Number(room.shared_schema_version) || null : null,
  });
  if (room_id) {
    try {
  ...
  ov2LeaveTrace("leave_with_retry_first_attempt", {
    room_id,
    participant_key,
    forfeit_game: Boolean(forfeit),
    status: room && typeof room === "object" ? String(room.status || "") : "",
    lifecycle_phase: room && typeof room === "object" ? String(room.lifecycle_phase || "") : "",
    shared_schema_version: room && typeof room === "object" ? Number(room.shared_schema_version) || null : null,
  });
  /** @type {{ stake_refund_vault_apply_error?: string } | null} */
  let unifiedOut = null;
  try {
    unifiedOut = await leaveOv2RoomUnified({ room, room_id, participant_key, forfeit_game: forfeit });
    ov2LeaveTrace("leave_with_retry_first_response", {
      room_id,
      participant_key,
      ok: true,
      must_forfeit_retry: false,
    });
  } catch (e) {
    const errCode = e && typeof e === "object" && "code" in e ? String(e.code || "") : "";
    const errMessage = e && typeof e === "object" && "message" in e ? String(e.message || "") : String(e || "");
    const canRetryForfeit = !forfeit && leaveRpcErrorSuggestsForfeit(e);
    ov2LeaveTrace("leave_with_retry_first_response", {
      room_id,
      participant_key,
      ok: false,
      code: errCode,
      message: errMessage,
      must_forfeit_retry: canRetryForfeit,
    });
    if (!forfeit && leaveRpcErrorSuggestsForfeit(e)) {
      ...
      ov2LeaveTrace("leave_with_retry_second_attempt", {
        room_id,
        participant_key,
        forfeit_game: true,
        status: room && typeof room === "object" ? String(room.status || "") : "",
      ...
      unifiedOut = await leaveOv2RoomUnified({ room, room_id, participant_key, forfeit_game: true });
      ov2LeaveTrace("leave_with_retry_second_response", {
        room_id,
        participant_key,
        ok: true,
      });
    } else {
      ov2LeaveTrace("leave_with_retry_abort", {
        room_id,
        participant_key,
        reason: "first_attempt_failed_no_retry",
      });
      throw e;
    }
  }
  const stakeRefundVaultApplyError =
    unifiedOut &&
    typeof unifiedOut === "object" &&
    unifiedOut.stake_refund_vault_apply_error != null &&
    String(unifiedOut.stake_refund_vault_apply_error).trim() !== ""
      ? String(unifiedOut.stake_refund_vault_apply_error).trim()
      : undefined;
  if (stakeRefundVaultApplyError) {
    console.warn("[OV2] stake refund vault sync failed after leave:", stakeRefundVaultApplyError);
  }
  ov2LeaveTrace("leave_with_retry_done", {
    room_id,
    participant_key,
    ok: true,
    stake_refund_vault_apply_error: stakeRefundVaultApplyError || null,
  });
  return { stakeRefundVaultApplyError };
}
