...
/**
 * Leave with one retry if the server returns MUST_FORFEIT (e.g. room fields stale).
...
export async function leaveOv2RoomWithForfeitRetry(params) {
  ...
  if (room_id && (!room || typeof room !== "object" || room.shared_schema_version == null)) {
    try {
      const fetched = await fetchOv2RoomById(room_id);
      if (fetched) room = fetched;
    } catch {
