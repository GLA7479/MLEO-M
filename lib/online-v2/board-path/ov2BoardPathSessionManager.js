...
export function ensureBoardStatePositionsForSeats(session, seats) {
  ...
  const pathLength = session.boardState?.pathLength ?? 30;
  const positions =
...
function normalizeTurnMetaFromDb(o) {
  ...
  const ai = t.activeSeatIndex;
  const activeSeatIndex =
    ai == null || ai === "" || Number.isNaN(Number(ai)) ? null : Number(ai);
  const startedAt = typeof t.startedAt === "number" ? t.startedAt : Number(t.startedAt) || Date.now();
  return { turnNumber: tn, activeSeatIndex, startedAt };
}

function normalizeBoardStateFromDb(raw) {
  const o = raw && typeof raw === "object" ? raw : {};
  const pl = typeof o.pathLength === "number" ? o.pathLength : Number(o.pathLength) || 30;
 ...
 */
export function boardPathDbSeatRowsToLocalSeats(members, hostKey, sessionId, selfKey, rows) {
...
function finalizeBundle(localSession, localSeats, selfKey, openMeta) {
  ...
  return {
    localSession: sess,
    localSeats,
    adapterSession: localBoardPathSessionToAdapterSession(sess, localSeats, selfKey),
    adapterSeats: localBoardPathSeatsToAdapterSeats(localSeats),
    openMeta,
  };
}
...
export function boardPathBundleFromDatabase(room, members, selfKey, sessionRow, seatRows, openedByParticipantKey) {
  ...
  const localSeats = boardPathDbSeatRowsToLocalSeats(members, hk, localSession.id, selfKey, seatRows);
