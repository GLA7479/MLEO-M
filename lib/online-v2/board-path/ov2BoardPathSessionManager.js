 ...
 * @property {"pending"|"finalized"} [settlementStatus]
 * @property {string|null} [finalizedAt]
 * @property {string|null} [finalizedByParticipantKey]
...
export function boardPathDbSessionRowToLocalRecord(row, openedByParticipantKey) {
  ...
  return {
    ...
    eventLog,
    settlementStatus: (() => {
      const v = row.settlement_status ?? row.settlementStatus;
      return String(v || "").toLowerCase() === "finalized" ? "finalized" : "pending";
    })(),
    finalizedAt: (() => {
      if (row.finalized_at instanceof Date) return row.finalized_at.toISOString();
      if (typeof row.finalized_at === "string") return row.finalized_at;
      const x = row.finalizedAt;
      if (x instanceof Date) return x.toISOString();
      if (typeof x === "string") return x;
      return null;
    })(),
    finalizedByParticipantKey: (() => {
      const v = row.finalized_by_participant_key ?? row.finalizedByParticipantKey;
      return v != null && String(v).trim() ? String(v).trim() : null;
    })(),
    settlementRevision: Math.floor(
      Number(row.settlement_revision ?? row.settlementRevision) || 0
    ),
  };
}
...
export function localBoardPathSessionToAdapterSession(local, seats, selfParticipantKey) {
  ...
  return {
    ...
    settlement_status: local.settlementStatus ?? "pending",
    settlementStatus: local.settlementStatus ?? "pending",
    finalized_at: local.finalizedAt ?? null,
    finalizedAt: local.finalizedAt ?? null,
    finalized_by_participant_key: local.finalizedByParticipantKey ?? null,
    finalizedByParticipantKey: local.finalizedByParticipantKey ?? null,
    settlement_revision: local.settlementRevision ?? 0,
    settlementRevision: local.settlementRevision ?? 0,
  };
}
