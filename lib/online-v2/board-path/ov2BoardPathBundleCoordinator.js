...
/**
 * Session-open follow-up (intended wiring — not implemented in this module):
 *
 * When `fetchBoardPathSessionDetailed` returns `{ ok: false, code: "NO_ACTIVE_SESSION_ID" }` while the
 * room is already in an active match lifecycle and the client is host-eligible, a **separate** app
 * effect may call `rpcOv2BoardPathOpenSession` once, then re-enter
 * `fetchBoardPathLiveCoordinatedBundle`. Do not add auto-retry loops or fake session rows here.
 */
