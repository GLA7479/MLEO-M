 ...
 * @param {string} roomId
 * @returns {Promise<
 *   | { ok: true, session: Record<string, unknown>, seats: Record<string, unknown>[], activeSessionId: string, roomMatchSeq: number, settlementLines: Record<string, unknown>[] }
...
export async function fetchBoardPathSessionDetailed(supabase, roomId) {
  ...
  const { data: settleRows, error: settleErr } = await supabase
    .from("ov2_settlement_lines")
    .select("*")
    .eq("game_session_id", sessionId);

  const settlementLines =
    !settleErr && Array.isArray(settleRows)
      ? settleRows.map(r => /** @type {Record<string, unknown>} */ (r))
      : [];
