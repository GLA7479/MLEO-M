...
import { ONLINE_V2_GAME_IDS } from "../onlineV2GameRegistry";
import { isOv2QuickMatchAllowedStakeUnits } from "../shared-rooms/ov2QuickMatchStakes";

/**
 * QM RPC allowlists follow shared-room product ids. Map legacy lobby id to the shared Snakes product.
 * @param {string} productGameId
 */
function normalizeProductGameIdForQuickMatch(productGameId) {
  const id = String(productGameId || "").trim();
  if (id === ONLINE_V2_GAME_IDS.SNAKES_LADDERS) return ONLINE_V2_GAME_IDS.SNAKES_AND_LADDERS;
  return id;
}
...
export async function ov2QuickMatchEnqueue(params) {
  ...
  const { data, error } = await supabase.rpc("ov2_quick_match_enqueue", {
    ...
    p_product_game_id: normalizeProductGameIdForQuickMatch(String(product_game_id || "").trim()),
