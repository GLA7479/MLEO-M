...
import { creditOnlineV2VaultForPreGameRefund } from "../onlineV2VaultBridge";
...
function translateSupabaseRpcError(error) {
  ...
  if (missingDbObject) {
    let detail =
      "This action is not available on the server (missing database function or table). Check Supabase logs and apply pending OV2 migrations.";
    if (/ov2_shared_join_room_by_code/i.test(msg)) {
      detail =
        "Hidden room join-by-code requires function public.ov2_shared_join_room_by_code. Apply migration migrations/online-v2/058_ov2_shared_hidden_join_by_code.sql (after 046–057).";
    } else if (/ov2_shared_join_room\b/i.test(msg) && /does not exist|schema cache|could not find/i.test(msg)) {
      detail =
        "Shared room join requires an updated public.ov2_shared_join_room (hidden rooms / password). Apply migration migrations/online-v2/058_ov2_shared_hidden_join_by_code.sql (after 046–057).";
    }
    return new Ov2SharedRoomRpcError("MIGRATION_REQUIRED", detail);
  }
  return error;
}
 ...
 */
async function applyPreGameStakeRefundFromRpc(out) {
  const sr = out.stake_refund;
  if (sr == null || typeof sr !== "object" || Array.isArray(sr)) {
    return { ok: true, skipped: true };
  }
  const o = /** @type {Record<string, unknown>} */ (sr);
  const amount = typeof o.amount === "number" ? o.amount : Number(o.amount);
  const gid = typeof o.product_game_id === "string" && o.product_game_id.trim() !== "" ? o.product_game_id.trim() : null;
  const idem = typeof o.idempotency_key === "string" && o.idempotency_key.trim() !== "" ? o.idempotency_key.trim() : null;
  if (!(amount > 0) || !gid || !idem) {
    return { ok: true, skipped: true };
  }
  const result = await creditOnlineV2VaultForPreGameRefund(Math.floor(amount), gid, idem);
  if (!result?.ok) {
    return { ok: false, error: String(result?.error || "Vault credit failed") };
  }
  return { ok: true };
}

/**
 * Apply `stake_refund` from any shared / QM RPC payload (idempotent local credit).
 * @param {unknown} rpcPayload
 */
export async function applyOv2SharedStakeRefundHintFromRpc(rpcPayload) {
  const out = rpcPayload && typeof rpcPayload === "object" ? /** @type {Record<string, unknown>} */ (rpcPayload) : {};
  return applyPreGameStakeRefundFromRpc(out);
}
...
export async function leaveOv2Room(params) {
  ...
  if (error) throw translateSupabaseRpcError(error);
  const out = assertSharedRpcSuccess(data);
  const refundApply = await applyPreGameStakeRefundFromRpc(out);
  const snap = normalizeOv2SharedRoomSnapshot(out);
  return {
    ...snap,
    closed: Boolean(out.closed),
    stake_refund_vault_apply_error: refundApply.ok ? undefined : refundApply.error,
  };
}
