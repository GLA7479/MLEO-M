  ...
  if (missingDbObject) {
    const detail =
      msg && String(msg).trim().length > 0
        ? String(msg).trim()
        : "Server rejected the request (missing database function or object). Check the Supabase error and project migrations.";
...
export async function leaveOv2Room(params) {
  ...
  try {
    console.info("[OV2][leave-trace]", "shared_leave_rpc_request", {
      room_id,
      participant_key,
      forfeit_game: Boolean(forfeit_game),
      rpc: "ov2_shared_leave_room",
    });
  } catch {
    // ignore
  }
  ...
  if (error) {
    try {
      console.info("[OV2][leave-trace]", "shared_leave_rpc_error", {
        room_id,
        participant_key,
        forfeit_game: Boolean(forfeit_game),
  ...
  const out = assertSharedRpcSuccess(data);
  try {
    console.info("[OV2][leave-trace]", "shared_leave_rpc_response", {
      room_id,
      participant_key,
      forfeit_game: Boolean(forfeit_game),
      ok: true,
