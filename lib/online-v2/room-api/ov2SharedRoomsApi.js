...
function translateSupabaseRpcError(error) {
  ...
  if (/relation .* does not exist|function .* does not exist/i.test(msg)) {
    return new Ov2SharedRoomRpcError(
      "MIGRATION_REQUIRED",
      "This action is not available on the server (missing database function or table). Check Supabase logs or apply pending OV2 migrations."
    );
  }
  return error;
}
