...
export function normalizeOv2CheckersSnapshot(raw) {
  ...
  let jumpChainAt =
    jca !== null && jca !== undefined && jca !== "null" && Number.isFinite(Number(jca))
      ...
      : null;
  if (jumpChainAt == null && raw.board && typeof raw.board === "object") {
    const jc = /** @type {Record<string, unknown>} */ (raw.board).jumpChain;
    if (jc && typeof jc === "object" && jc.at != null && Number.isFinite(Number(jc.at))) {
      const n = Math.floor(Number(jc.at));
      if (n >= 0 && n <= 63) jumpChainAt = n;
    }
  }
  const missedTurns =
 ...
 */
export async function requestOv2CheckersOfferDouble(roomId, participantKey, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  const rev = _opts?.revision;
  try {
    const { data, error } = await supabase.rpc("ov2_checkers_offer_double", {
      p_room_id: roomId,
      p_participant_key: pk,
      p_expected_revision: rev != null && rev !== "" ? Number(rev) : null,
    });
...
export async function requestOv2CheckersRespondDouble(roomId, participantKey, accept, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  const rev = _opts?.revision;
  try {
    const { data, error } = await supabase.rpc("ov2_checkers_respond_double", {
      p_room_id: roomId,
      p_participant_key: pk,
