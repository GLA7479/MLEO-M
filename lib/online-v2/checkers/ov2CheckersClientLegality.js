/**
 * Client-side legality hints for highlights (mirrors server rules; server is authoritative).
...
/**
 * Legal destination indices for a selected `fromIdx` (viewer board indices = server indices).
 * @param {number[]} cells
 * @param {number} turn
...
export function ov2CheckersLegalTosForFrom(cells, turn, chainAt, fromIdx) {
  ...
  const glob = ov2CheckersSideHasCapture(cells, turn);
...
/**
 * Map server linear index to viewer top-left index (seat 1 sees board flipped).
