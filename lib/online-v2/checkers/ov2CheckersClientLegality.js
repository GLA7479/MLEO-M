 ...
 * - Men: forward slide; opening man capture forward only; in jumpChain the same man may capture forward or backward.
 * - Kings: may capture (and continue chains) in any diagonal direction; flying; not globally forced to capture.
 * - Forced capture: any **man** has a legal **opening** (forward-only) capture; kings do not trigger forced capture.
 */
...
export function ov2CheckersSideHasMenCapture(cells, turn) {
  for (let i = 0; i < 64; i += 1) {
    ...
    if (manHasCapture(cells, i, turn, true)) return true;
  }
  return false;
}
...
export function ov2CheckersForcedMenCaptureFromIndices(cells, turn) {
  ...
  for (let i = 0; i < 64; i += 1) {
    if (owner(cells[i]) !== turn) continue;
    if (isKing(cells[i])) continue;
    if (manHasCapture(cells, i, turn, true)) out.push(i);
  }
  return out;
}

function cellHasCapture(cells, fromIdx, turn, manForwardOnly = true) {
  const r = Math.floor(fromIdx / 8);
  const c = fromIdx % 8;
  const p = cells[fromIdx];
  if (isKing(p)) return kingHasCapture(cells, r, c, turn);
  return manHasCapture(cells, fromIdx, turn, manForwardOnly);
}

/**
 * @param {boolean} [forwardOnly] opening / forced-capture scan: true = forward diagonals only; false = all four (chain continuation).
 */
function manHasCapture(cells, fromIdx, turn, forwardOnly = true) {
  const r = Math.floor(fromIdx / 8);
  const c = fromIdx % 8;
  ...
  for (let to = 0; to < 64; to += 1) {
    ...
    if (inChainHere) {
      const ok = isKing(p)
        ...
        : tryManCapture(cells, fr, fc, tr, tc, turn, true);
      if (ok) out.push(to);
      continue;
    }
    ...
    if (glob) {
      const ok = isKing(p)
        ? tryKingCapture(cells, fr, fc, tr, tc, turn)
        : tryManCapture(cells, fr, fc, tr, tc, turn, false);
      if (ok) out.push(to);
      continue;
    }
...
/**
 * @param {boolean} [chainContinuation] true = same man in jumpChain (forward or backward); false = opening / non-chain (forward only).
 */
