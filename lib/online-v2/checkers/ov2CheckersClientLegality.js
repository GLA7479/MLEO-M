 ...
 * - Men capture forward only, except backward allowed while continuing the same jump chain (after a forward jump).
 * - Forced capture applies when any **man** can capture; kings are never under forced capture (slides remain
 *   when only kings could capture).
 */
...
export function ov2CheckersSideHasCapture(cells, turn) {
  for (let i = 0; i < 64; i += 1) {
    ...
    if (cellHasCapture(cells, i, turn, null)) return true;
  }
  return false;
}
...
export function ov2CheckersSideHasMenCapture(cells, turn) {
  for (let i = 0; i < 64; i += 1) {
    ...
    if (manHasCapture(cells, i, turn, false)) return true;
  }
  return false;
}
...
export function ov2CheckersForcedMenCaptureFromIndices(cells, turn) {
  ...
  for (let i = 0; i < 64; i += 1) {
    if (owner(cells[i]) !== turn) continue;
    if (isKing(cells[i])) continue;
    if (manHasCapture(cells, i, turn, false)) out.push(i);
  }
  return out;
}

/**
 * @param {number|null} chainAt — when equal to fromIdx, man may capture backward (chain continuation).
 */
function cellHasCapture(cells, fromIdx, turn, chainAt) {
  const r = Math.floor(fromIdx / 8);
  const c = fromIdx % 8;
  const p = cells[fromIdx];
  const inChain = chainAt != null && chainAt === fromIdx;
  if (isKing(p)) return kingHasCapture(cells, r, c, turn);
  return manHasCapture(cells, fromIdx, turn, inChain);
}

function manHasCapture(cells, fromIdx, turn, allowBackward) {
  const r = Math.floor(fromIdx / 8);
  const c = fromIdx % 8;
  ...
  for (const [dr, dc] of dirs) {
    if (!allowBackward && dr !== fwd) continue;
    const rm = r + dr;
    const cm = c + dc;
    const r2 = r + 2 * dr;
    const c2 = c + 2 * dc;
...
export function ov2CheckersLegalTosForFrom(cells, turn, chainAt, fromIdx) {
  ...
  const fc = fromIdx % 8;
  const p = cells[fromIdx];
  const manChainCont = chainAt != null && chainAt === fromIdx && !isKing(p);
  for (let to = 0; to < 64; to += 1) {
    ...
    if (glob) {
      const ok = isKing(p)
        ...
        : tryManCapture(cells, fr, fc, tr, tc, turn, manChainCont);
      if (ok) out.push(to);
    } else {
      const ok = isKing(p) ? trySlideKing(cells, fr, fc, tr, tc) : trySlideMan(cells, fr, fc, tr, tc, turn);
      if (ok) out.push(to);
    }
  }
  return out;
...
function tryManCapture(cells, fr, fc, tr, tc, turn, allowBackward) {
  const opp = turn === 0 ? 1 : 0;
  if (tr < 0 || tc < 0 || tr > 7 || tc > 7) return false;
  if (!isDark(tr, tc) || cells[idx(tr, tc)] !== 0) return false;
  const dr = tr - fr;
