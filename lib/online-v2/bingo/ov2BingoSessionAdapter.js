...
function computeCapabilityFlags(viewerPk, snap) {
  ...
  const seatedMember = pk ? snap.members.find(m => m.participantKey === pk) : null;
  const mySeat = seatedMember?.seatIndex ?? null;
  const inActive =
    mySeat != null &&
    snap.activeSeats.length > 0 &&
    snap.activeSeats.some(s => Number(s) === mySeat);
  const committed = seatedMember?.walletState === "committed";
  ...
  const canOpenSession =
    ...
    (snap.roomActiveSessionId == null || String(snap.roomActiveSessionId).trim() === "");
  ...
  const card = resolveOv2BingoSeatCard(snap.deckCardsBySeat, snap.seed, snap.roundId, mySeat);
  const existingClaims = snap.claims.map(c => ({ prize_key: c.prizeKey, amount: c.amount }));
  let canClaimAnyPrize = false;
  if (playing && inActive && card && snap.calledNumbers.length >= 0) {
    const keys = getAvailablePrizeKeys({
      card,
      called: snap.calledNumbers,
      existingClaims,
    });
    canClaimAnyPrize = keys.length > 0;
  }

  const canRequestRematch =
    finished && committed && inActive && sess && !seatedMember?.rematchRequested;
  const canCancelRematch = finished && committed && inActive && sess && Boolean(seatedMember?.rematchRequested);

  const seatedCommitted = snap.members.filter(m => m.seatIndex != null && m.walletState === "committed");
  const allRematchReady =
    seatedCommitted.length > 0 && seatedCommitted.every(m => m.rematchRequested);
  const canStartNextMatch =
    isHost && activeRoom && finished && sess && allSeatedCommitted(snap.members) && allRematchReady;
...
export function normalizeOv2BingoAuthoritativeSnapshot(rawSnapshot, opts) {
  ...
  const claims = claimsRaw.map(c => {
    ...
    return {
      ...
      claimedByParticipantKey: String(r.claimed_by_participant_key ?? ""),
      claimedByName: String(r.claimed_by_name ?? ""),
      seatIndex: num(r.seat_index, 0),
  ...
  let availablePrizeKeys = [];
  const myMember = viewerPk ? members.find(m => m.participantKey === viewerPk) : null;
  const mySeat = myMember?.seatIndex ?? null;
  if (seed && roundId != null && mySeat != null && sessionPhase === "playing") {
    const card = resolveOv2BingoSeatCard(deckCardsBySeat, seed, roundId, mySeat);
    if (card) {
      availablePrizeKeys = getAvailablePrizeKeys({
        card,
        called: calledNumbers,
        existingClaims: claims.map(c => ({ prize_key: c.prizeKey, amount: c.amount })),
      });
    }
  }
  ...
  const caps = computeCapabilityFlags(viewerPk, snap);
  return { ...snap, ...caps };
}
...
export async function openOv2BingoSession(roomId, hostParticipantKey) {
  ...
  try {
    ...
    if (parsed.ok) {
      return { ok: true, snapshot: parsed.snapshot, idempotent: /** @type {any} */ (data)?.idempotent === true };
    }
