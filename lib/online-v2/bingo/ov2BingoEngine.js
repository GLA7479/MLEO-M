...
/**
 * @param {readonly unknown[]|null|undefined} existingClaims
 * @returns {number}
 */
function sumClaimAmounts(existingClaims) {
  if (!existingClaims || !Array.isArray(existingClaims)) return 0;
  let s = 0;
  for (const row of existingClaims) {
    if (row == null || typeof row !== "object") continue;
    const a = Number(/** @type {Record<string, unknown>} */ (row).amount);
...
/**
 * Remainder of pot after paid claims; `full` ends the match and must not overpay the pot.
 * @param {{ potTotal: number, existingClaims: unknown }} p
 */
export function computeFullPrizeAmount({ potTotal, existingClaims }) {
  const p = Number(potTotal);
  if (!Number.isFinite(p) || p <= 0) return 0;
  return Math.max(0, p - sumClaimAmounts(existingClaims));
}
