...
test("prize amounts: row 10%, full remainder", () => {
  assert.equal(computeRowPrizeAmount(100), 10);
  assert.equal(computeFullPrizeAmount({ potTotal: 100, existingClaims: [{ prize_key: "row1", amount: 10 }] }), 90);
  assert.equal(computeFullPrizeAmount({ potTotal: 100, existingClaims: [{ prize_key: "row1", amount: 200 }] }), 0);
});
