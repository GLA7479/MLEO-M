 ...
 * @returns {number|null}
 */
export function ccCardNum(card) {
...
/**
 * Backend may return pending draw as a single card (legacy) or an array (V2 clash draws).
 * @param {unknown} list
 * @param {unknown} card
 * @returns {boolean}
 */
export function ccCardInPendingDrawList(list, card) {
  if (card == null || typeof card !== "object") return false;
  if (Array.isArray(list)) {
    return list.some(c => ccCardsEqual(c, card));
  }
  if (list && typeof list === "object" && !Array.isArray(list)) {
    return ccCardsEqual(list, card);
  }
  return false;
}
