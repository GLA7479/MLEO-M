/**
 * OV2 Chess — RPC + Realtime boundary (no React).
 */

import { supabaseMP as supabase } from "../../supabaseClients";

export const OV2_CHESS_PRODUCT_GAME_ID = "ov2_chess";

export function isOv2ChessBackendUnavailableError(err) {
 ...
 */
export function normalizeOv2ChessSnapshot(raw) {
...
function parseSnapshotRpc(data) {
  ...
  if (d.ok === true && d.snapshot) {
    const snap = normalizeOv2ChessSnapshot(/** @type {Record<string, unknown>} */ (d.snapshot));
 ...
 */
export async function fetchOv2ChessSnapshot(roomId, _opts) {
  if (!roomId) return null;
  const pk = _opts?.participantKey != null ? String(_opts.participantKey).trim() : "";
  try {
    const { data, error } = await supabase.rpc("ov2_chess_get_snapshot", {
      p_room_id: roomId,
      p_participant_key: pk,
    });
    if (error) {
      if (isOv2ChessBackendUnavailableError(error)) return null;
      return null;
    }
    const body = /** @type {Record<string, unknown>} */ (data);
    if (body?.ok !== true || !body.snapshot) return null;
    return normalizeOv2ChessSnapshot(/** @type {Record<string, unknown>} */ (body.snapshot));
  } catch (e) {
    if (isOv2ChessBackendUnavailableError(e)) return null;
    return null;
  }
}
...
/**
 * @param {string} roomId
 * @param {{ participantKey?: string|null, onSnapshot: (s: NonNullable<ReturnType<typeof normalizeOv2ChessSnapshot>>) => void, onError?: (e: Error) => void }} handlers
 */
export function subscribeOv2ChessSnapshot(roomId, handlers) {
  ...
  const pushLatest = async () => {
    if (cancelled) return;
    const snap = await fetchOv2ChessSnapshot(roomId, { participantKey: pk });
    if (snap && !cancelled) handlers.onSnapshot(snap);
  };
  try {
    const channel = supabase
      .channel(`ov2-chess:${roomId}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "ov2_chess_sessions", filter: `room_id=eq.${roomId}` },
        () => {
          void pushLatest();
        }
      )
 ...
 */
export async function requestOv2ChessOpenSession(roomId, participantKey, _opts) {
  ...
  try {
    const { data, error } = await supabase.rpc("ov2_chess_open_session", {
      p_room_id: roomId,
      p_participant_key: pk,
      p_presence_leader_key: leader,
    });
    if (error) {
      if (isOv2ChessBackendUnavailableError(error)) {
        return { ok: false, error: "Chess backend not available (migrations not applied?)" };
      }
...
export async function requestOv2ChessOfferDouble(roomId, participantKey, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  const rev = _opts?.revision;
  try {
    const { data, error } = await supabase.rpc("ov2_chess_offer_double", {
      p_room_id: roomId,
      p_participant_key: pk,
      p_expected_revision: rev != null && rev !== "" ? Number(rev) : null,
    });
    if (error) {
      if (isOv2ChessBackendUnavailableError(error)) return { ok: false, error: "Chess backend not available" };
      return { ok: false, error: error.message || String(error) };
    }
    return parseSnapshotRpc(data);
  } catch (e) {
...
export async function requestOv2ChessRespondDouble(roomId, participantKey, accept, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  const rev = _opts?.revision;
  try {
    const { data, error } = await supabase.rpc("ov2_chess_respond_double", {
      p_room_id: roomId,
      p_participant_key: pk,
...
export async function requestOv2ChessMarkTurnTimeout(roomId, participantKey, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  const rev = _opts?.revision;
  try {
    const { data, error } = await supabase.rpc("ov2_chess_mark_turn_timeout", {
      p_room_id: roomId,
      p_participant_key: pk,
      p_expected_revision: rev != null && rev !== "" ? Number(rev) : null,
    });
 ...
 * @param {number} toIdx
 * @param {{ revision?: number|string, promo?: string }} [_opts]
 */
/**
 * Authoritative legal destination squares for the selected `from` index (server 0..63).
 * @param {string} roomId
 * @param {string} participantKey
 * @param {number} fromIdx
 * @param {{ revision?: number|string }} [_opts]
 */
export async function requestOv2ChessLegalTos(roomId, participantKey, fromIdx, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant", tos: [] };
  const rev = _opts?.revision;
  try {
    const { data, error } = await supabase.rpc("ov2_chess_legal_tos", {
      p_room_id: roomId,
      p_participant_key: pk,
    ...
    if (error) {
      if (isOv2ChessBackendUnavailableError(error)) return { ok: false, error: "Chess backend not available", tos: [] };
      return { ok: false, error: error.message || String(error), tos: [] };
    }
    const d = /** @type {Record<string, unknown>} */ (data);
    if (d?.ok !== true) {
      return {
    ...
    if (Array.isArray(raw)) {
      for (const x of raw) {
        ...
        if (Number.isInteger(n) && n >= 0 && n <= 63) tos.push(n);
      }
    }
    return { ok: true, tos };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : String(e), tos: [] };
  }
}

export async function requestOv2ChessApplyMove(roomId, participantKey, fromIdx, toIdx, _opts) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  const rev = _opts?.revision;
  const promo = _opts?.promo != null ? String(_opts.promo).trim().slice(0, 1) : "Q";
  try {
    const { data, error } = await supabase.rpc("ov2_chess_apply_move", {
      p_room_id: roomId,
      p_participant_key: pk,
      p_from: Math.floor(Number(fromIdx)),
...
export async function requestOv2ChessRequestRematch(roomId, participantKey) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  try {
    const { data, error } = await supabase.rpc("ov2_chess_request_rematch", {
      p_room_id: roomId,
      p_participant_key: pk,
    });
...
export async function requestOv2ChessCancelRematch(roomId, participantKey) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  try {
    const { data, error } = await supabase.rpc("ov2_chess_cancel_rematch", {
      p_room_id: roomId,
      p_participant_key: pk,
    });
    if (error) return { ok: false, error: error.message || String(error) };
...
export async function requestOv2ChessStartNextMatch(roomId, participantKey, expectedMatchSeq) {
  const pk = participantKey != null ? String(participantKey).trim() : "";
  if (!roomId || !pk) return { ok: false, error: "Missing room or participant" };
  try {
    const { data, error } = await supabase.rpc("ov2_chess_start_next_match", {
      p_room_id: roomId,
      p_participant_key: pk,
