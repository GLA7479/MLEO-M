...
async function apiFetch(path, options = {}, retried = false) {
  ...
  // Retry once on 403 after refreshing CSRF:
  // - GET/HEAD/OPTIONS: same as before (any 403 — stale CSRF is the common case).
  // - POST/PATCH/...: only when the body clearly indicates CSRF failure (safe: handler rejects before work).
  if (response.status === 403 && !retried) {
    const payload403 = await response.json().catch(() => ({}));
    const msg = typeof payload403?.message === "string" ? payload403.message : "";
    const isCsrfFailure =
      payload403?.code === "CSRF_INVALID" || msg.includes("Invalid CSRF");

    if (isIdempotentMethod || isCsrfFailure) {
      clearCsrfTokenCache();
    ...
    return {
      success: false,
      message: payload403?.message || "Base API request failed",
      code: payload403?.code || "API_ERROR",
      ...payload403,
    };
  }
...
/** Command Protocol Phase 1A — server enforces unlock + one swap per UTC day. */
export async function setCommandProtocol({ protocol_id = "none" } = {}) {
  return apiFetch("/api/base/action/command-protocol", {
    method: "POST",
...
/**
 * DEV ONLY: force sector_world 1–6 for the current device (API no-ops when dev tools disabled).
 * Returns the same object shape as other BASE actions: apiFetch spreads server JSON (success, code, message, state, …).
 */
export async function devSetBaseSectorWorld(targetWorld) {
  return apiFetch("/api/base/action/dev-set-sector", {
    method: "POST",
    body: JSON.stringify({ target_world: targetWorld }),
  });
}
