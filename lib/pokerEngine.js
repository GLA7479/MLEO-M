...
export async function startHand(sessionId) {
  const { data: ses } = await supabase.from('poker_sessions').select('*').eq('id', sessionId).single();
  ...
  if (!ses || !players || players.length < 2) return { ok:false, reason:'need 2 seated' };

  const dealer = (typeof ses.dealer_seat==='number') ? nextOccupied(players, ses.dealer_seat) : players[0].seat_index;
  const sbSeat = nextOccupied(players, dealer);
  const bbSeat = nextOccupied(players, sbSeat);
  const utg    = nextOccupied(players, bbSeat);

  const sb = Number(ses.sb || 10);
  const bb = Number(ses.bb || 20);

  await supabase.from('poker_players').update({ bet_street: 0, acted: false, folded: false }).eq('session_id', sessionId);
...
export async function advanceStreet(sessionId){
  const { data: ses } = await supabase.from('poker_sessions').select('*').eq('id', sessionId).single();
  const { data: players } = await supabase.from('poker_players').select('*').eq('session_id', sessionId);
  ...
  const next = nextStage(ses.stage);
  if (next === 'showdown') {
    await settlePots(sessionId, ses.board||[], players||[]);
    return { ok:true };
  }

  await supabase.from('poker_players').update({ bet_street: 0, acted: false }).eq('session_id', sessionId);
  await supabase.from('poker_sessions').update({ stage: next, current_turn: nextToActAfterDealer(players||[], ses.dealer_seat), to_call: 0 }).eq('id', sessionId);
  return { ok:true };
}