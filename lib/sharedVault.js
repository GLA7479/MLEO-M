...
export async function syncSharedVault(gameId = "shared-vault") {
  ...
  const synced = Boolean(result?.ok);
...
export async function creditSharedVault(amount, gameId) {
  ...
  if (!isVaultAdapterEnabled()) {
    const snap = currentSnapshot();
    return {
      ok: false,
      synced: false,
      error: "Vault adapter disabled or unavailable",
      ...snap,
    };
  }
  ...
  return {
    ok: synced,
    synced,
    error: synced ? null : result?.error || "Sync failed",
    ...snapshot,
  };
}
