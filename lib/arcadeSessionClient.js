...
import { isKnownArcadeAppId, legacyDbIdFromAppId } from "./arcadeGameIds";

/** Map canonical arcade app id to legacy RPC game_id; pass through other ids (e.g. plinko) unchanged. */
function resolveRpcGameId(gameId) {
  if (isKnownArcadeAppId(gameId)) return legacyDbIdFromAppId(gameId);
  return gameId;
}
...
export async function startPaidArcadeSession(gameId, stake) {
  ...
  const rpcGameId = resolveRpcGameId(gameId);
  ...
  return {
    ...
    gameId: result?.gameId || rpcGameId,
    appGameId: gameId,
