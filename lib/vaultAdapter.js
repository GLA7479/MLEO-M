// lib/vaultAdapter.js
import { clearCsrfTokenCache, ensureArcadeDeviceCookie, ensureCsrfToken } from "./arcadeDeviceClient";
...
async function callArcadeApi(path, options = {}) {
  ...
  async function vaultFetchOnce(forceCsrf) {
    const csrfToken = await ensureCsrfToken(forceCsrf);
    if (!csrfToken && !isGet) {
      return {
        ok: false,
        error: "CSRF token not ready",
        payload: null,
        status: 0,
      };
    }
    ...
    try {
      ...
      return { ok: false, error: e?.message || String(e), payload: null, status: 0 };
    }
    ...
    return { ok: true, error: null, payload, status: response.status };
  }

  let out = await vaultFetchOnce(false);
  const csrfLike403 =
    !out.ok &&
    out.status === 403 &&
    (out.payload?.code === "CSRF_INVALID" ||
      /csrf/i.test(String(out.payload?.message || out.error || "")));
  if (csrfLike403) {
    clearCsrfTokenCache();
    out = await vaultFetchOnce(true);
  }

  if (out.ok) {
    return { ok: true, error: null, payload: out.payload };
  }
  return { ok: false, error: out.error, payload: out.payload };
}
...
/** True when this tab can reach the arcade vault sync API (`/api/arcade/vault/*`). */
export function isVaultAdapterEnabled() {
  return FLAG && hasWindow();
}
...
export async function flushDelta(gameId = "generic") {
  if (!hasWindow()) return { ok: true, skipped: true };
  const pending = roundDelta(deltaQueue);
  if (!FLAG) {
    if (!pending) return { ok: true, skipped: true };
    return { ok: false, skipped: true, error: "Vault adapter disabled" };
  }
  if (!pending) return { ok: true, skipped: true };
