...
export function setLocalVault(amount) {
...
export async function getBalance() {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_vault_balance", { p_device_id: deviceId });
    if (error) throw error;
    if (Array.isArray(data) && data.length) {
      // הפונקציה מחזירה table עם עמודה vault_balance (לאחר התיקון)
      const row = data[0];
      const bal = Number(row?.vault_balance ?? row?.balance ?? row ?? 0);
      return setLocalVault(bal);
    }
  } catch (err) {
    // השתק שגיאות "ambiguous balance" (42702) - זה אומר שהפונקציה SQL עדיין לא עודכנה
    const errMsg = String(err?.message || err || "");
    const errCode = err?.code || "";
    if (errCode !== "42702" && !errMsg.includes("ambiguous")) {
...
export async function flushDelta(gameId = "generic") {
  ...
  try {
    ...
    // Only log if it's not an auth error or 400 (function might not exist or device not found)
    const errMsg = String(err?.message || err || "");
    const errCode = err?.code || "";
    // Ignore: auth errors (401/403), 400 (bad request - might be expected), ambiguous errors, and auth messages
    if (errMsg && 
