...
/**
 * Ensure signed arcade device cookies exist. Prefer GET (no CSRF) so balance reads do not fail on CSRF races.
 */
async function ensureArcadeDeviceCookieClient() {
  const getRes = await fetch("/api/arcade/device", {
    method: "GET",
    credentials: "include",
  });
  if (getRes.ok) return;
  ...
  const postRes = await fetch("/api/arcade/device", {
    method: "POST",
    credentials: "include",
    headers: {
  ...
  if (!postRes.ok) {
    ...
    throw new Error(
      payload?.message || `Arcade device unavailable (${postRes.status})`,
    );
  }
}

async function callArcadeApi(path, options = {}) {
  await ensureArcadeDeviceCookieClient();
...
export async function flushDelta(gameId = "generic") {
  ...
  try {
    const payload = await callArcadeApi("/api/arcade/vault/credit", {
      method: "POST",
      body: JSON.stringify({
        amount: pending,
        gameId: String(gameId || "generic"),
    ...
    if (
      ...
      console.error("[vaultAdapter] flushDelta credit failed", err);
    }
    return { ok: false, error: errMsg };
  }
}
