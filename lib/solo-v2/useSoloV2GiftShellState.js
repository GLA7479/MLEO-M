...
import {
  ...
  SOLO_V2_GIFT_STORAGE_EVENT,
  SOLO_V2_GIFT_STORAGE_KEY,
  SOLO_V2_GIFT_STORAGE_LEGACY_KEY,
...
export function useSoloV2GiftShellState() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(SOLO_V2_GIFT_STORAGE_EVENT, bump);
    const onStorage = e => {
      if (e.key === SOLO_V2_GIFT_STORAGE_KEY || e.key === SOLO_V2_GIFT_STORAGE_LEGACY_KEY) bump();
    };
    window.addEventListener("storage", onStorage);
    return () => {
      window.removeEventListener(SOLO_V2_GIFT_STORAGE_EVENT, bump);
