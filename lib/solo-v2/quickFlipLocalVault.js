...
import { setLocalVault } from "../vaultAdapter";
...
export async function readQuickFlipSharedVaultBalance() {
  ...
  try {
    const snapshot = await readSharedVault({ fresh: false });
...
async function applySharedVaultDelta(netDelta) {
  ...
  const current = Math.max(0, toFiniteNumber(peekSharedVault()?.balance, 0));
  const next = Math.max(0, current + netDelta);
  setLocalVault(next);
  return { ok: true, balance: next, error: null };
}
