...
export const RAIL_LOGIC_MIN_WAGER = QUICK_FLIP_MIN_WAGER;

/** Payout multiplier when the route is valid on submit. */
export const RAIL_LOGIC_WIN_MULTIPLIER = 1.82;

export function payoutForRailLogic(entryCost) {
  const entry = Math.max(RAIL_LOGIC_MIN_WAGER, Math.floor(Number(entryCost) || 0));
  return Math.max(0, Math.floor(entry * RAIL_LOGIC_WIN_MULTIPLIER));
}

/**
 * @param {{ terminalKind: "win" | "lose"; payoutReturn: number; entryCost: number; fundingSource?: "vault" | "gift" }} args
 */
export function buildRailLogicSettlementSummary({
  terminalKind,
  payoutReturn,
  entryCost,
  fundingSource = "vault",
