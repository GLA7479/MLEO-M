...
import { SOLO_V2_SESSION_MODE } from "./server/sessionTypes";
...
/**
 * Ensures client vault apply matches Solo V2 gift rules: freeplay losses do not debit the vault;
 * wins credit the full round return (payoutReturn), not profit-only, when funding is gift or session is freeplay.
 */
export function normalizeMysteryChamberVaultSettlement(settlementSummary, sessionMode) {
  if (!settlementSummary || typeof settlementSummary !== "object") return settlementSummary;
  const payout = Math.max(0, Math.floor(Number(settlementSummary.payoutReturn ?? 0)));
  const entry = Math.max(
    MYSTERY_CHAMBER_MIN_WAGER,
    Math.floor(Number(settlementSummary.entryCost ?? 0)),
  );
  const isGift =
    settlementSummary.fundingSource === "gift" || sessionMode === SOLO_V2_SESSION_MODE.FREEPLAY;
  const netDelta = isGift ? payout : payout - entry;
  return {
    ...settlementSummary,
    fundingSource: isGift ? "gift" : settlementSummary.fundingSource || "vault",
    payoutReturn: payout,
    entryCost: entry,
    netDelta,
  };
