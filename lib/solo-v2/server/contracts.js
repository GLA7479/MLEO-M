...
export function parseCreateSessionPayload(body) {
  ...
  let gameOptions = {};
  if (gameKey === "diamonds") {
    const go = validateObject(payload.gameOptions ?? {}, 8);
    const diffRaw = go && go.difficulty != null ? String(go.difficulty).toLowerCase() : "medium";
    const allowed = new Set(["easy", "medium", "hard", "expert"]);
    gameOptions = { difficulty: allowed.has(diffRaw) ? diffRaw : "medium" };
  }

  return {
    ok: true,
    value: { gameKey, sessionMode, entryAmount, clientNonce, idempotencyKey, gameOptions },
  };
}
