...
/** @returns {number[][] | null} */
function parseSafeSigilSetsFromSummaryBody(s) {
  if (Array.isArray(s.safeSigilSets) && s.safeSigilSets.length === MYSTERY_CHAMBER_CHAMBER_COUNT) {
    const sets = [];
    for (let i = 0; i < MYSTERY_CHAMBER_CHAMBER_COUNT; i += 1) {
      const row = s.safeSigilSets[i];
  ...
  const legacy = Array.isArray(s.safeSigils) ? s.safeSigils.map(x => Math.floor(Number(x))) : [];
  if (
    legacy.length === MYSTERY_CHAMBER_CHAMBER_COUNT &&
    legacy.every(x => Number.isFinite(x) && x >= 0 && x < MYSTERY_CHAMBER_SIGIL_COUNT)
  ) {
    return legacy.map(x => [x]);
  }
  return null;
}
...
export function parseMysteryChamberActiveSummary(sessionRow) {
  ...
  const safeSigilSets = parseSafeSigilSetsFromSummaryBody(s);
  if (!safeSigilSets) return null;
...
export async function buildMysteryChamberSessionSnapshot(supabase, sessionRow) {
  ...
  if (sessionRow.session_status === SOLO_V2_SESSION_STATUS.RESOLVED) {
    ...
    let safeSigilSetRevealed = null;
    if (Array.isArray(summary.safeSigilSetRevealed) && summary.safeSigilSetRevealed.length >= 1) {
      const raw = summary.safeSigilSetRevealed.map(x => Math.floor(Number(x)));
      if (raw.every(n => Number.isFinite(n) && n >= 0 && n < MYSTERY_CHAMBER_SIGIL_COUNT)) {
        safeSigilSetRevealed = raw;
      }
    } else if (summary.safeSigilRevealed != null) {
      const x = Math.floor(Number(summary.safeSigilRevealed));
      if (Number.isFinite(x) && x >= 0 && x < MYSTERY_CHAMBER_SIGIL_COUNT) {
        safeSigilSetRevealed = [x];
      }
    }

    let safeSigilSetsResolved = null;
    if (Array.isArray(summary.safeSigilSets) && summary.safeSigilSets.length === MYSTERY_CHAMBER_CHAMBER_COUNT) {
      const parsed = parseSafeSigilSetsFromSummaryBody({ safeSigilSets: summary.safeSigilSets });
      if (parsed) safeSigilSetsResolved = parsed;
    } else if (Array.isArray(summary.safeSigils) && summary.safeSigils.length === MYSTERY_CHAMBER_CHAMBER_COUNT) {
      const legacy = summary.safeSigils.map(x => Math.floor(Number(x)));
      if (legacy.every(n => Number.isFinite(n) && n >= 0 && n < MYSTERY_CHAMBER_SIGIL_COUNT)) {
        safeSigilSetsResolved = legacy.map(n => [n]);
      }
    }

    let finalChamberIndex = null;
    if (summary.finalChamberIndex != null) {
      const n = Math.floor(Number(summary.finalChamberIndex));
      if (Number.isFinite(n) && n >= 0 && n < MYSTERY_CHAMBER_CHAMBER_COUNT) finalChamberIndex = n;
    }

    const failRowFromGrid =
      terminalKind === "fail" && safeSigilSetsResolved && finalChamberIndex != null
        ? safeSigilSetsResolved[finalChamberIndex]
        : [];
    const mergeFailSafe = (a, row) => {
      const s = new Set();
      for (const x of a || []) {
    ...
    const mergedSafeSigilSet =
      terminalKind === "fail"
        ? mergeFailSafe(safeSigilSetRevealed || [], failRowFromGrid)
        : safeSigilSetRevealed;

    const lastChosenSigilResolved =
    ...
    return {
      ...
      snapshot: {
        ...
        resolvedResult: {
          ...
          finalChamberIndex,
          chosenSigil: lastChosenSigilResolved,
          lastChosenSigil: lastChosenSigilResolved,
          safeSigilSet:
            mergedSafeSigilSet && mergedSafeSigilSet.length ? mergedSafeSigilSet : null,
          safeSigilSetRevealed,
          safeSigilRevealed:
            mergedSafeSigilSet && mergedSafeSigilSet.length ? mergedSafeSigilSet[0] : null,
          safeSigilSets: safeSigilSetsResolved,
