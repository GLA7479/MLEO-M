...
function buildPlayingTripleDice(sessionRow, pendingZone) {
  const entry = entryCostFromSessionRow(sessionRow);
  const zone = pendingZone != null ? normalizeTripleDiceZone(pendingZone) : null;
  const winChance = zone != null ? tripleDiceWinChancePercent(zone) : null;
  const projected = zone != null ? tripleDiceProjectedPayout(entry, zone) : null;
...
export async function buildTripleDiceSessionSnapshot(supabase, sessionRow) {
  ...
  if (sessionRow.session_status === SOLO_V2_SESSION_STATUS.RESOLVED) {
    ...
    const selectedZone = normalizeTripleDiceZone(summary.selectedZone);
  ...
  if (!rollConflict && forRoll.length === 1) {
    ...
    const zone = normalizeTripleDiceZone(row?.event_payload?.zone ?? row?.event_payload?.tripleDiceZone);
    if (Number.isFinite(eid) && eid > 0 && zone != null) {
      pendingRoll = {
        rollEventId: eid,
        zone,
  ...
  const playing = buildPlayingTripleDice(sessionRow, pendingRoll?.zone ?? null);
