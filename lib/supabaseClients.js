...
export const supabaseMP = (() => {
  ...
  if (!url || !key) {
    // SSR / Node: importing this module must not throw or Next will 500 every page that pulls Layout
    // or OV2 modules. Real Supabase calls run in the browser where NEXT_PUBLIC_* is inlined at build.
    if (typeof window === 'undefined') {
      console.warn(
        '[Supabase MP] Missing NEXT_PUBLIC_SUPABASE_URL_MP / ANON_KEY on server; using placeholder client for SSR only.'
      );
      const client = createClient('https://invalid.localhost', 'sb-ssr-placeholder-not-for-network', {
