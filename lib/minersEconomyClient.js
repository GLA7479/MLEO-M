import {
  ensureArcadeDeviceCookie,
  ensureCsrfToken,
  clearCsrfTokenCache,
} from "./arcadeDeviceClient";
...
/** Serialize miners POSTs so parallel calls cannot overwrite each other's CSRF cookie. */
let apiFetchChain = Promise.resolve();
...
function stopTabLockHeartbeat() {
  ...
  // Clean up lock from localStorage if we own it
  try {
    const currentLock = localStorage.getItem(TAB_LOCK_KEY);
    if (currentLock) {
...
function apiFetch(path, body) {
  const p = apiFetchChain.then(() => apiFetchImpl(path, body));
  apiFetchChain = p.then(
    () => {},
    () => {}
  );
  return p;
...
async function apiFetchImpl(path, body) {
  const deviceInit = await ensureArcadeDeviceCookie();
  if (!deviceInit?.success) {
    const error = new Error(deviceInit?.message || "Device not initialized");
    error.status = deviceInit?.status || 0;
    throw error;
  }

  // Fresh token every POST: cookie + header must match; parallel GET /api/csrf-token calls race otherwise.
  csrfTokenCache = null;
  clearCsrfTokenCache();
  let csrfToken = await ensureCsrfToken(true);
  if (!csrfToken) {
    const err = new Error("Failed to obtain CSRF token");
    err.status = 0;
    throw err;
  }
  ...
  const isCsrf403 =
    ...
    (payload?.code === "CSRF_INVALID" ||
      /csrf/i.test(String(payload?.message || "")));
  if (isCsrf403) {
    csrfTokenCache = null;
    clearCsrfTokenCache();
...
export async function fetchMinersState() {
  const deviceInit = await ensureArcadeDeviceCookie();
  if (!deviceInit?.success) {
    throw new Error(deviceInit?.message || "Device not initialized");
  }
