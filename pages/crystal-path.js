...
import CrystalPathBoard from "../components/solo-v2/CrystalPathBoard";
...
import {
  ...
  payoutForMultiplier,
} from "../lib/solo-v2/crystalPathConfig";
import {
  applyCrystalPathSettlementOnce,
...
const STATS_KEY = "solo_v2_crystal_path_stats_v1";
...
function writeCrystalPathStats(next) {
...
function crystalStripModel(rowCount, uiState, nCleared, terminalKind) {
...
function CrystalPathGameplayPanel({
  session,
  uiState,
  pulseCell,
  shakeCell,
  ...
  const cp = session?.crystalPath;
  const playing = cp?.playing;
  const rr = cp?.resolvedResult;
  const isTerminal = Boolean(rr) || session?.sessionStatus === "resolved";
  const revealPath = isTerminal && Array.isArray(rr?.safeColumns);
  const safeColumns = revealPath ? rr.safeColumns : null;
  const baseHist = isTerminal ? rr?.digHistory || [] : playing?.digHistory || [];
  const digHistory =
    isTerminal && rr?.terminalKind === "bomb" && rr?.finalRowIndex != null && rr?.lastPickColumn != null
      ? [...baseHist, { rowIndex: rr.finalRowIndex, column: rr.lastPickColumn }]
      : baseHist;
          ...
          >
            <div className="solo-v2-ladder-play-inner flex min-h-0 min-h-[11rem] flex-1 flex-col items-center justify-center px-0.5 py-1 sm:min-h-[12rem] sm:px-1 sm:py-1.5 lg:min-h-0 lg:px-1 lg:py-0.5">
              <CrystalPathBoard
                ...
                revealTerminal={revealPath}
                disabled={!canDig}
                pulseCell={pulseCell}
                shakeCell={shakeCell}
                onPickTile={onDigColumn}
              />
            </div>
...
export default function CrystalPathPage() {
  const [vaultBalance, setVaultBalance] = useState(0);
  const [vaultReady, setVaultReady] = useState(false);
  const [wagerInput, setWagerInput] = useState(String(CRYSTAL_PATH_MIN_WAGER));
  ...
  const [stats, setStats] = useState(readCrystalPathStats);
  ...
  useEffect(() => {
    ...
    applyCrystalPathSettlementOnce(sessionId, settlementSummary).then(settlementResult => {
  ...
  function applySessionReadState(sessionPayload, { resumed = false } = {}) {
    const cp = sessionPayload?.crystalPath;
    const readState = String(cp?.readState || sessionPayload?.readState || "");
    const st = String(sessionPayload?.sessionStatus || "");

    if (st === "resolved" && cp?.resolvedResult) {
      setResolvedResult({
        ...cp.resolvedResult,
        sessionId: sessionPayload.id,
        settlementSummary: cp.resolvedResult.settlementSummary,
      });
    ...
    if (readState === "choice_submitted") {
      setUiState(UI_STATE.SESSION_ACTIVE);
      setSessionNotice(resumed ? "Pick locked — resolving." : "Resolving your pick…");
      setErrorMessage("");
      return;
    }
    ...
    if (readState === "choice_required" || readState === "ready") {
      setResolvedResult(null);
      setUiState(UI_STATE.SESSION_ACTIVE);
      setSessionNotice(resumed ? "Resumed active run." : "Pick a safe tile on the current row.");
      setErrorMessage("");
      return;
    }
  ...
  async function postResolve(sessionId, body, activeCycle) {
    const response = await fetch("/api/solo-v2/crystal-path/resolve", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-solo-v2-player": PLAYER_HEADER,
  ...
  async function handleDigColumn(col) {
      ...
      const playing = sessionRef.current?.crystalPath?.playing;
  ...
  async function runStartRun() {
    ...
    if (!isGiftRound && wager < CRYSTAL_PATH_MIN_WAGER) return;
    ...
    const cpBoot = boot.session?.crystalPath;
    if (cpBoot?.readState === "choice_submitted") {
  ...
  useEffect(() => {
    const sid = session?.id;
    const cpResume = session?.crystalPath;
    if (!sid || !cpResume || uiState !== UI_STATE.SESSION_ACTIVE) return;
    if (cpResume.readState !== "choice_submitted" || !cpResume.canResolveTurn) return;
  ...
  }, [session?.id, session?.crystalPath?.readState, session?.crystalPath?.canResolveTurn, uiState]);

  const numericWager = parseWagerInput(wagerInput);
  const wagerPlayable =
    vaultReady && numericWager >= CRYSTAL_PATH_MIN_WAGER && vaultBalance >= numericWager;
  ...
  const stakeExceedsVault =
    vaultReady &&
    idleLike &&
    numericWager >= CRYSTAL_PATH_MIN_WAGER &&
  ...
  const cp = session?.crystalPath;
  const playing = cp?.playing;
  const readState = String(cp?.readState || "");

  const runEntryFromSession =
    session != null &&
    Number(session.entryAmount) >= CRYSTAL_PATH_MIN_WAGER &&
  ...
  const firstStepWinPreview = payoutForMultiplier(
    Math.max(CRYSTAL_PATH_MIN_WAGER, numericWager),
    CRYSTAL_PATH_MULTIPLIER_LADDER[0],
  );
  ...
  const rrSnap = cp?.resolvedResult;
  ...
  const strip = crystalStripModel(rowTotal, uiState, nCleared, stripTerminalKind);
  ...
  if (terminalKind === "bomb") resultTitle = "Wrong path — run lost";
  ...
  if (terminalKind === "bomb") {
    ...
    popupLine2 = Number.isFinite(fr) ? `Miss · row ${fr + 1}` : "Path failed";
  ...
  return (
    <SoloV2GameShell
      title="Crystal Path"
      subtitle="Choose paths. Climb higher."
      ...
      soloV2Footer={{
        ...
        onDecreaseAmount: () => {
          ...
          setWagerInput(prev => {
            ...
            return String(Math.min(MAX_WAGER, Math.max(0, c - CRYSTAL_PATH_MIN_WAGER)));
          });
        },
        onIncreaseAmount: () => {
          clearPresetChain();
        ...
        onResetAmount: () => {
          clearPresetChain();
          setWagerInput(String(CRYSTAL_PATH_MIN_WAGER));
        },
      ...
      gameplaySlot={
        <CrystalPathGameplayPanel
          ...
          stepLabels={stepLabels}
          showBoardCashOut={
            uiState === UI_STATE.SESSION_ACTIVE && !terminalSession && Boolean(cp?.canCashOut)
          }
      ...
      helpContent={
        <div className="space-y-2">
          <p>
            Crystal Path is a six-row climb: each row has three tiles, and exactly one is safe. The server seals safe
            positions before you pick. A safe tile advances you and raises your secured payout; a wrong tile ends the run.
          </p>
          <p>
            After any safe row you may cash out from the lower band under the grid and above the stake bar. Secured
            payout on small screens appears in the summary strip above the grid; on large screens it also appears beside the
            stake controls. Gift rounds use freeplay — a loss does not debit your vault; a win credits the full payout.
          </p>
          <p>
            After the result popup closes, the finished recap stays visible — press START RUN explicitly for the next
