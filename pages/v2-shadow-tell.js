...
import ShadowTellBoard from "../components/solo-v2/ShadowTellBoard";
...
import { SHADOW_TELL_MIN_WAGER, shadowTellBestCaseReturn } from "../lib/solo-v2/shadowTellConfig";
import {
  applyShadowTellSettlementOnce,
...
const GAME_KEY = "shadow_tell";
const PLAYER_HEADER = "v2-shadow-tell-client";
...
const STATS_KEY = "solo_v2_shadow_tell_stats_v1";
...
function profilePublicLabel(key) {
  const k = String(key || "");
  if (k === "weak") return "Soft stance";
  if (k === "balanced") return "Even pressure";
  if (k === "strong") return "Hard line";
  return k || "—";
}

function choicePublicLabel(choice) {
  const c = String(choice || "");
  if (c === "challenge") return "Challenged";
  if (c === "safe") return "Played safe";
  if (c === "middle") return "Split commit";
  return c || "—";
}

function ShadowGameplayPanel({
  session,
  uiState,
  sessionNotice,
  boardPhase,
  ...
  const st = session?.shadowTell;
  const playing = st?.playing;
  const rr = st?.resolvedResult || pendingResolved;
  ...
  const total = Math.max(1, Math.floor(Number(stepTotal) || 3));
  ...
  const busyFooter = uiState === UI_STATE.RESOLVING || uiState === UI_STATE.LOADING || decideLoading;

  const clues = playing?.clues || [];
  const boardPhaseForUi = isTerminal ? BOARD_PHASE.DECIDE : boardPhase;
  const caption =
    boardPhase === BOARD_PHASE.SEALED
      ? "One tell may mislead"
      : boardPhase === BOARD_PHASE.CLUES
        ? "Weigh the set"
        : boardPhase === BOARD_PHASE.DECIDE
          ? "Pick your line"
          : "";
    ...
    <div className="solo-v2-route-stack relative flex h-full min-h-0 w-full flex-col px-1 pt-0 text-center sm:px-2 sm:pt-1 lg:px-4 lg:pt-1">
      <div className="solo-v2-board-frame flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border-2 border-amber-900/45 bg-gradient-to-b from-zinc-900 to-zinc-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
        ...
        <SoloV2ProgressStrip
          keyPrefix="st"
          rowLabel="Round"
          ariaLabel="Shadow Tell progress"
          stepTotal={total}
        ...
        <div className="solo-v2-ladder-playfield-wrap flex min-h-0 flex-1 flex-col px-1 pb-1 sm:px-2 lg:min-h-0 lg:px-4 lg:pb-1.5">
          <div
            ...
            aria-label="Shadow Tell board"
          >
            <div className="solo-v2-ladder-play-inner flex min-h-0 min-h-[11rem] flex-1 flex-col items-center justify-center px-0.5 py-1.5 sm:min-h-[12rem] sm:px-1 sm:py-2 lg:min-h-0 lg:py-1.5">
              {playing && !isTerminal ? (
                <>
                  <ShadowTellBoard
                    clues={clues}
                    revealCount={boardPhase === BOARD_PHASE.SEALED ? 0 : revealCount}
                    phase={boardPhaseForUi === BOARD_PHASE.SEALED ? "sealed" : boardPhaseForUi === BOARD_PHASE.CLUES ? "clues" : "decide"}
                    subtleCaption={caption}
                  />
                  {boardPhase === BOARD_PHASE.DECIDE ? (
                    <div className="mt-4 grid w-full max-w-md grid-cols-1 gap-2 sm:max-w-lg sm:grid-cols-3 sm:gap-2">
                      <button
                        type="button"
                        onClick={() => onDecide("challenge")}
                        disabled={busyFooter || !st?.canDecide}
                        className={`min-h-[48px] rounded-xl border px-2 py-2.5 text-[10px] font-extrabold uppercase tracking-wide sm:text-xs ${
                          busyFooter || !st?.canDecide
                      ...
                      <button
                        ...
                        className={`min-h-[48px] rounded-xl border px-2 py-2.5 text-[10px] font-extrabold uppercase tracking-wide sm:text-xs ${
                          busyFooter || !st?.canDecide
                            ? "cursor-not-allowed border-white/12 bg-white/5 text-zinc-500"
                            : "border-emerald-500/45 bg-emerald-950/30 text-emerald-100 hover:bg-emerald-900/35"
                        }`}
                      >
                        Play safe
                      </button>
                      <button
                        type="button"
                        onClick={() => onDecide("middle")}
                        disabled={busyFooter || !st?.canDecide}
                        className={`min-h-[48px] rounded-xl border px-2 py-2.5 text-[10px] font-extrabold uppercase tracking-wide sm:text-xs ${
                          busyFooter || !st?.canDecide
                            ? "cursor-not-allowed border-white/12 bg-white/5 text-zinc-500"
                            : "border-violet-500/45 bg-violet-950/30 text-violet-100 hover:bg-violet-900/35"
                        }`}
                      >
                        Middle
                      </button>
                ...
                <div className="px-2 text-center">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-zinc-400">Round closed</p>
                  <p className="mt-2 text-xs font-semibold text-zinc-200">
                    They showed: <span className="text-violet-200">{profilePublicLabel(rr.revealedProfile)}</span>
                  </p>
                  <p className="mt-1 text-[11px] text-zinc-400">You {choicePublicLabel(rr.playerChoice).toLowerCase()}.</p>
                </div>
              ) : null}
            </div>
          </div>
      ...
      >
        <div className="text-[13px] font-black uppercase tracking-wide">Shadow Tell</div>
...
export default function ShadowTellPage() {
  ...
  const [wagerInput, setWagerInput] = useState(String(SHADOW_TELL_MIN_WAGER));
  const [resultPopupOpen, setResultPopupOpen] = useState(false);
  const [boardPhase, setBoardPhase] = useState(BOARD_PHASE.IDLE);
  ...
  function resetRoundAfterResultPopup() {
    ...
    resolveInFlightRef.current = false;
    clearRevealTimers();
    setBoardPhase(BOARD_PHASE.IDLE);
    setRevealCount(0);
    setSession(null);
  ...
  useEffect(() => {
    ...
    applyShadowTellSettlementOnce(sessionId, settlementSummary).then(settlementResult => {
  ...
  }, [uiState, resolvedResult?.sessionId, resolvedResult?.settlementSummary, resolvedResult?.isWin, resolvedResult?.won, session?.id, openResultPopup]);

  const applySessionReadState = useCallback((sessionPayload, { resumed = false } = {}) => {
    const st = sessionPayload?.shadowTell;
    const readState = String(st?.readState || sessionPayload?.readState || "");
    const st0 = String(sessionPayload?.sessionStatus || "");

    if (st0 === "resolved" && st?.resolvedResult) {
      setResolvedResult({
        ...st.resolvedResult,
        sessionId: sessionPayload.id,
        settlementSummary: st.resolvedResult.settlementSummary,
      });
      setUiState(UI_STATE.RESOLVED);
      setBoardPhase(BOARD_PHASE.DECIDE);
      setRevealCount(99);
      setSessionNotice(resumed ? "Round restored (finished)." : "");
      setErrorMessage("");
      return;
    }

    if (readState === "inference_active") {
      setResolvedResult(null);
      setUiState(UI_STATE.SESSION_ACTIVE);
      setBoardPhase(BOARD_PHASE.SEALED);
      setRevealCount(0);
      setSessionNotice(resumed ? "Run resumed." : "Opponent is sealed — watch the tells.");
      setErrorMessage("");
      return;
    }
  ...
  const clueCount = session?.shadowTell?.playing?.clues?.length ?? 0;

  useEffect(() => {
    if (uiState !== UI_STATE.SESSION_ACTIVE || boardPhase !== BOARD_PHASE.SEALED) return;
    clearRevealTimers();
    if (clueCount < 1) return;

    const tSeal = window.setTimeout(() => {
      setBoardPhase(BOARD_PHASE.CLUES);
      setRevealCount(0);
    }, 520);
  ...
  }, [uiState, boardPhase, session?.id, clueCount, clearRevealTimers]);

  useEffect(() => {
    if (uiState !== UI_STATE.SESSION_ACTIVE || boardPhase !== BOARD_PHASE.CLUES) return;
    clearRevealTimers();
    const n = clueCount;
    if (!n) return;

    for (let i = 0; i < n; i++) {
      const t = window.setTimeout(() => setRevealCount(i + 1), 360 * (i + 1));
      timersRef.current.push(t);
    }
    const tDone = window.setTimeout(() => setBoardPhase(BOARD_PHASE.DECIDE), 360 * (n + 1) + 280);
  ...
  async function handleDecide(choice) {
    ...
    if (!sid || decideLoading || resolveInFlightRef.current) return;
    if (boardPhase !== BOARD_PHASE.DECIDE) return;
    const activeCycle = cycleRef.current;
    setDecideLoading(true);
    setUiState(UI_STATE.RESOLVING);
    try {
      const response = await fetch("/api/solo-v2/shadow-tell/resolve", {
        method: "POST",
        headers: { "content-type": "application/json", "x-solo-v2-player": PLAYER_HEADER },
        body: JSON.stringify({ sessionId: sid, action: "decide", choice }),
      });
      const payload = await response.json().catch(() => null);
      ...
      if (api === SOLO_V2_API_RESULT.PENDING_MIGRATION) {
        setUiState(UI_STATE.PENDING_MIGRATION);
        setErrorMessage(buildSoloV2ApiErrorMessage(payload, "Migration is pending."));
      } else {
        setUiState(UI_STATE.SESSION_ACTIVE);
        setErrorMessage(buildSoloV2ApiErrorMessage(payload, "Commit rejected."));
      }
    } catch (_e) {
      ...
      if (activeCycle === cycleRef.current) setDecideLoading(false);
    }
  }
  ...
  async function runStartRun() {
    ...
    if (!isGiftRound && wager < SHADOW_TELL_MIN_WAGER) {
      setErrorMessage(`Minimum play is ${SHADOW_TELL_MIN_WAGER}.`);
      return;
    }
  ...
  const wagerPlayable =
    vaultReady && numericWager >= SHADOW_TELL_MIN_WAGER && vaultBalance >= numericWager;
  ...
  const stakeExceedsVault =
    vaultReady &&
    idleLike &&
    numericWager >= SHADOW_TELL_MIN_WAGER &&
  ...
  const runEntryFromSession =
    session != null &&
    Number(session.entryAmount) >= SHADOW_TELL_MIN_WAGER &&
    Number.isFinite(Number(session.entryAmount))
      ? Math.floor(Number(session.entryAmount))
      : null;

  let summaryPlay = numericWager;
  let summaryWin = shadowTellBestCaseReturn(Math.max(SHADOW_TELL_MIN_WAGER, numericWager));

  const inActiveRunUi =
    uiState === UI_STATE.SESSION_ACTIVE || uiState === UI_STATE.RESOLVING || uiState === UI_STATE.LOADING;

  if (runEntryFromSession != null && (inActiveRunUi || uiState === UI_STATE.RESOLVED)) {
    summaryPlay = runEntryFromSession;
  }

  if (session?.shadowTell?.playing?.bestCaseReturn != null && inActiveRunUi) {
    summaryWin = Math.floor(Number(session.shadowTell.playing.bestCaseReturn) || summaryWin);
  }
  ...
  if (uiState === UI_STATE.RESOLVED) {
    stepsComplete = 3;
    currentStepIndex = 2;
  } else if (boardPhase === BOARD_PHASE.DECIDE || uiState === UI_STATE.RESOLVING) {
    stepsComplete = 2;
    currentStepIndex = 2;
  } else if (boardPhase === BOARD_PHASE.CLUES) {
    stepsComplete = 1;
    currentStepIndex = 1;
  } else if (boardPhase === BOARD_PHASE.SEALED && uiState === UI_STATE.SESSION_ACTIVE) {
    stepsComplete = 0;
    currentStepIndex = 0;
  }

  const stepLabels = ["Seal", "Tells", "Call"];

  let payoutBandLabel = "Best-case read";
  ...
  const revealed = profilePublicLabel(resolvedResult?.revealedProfile);
  const popupLine3 = resolvedIsWin ? `${revealed} — paid` : `${revealed} — under water`;
  ...
  return (
    <SoloV2GameShell
      title="Shadow Tell"
      subtitle="Read the sealed stance."
      ...
      topGameStatsSlot={
        <>
          ...
          <span className="inline-flex shrink-0 items-baseline gap-0.5 whitespace-nowrap text-zinc-500">
            <span>Cap</span>
            <span className="font-semibold tabular-nums text-violet-200/90">{formatCompact(summaryWin)}</span>
          </span>
        </>
      }
      soloV2Footer={{
        ...
        onDecreaseAmount: () => {
          ...
          setWagerInput(prev => {
            ...
            return String(Math.min(MAX_WAGER, Math.max(0, c - SHADOW_TELL_MIN_WAGER)));
          });
        },
        onIncreaseAmount: () => {
          clearPresetChain();
        ...
        onResetAmount: () => {
          clearPresetChain();
          setWagerInput(String(SHADOW_TELL_MIN_WAGER));
        },
      ...
      helpContent={
        <div className="space-y-2">
          <p>
            Shadow Tell hides a single opponent stance: <strong>soft</strong>, <strong>even</strong>, or{" "}
            <strong>hard</strong>. You only see oblique tells — plus one line that may not belong to the truth.
          </p>
          <p>
            <strong>Challenge</strong> pays big against a soft read but bleeds against hard pressure.{" "}
            <strong>Play safe</strong> leans defensive. <strong>Middle</strong> splits exposure when you cannot
            trust the full set.
          </p>
          <p>
            Outcomes are server-sealed; clues are partial by design. This is inference, not a second treasure-door
            pick.
          </p>
        </div>
      }
