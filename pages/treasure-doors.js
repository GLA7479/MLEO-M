      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border-2 border-amber-900/45 bg-gradient-to-b from-zinc-900 to-zinc-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
        ...
        <div className="flex min-h-0 flex-1 flex-col px-1 pb-1 sm:px-2 lg:min-h-0 lg:px-4 lg:pb-1.5">
          <div
            ...
            aria-label="Treasure Doors chamber"
          >
            <div className="flex min-h-0 min-h-[11rem] flex-1 flex-col py-0.5 sm:min-h-[12rem] sm:py-1 lg:min-h-0 lg:px-1 lg:py-0.5">
              <TreasureDoorsBoard
...
export default function TreasureDoorsPage() {
  ...
  if (uiState === UI_STATE.UNAVAILABLE) {
    ...
    const chIdx = Math.floor(Number(playing?.currentChamberIndex ?? 0));
    statusTop = `Chamber ${Math.min(chIdx + 1, chamberTotal)} of ${chamberTotal} — choose a door.`;
    ...
    <SoloV2GameShell
      ...
      helpContent={
        <div className="space-y-2">
          ...
          <p>
            After any safe chamber you can bank secured loot from the lower band of the temple panel, under the doors and
            above the stake bar. Secured payout on small screens appears in the summary strip above the doors; on large screens
            it also appears beside the stake controls. Gift rounds use freeplay — a loss does not debit your vault; a win
            credits the full payout.
          </p>
          <p>
