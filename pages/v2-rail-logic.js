...
import RailLogicBoard from "../components/solo-v2/RailLogicBoard";
...
import {
  ...
  payoutForRailLogic,
} from "../lib/solo-v2/railLogicConfig";
import {
  applyRailLogicSettlementOnce,
...
const GAME_KEY = "rail_logic";
const PLAYER_HEADER = "v2-rail-logic-client";
...
const STATS_KEY = "solo_v2_rail_logic_stats_v1";
...
function RailGameplayPanel({
  ...
  const rl = session?.railLogic;
  const playing = rl?.playing;
  const rr = rl?.resolvedResult;
  ...
  const busyFooter =
    uiState === UI_STATE.RESOLVING || uiState === UI_STATE.LOADING || rotateLoading || submitLoading || forfeitLoading;
              ...
              {playing && !isTerminal ? (
                <>
                  <RailLogicBoard
                    ...
                    gridH={playing.gridH}
                    types={playing.types}
                    rotations={playing.rotations}
                    startIdx={playing.startIdx}
                    endIdx={playing.endIdx}
                    startGate={playing.startGate}
                    endGate={playing.endGate}
                    routeComplete={Boolean(playing.routeComplete)}
                    disabled={busyFooter || !rl?.canRotate}
                    onCellTap={onCellTap}
                  />
                  <div className="mt-3 flex w-full max-w-md flex-col gap-2 sm:max-w-lg">
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={onSubmit}
                        disabled={busyFooter || !rl?.canSubmit}
                        className={`min-h-[48px] flex-1 rounded-xl border px-3 py-2.5 text-xs font-extrabold uppercase tracking-wide sm:text-sm ${
                          busyFooter || !rl?.canSubmit
                            ? "cursor-not-allowed border-white/12 bg-white/5 text-zinc-500"
                            : "border-emerald-500/45 bg-emerald-950/35 text-emerald-100 hover:bg-emerald-900/40"
                        }`}
                      >
                        {submitLoading ? "Locking…" : "Lock route"}
                      </button>
                      <button
                        type="button"
                        onClick={onForfeit}
                        disabled={busyFooter}
                        className={`min-h-[48px] flex-1 rounded-xl border px-3 py-2.5 text-xs font-bold uppercase tracking-wide sm:text-sm ${
                          busyFooter
                            ? "cursor-not-allowed border-white/10 bg-white/5 text-zinc-500"
                            : "border-rose-500/35 bg-rose-950/25 text-rose-100/90 hover:bg-rose-900/35"
                        }`}
                      >
                        {forfeitLoading ? "…" : "Give up"}
                      </button>
                    </div>
                    <p className="text-[10px] font-semibold text-zinc-500 sm:text-[11px]">
                      Moves left:{" "}
                      <span className="tabular-nums text-amber-100/90">{playing.movesRemaining ?? "—"}</span>
                      {playing.routeComplete ? (
                        <span className="text-emerald-300/90"> · path complete — lock when ready</span>
                      ) : null}
                    </p>
                  </div>
                </>
              ) : isTerminal && rr ? (
                <div className="px-2 py-4 text-sm text-zinc-400">
                  <p className="font-semibold text-zinc-300">
                    {rr.terminalKind === "win" ? "Route locked — payout pending." : "Round ended."}
                  </p>
                  <p className="mt-2 text-xs">Press START RUN for another puzzle.</p>
                </div>
              ) : null}
            </div>
          </div>
...
export default function V2RailLogicPage() {
  const [vaultBalance, setVaultBalance] = useState(0);
  const [vaultReady, setVaultReady] = useState(false);
  const [wagerInput, setWagerInput] = useState(String(RAIL_LOGIC_MIN_WAGER));
  ...
  useEffect(() => {
    ...
    applyRailLogicSettlementOnce(sessionId, settlementSummary).then(settlementResult => {
  ...
  const applySessionReadState = useCallback((sessionPayload, { resumed = false } = {}) => {
    const rl = sessionPayload?.railLogic;
    const readState = String(rl?.readState || sessionPayload?.readState || "");
    const st = String(sessionPayload?.sessionStatus || "");

    if (st === "resolved" && rl?.resolvedResult) {
      setResolvedResult({
        ...rl.resolvedResult,
        sessionId: sessionPayload.id,
        settlementSummary: rl.resolvedResult.settlementSummary,
      });
    ...
    if (readState === "puzzle_active") {
      setResolvedResult(null);
      setUiState(UI_STATE.SESSION_ACTIVE);
      setSessionNotice(
        resumed ? "Puzzle resumed." : "Rotate tiles to connect mine → exit. Lock when the route is valid.",
      );
      setErrorMessage("");
      return;
    }
  ...
  async function postRailAction(sessionId, body, activeCycle) {
    const response = await fetch("/api/solo-v2/rail-logic/resolve", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-solo-v2-player": PLAYER_HEADER,
  ...
  async function refreshSessionRow(sessionId, activeCycle) {
  ...
  async function handleCellTap(idx) {
    const sid = session?.id;
    if (!sid || rotateLoading || submitLoading || forfeitLoading) return;
    setRotateLoading(true);
    const activeCycle = cycleRef.current;
    try {
      const { response, payload, halted } = await postRailAction(sid, { action: "rotate", cellIndex: idx }, activeCycle);
      if (halted) return;
      const api = classifySoloV2ApiResult(response, payload);
      const status = String(payload?.status || "");
      if (api === SOLO_V2_API_RESULT.SUCCESS && status === "rotated") {
        await refreshSessionRow(sid, activeCycle);
        return;
      }
      if (status === "no_moves") {
        setSessionNotice("No rotations left — lock route or give up.");
      }
      setErrorMessage(buildSoloV2ApiErrorMessage(payload, "Rotate failed."));
      await refreshSessionRow(sid, activeCycle);
    } finally {
  ...
  async function handleSubmit() {
    const sid = session?.id;
    if (!sid || submitLoading || forfeitLoading || rotateLoading) return;
    setSubmitLoading(true);
    setUiState(UI_STATE.RESOLVING);
    const activeCycle = cycleRef.current;
    try {
      const { response, payload, halted } = await postRailAction(sid, { action: "submit" }, activeCycle);
      if (halted) return;
      const status = String(payload?.status || "");
      const api = classifySoloV2ApiResult(response, payload);
      if (api === SOLO_V2_API_RESULT.SUCCESS && status === "not_solved") {
        setSessionNotice("Route not complete — keep rotating, or give up.");
        await refreshSessionRow(sid, activeCycle);
        setUiState(UI_STATE.SESSION_ACTIVE);
        return;
      }
      ...
      setErrorMessage(buildSoloV2ApiErrorMessage(payload, "Lock route failed."));
      await refreshSessionRow(sid, activeCycle);
      setUiState(UI_STATE.SESSION_ACTIVE);
    } finally {
  ...
  async function handleForfeit() {
    const sid = session?.id;
    if (!sid || forfeitLoading || submitLoading || rotateLoading) return;
    setForfeitLoading(true);
    setUiState(UI_STATE.RESOLVING);
    const activeCycle = cycleRef.current;
    try {
      const { response, payload, halted } = await postRailAction(sid, { action: "forfeit" }, activeCycle);
      if (halted) return;
      const status = String(payload?.status || "");
      const api = classifySoloV2ApiResult(response, payload);
      if (api === SOLO_V2_API_RESULT.SUCCESS && status === "resolved" && payload?.result) {
      ...
      setErrorMessage(buildSoloV2ApiErrorMessage(payload, "Give up failed."));
      await refreshSessionRow(sid, activeCycle);
      setUiState(UI_STATE.SESSION_ACTIVE);
    } finally {
  ...
  async function runStartRun() {
    if (createInFlightRef.current || submitLoading || forfeitLoading || rotateLoading) return;
    ...
    if (!isGiftRound && wager < RAIL_LOGIC_MIN_WAGER) return;
  ...
  const wagerPlayable =
    vaultReady && numericWager >= RAIL_LOGIC_MIN_WAGER && vaultBalance >= numericWager;
  ...
  const stakeExceedsVault =
    vaultReady &&
    idleLike &&
    numericWager >= RAIL_LOGIC_MIN_WAGER &&
  ...
  const runEntryFromSession =
    session != null &&
    Number(session.entryAmount) >= RAIL_LOGIC_MIN_WAGER &&
    Number.isFinite(Number(session.entryAmount))
      ? Math.floor(Number(session.entryAmount))
      : null;

  let summaryPlay = numericWager;
  let summaryWin = payoutForRailLogic(Math.max(RAIL_LOGIC_MIN_WAGER, numericWager));
  ...
  if (playing?.potentialPayout != null && inActiveRunUi) {
    summaryWin = Math.floor(Number(playing.potentialPayout) || summaryWin);
  }
  ...
  const terminalSession = Boolean(rl?.resolvedResult) || session?.sessionStatus === "resolved";
  const stepTotal = 2;
  let stepsComplete = 0;
  ...
  if (terminalSession || uiState === UI_STATE.RESOLVED) {
    stepsComplete = 2;
    currentStepIndex = 1;
  } else if (playing && (playing.movesUsed || 0) > 0) {
    stepsComplete = 1;
    currentStepIndex = 1;
  }
  const stepLabels = ["Board", "Lock"];

  let payoutBandLabel = "Solve payout";
  ...
  let popupLine3 = resolvedIsWin ? "Rail connected" : "No payout this round";
  if (resolvedResult?.terminalKind === "lose") {
    popupLine3 = "Give up or route failed";
  }
  ...
  const handleGiftPlay = useCallback(() => {
    ...
    if (createInFlightRef.current || submitLoading || forfeitLoading || rotateLoading) return;
    if ([UI_STATE.LOADING, UI_STATE.RESOLVING, UI_STATE.PENDING_MIGRATION].includes(uiState)) {
      return;
    }
    giftRoundRef.current = true;
    void runStartRun();
  }, [vaultReady, giftShell.giftCount, uiState, submitLoading, forfeitLoading, rotateLoading]);
  ...
  return (
    <SoloV2GameShell
      title="Rail Logic"
      subtitle="Full board. Fix the route."
      ...
      soloV2Footer={{
        ...
        onDecreaseAmount: () => {
          ...
          setWagerInput(prev => {
            ...
            return String(Math.min(MAX_WAGER, Math.max(0, c - RAIL_LOGIC_MIN_WAGER)));
          });
        },
        onIncreaseAmount: () => {
          clearPresetChain();
        ...
        onResetAmount: () => {
          clearPresetChain();
          setWagerInput(String(RAIL_LOGIC_MIN_WAGER));
        },
      ...
      helpContent={
        <div className="space-y-2">
          <p>
            Rail Logic is a full-information puzzle: every track tile, the mine (IN), and the exit (OUT) are visible
            from the first frame. There are no hidden hazards or secret trap paths — you win by logic, not luck.
          </p>
          <p>
            Tap track tiles to rotate them 90°. You have a limited number of rotations; when the rails connect a
            continuous path from the mine portal to the exit portal, tap Lock route to validate. Wrong locks warn
            without ending the run until you give up or run out of moves.
          </p>
          <p>
            This is intentionally different from crystal path or treasure doors: nothing is “safe until you pick it”.
            Plan ahead, then commit. Gift rounds are freeplay on stake; wins credit the full return.
          </p>
        </div>
      }
      statsContent={
