...
const STATS_KEY = "solo_v2_solo_ladder_stats_v1";
...
function LadderGameplayPanel({
  ...
  const sl = session?.soloLadder;
  const playing = sl?.playing;
  const rr = sl?.resolvedResult;
  const isTerminal = Boolean(rr) || session?.sessionStatus === "resolved";
  const successCount = isTerminal
    ? Math.floor(Number(rr?.successCount ?? 0))
    : Math.floor(Number(playing?.successCount ?? 0));

  const showSession = Boolean(sessionNotice);
  const total = Math.max(1, Math.floor(Number(stepTotal) || SOLO_LADDER_STEP_COUNT));
...
export default function V2LadderPage() {
  const [vaultBalance, setVaultBalance] = useState(0);
  const [vaultReady, setVaultReady] = useState(false);
  const [wagerInput, setWagerInput] = useState(String(SOLO_LADDER_MIN_WAGER));
  ...
  const [climbLoading, setClimbLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    applySoloLadderSettlementOnce(sessionId, settlementSummary).then(settlementResult => {
  ...
  function applySessionReadState(sessionPayload, { resumed = false } = {}) {
    const sl = sessionPayload?.soloLadder;
    const readState = String(sl?.readState || sessionPayload?.readState || "");
    const st = String(sessionPayload?.sessionStatus || "");

    if (st === "resolved" && sl?.resolvedResult) {
      setResolvedResult({
        ...sl.resolvedResult,
        sessionId: sessionPayload.id,
        settlementSummary: sl.resolvedResult.settlementSummary,
      });
  ...
  async function postResolve(sessionId, body, activeCycle) {
    const response = await fetch("/api/solo-v2/solo-ladder/resolve", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-solo-v2-player": PLAYER_HEADER,
  ...
  async function handleClimb() {
    ...
    if (!sid || climbLoading || resolveInFlightRef.current || cashOutLoading) return;
    if (resultPopupTimerRef.current) {
      clearTimeout(resultPopupTimerRef.current);
      resultPopupTimerRef.current = null;
    }
    setResultPopupOpen(false);
    setClimbLoading(true);
    const activeCycle = cycleRef.current;
    setUiState(UI_STATE.RESOLVING);
    try {
      const { response, payload, halted } = await postResolve(sid, { action: "climb" }, activeCycle);
      if (halted) return;
      const status = String(payload?.status || "");
      ...
      setErrorMessage(buildSoloV2ApiErrorMessage(payload, "Climb failed."));
      const readResult = await readSessionTruth(sid, activeCycle);
      if (readResult?.ok && readResult.session) {
        setSession(readResult.session);
        applySessionReadState(readResult.session, { resumed: true });
  ...
  async function handleCashOut() {
    const sid = session?.id;
    if (!sid || cashOutLoading || resolveInFlightRef.current || climbLoading) return;
    if (resultPopupTimerRef.current) {
      clearTimeout(resultPopupTimerRef.current);
      resultPopupTimerRef.current = null;
    }
  ...
  async function runStartClimb() {
    ...
    if (!isGiftRound && wager < SOLO_LADDER_MIN_WAGER) return;
  ...
  const wagerPlayable =
    vaultReady && numericWager >= SOLO_LADDER_MIN_WAGER && vaultBalance >= numericWager;
  ...
  const stakeExceedsVault =
    vaultReady &&
    idleLike &&
    numericWager >= SOLO_LADDER_MIN_WAGER &&
  ...
  const runEntryFromSession =
    session != null &&
    Number(session.entryAmount) >= SOLO_LADDER_MIN_WAGER &&
    Number.isFinite(Number(session.entryAmount))
      ? Math.floor(Number(session.entryAmount))
      : null;

  const firstMult = soloLadderMultiplierAfterSuccesses(1);
  let summaryPlay = numericWager;
  let summaryWin =
    firstMult != null ? payoutForSoloLadder(Math.max(SOLO_LADDER_MIN_WAGER, numericWager), firstMult) : numericWager;
  ...
  if (playing && (uiState === UI_STATE.SESSION_ACTIVE || uiState === UI_STATE.RESOLVING)) {
  ...
  const nSuccess = Math.floor(Number(playing?.successCount ?? 0)) || 0;
  const rrSnap = sl?.resolvedResult;
  const terminalSession = Boolean(rrSnap) || session?.sessionStatus === "resolved";
  const successForStrip = terminalSession
    ? Math.floor(Number(rrSnap?.successCount ?? SOLO_LADDER_STEP_COUNT))
    : nSuccess;
  const stripTerminalKind =
    uiState === UI_STATE.RESOLVED ? resolvedResult?.terminalKind ?? null : rrSnap?.terminalKind ?? null;
  const stepTotal = SOLO_LADDER_STEP_COUNT;
  let stepsComplete = Math.min(stepTotal, successForStrip);
  let currentStepIndex = Math.min(stepTotal - 1, successForStrip);
  if (uiState === UI_STATE.RESOLVED && stripTerminalKind === "full_clear") {
    stepsComplete = stepTotal;
    currentStepIndex = stepTotal - 1;
  }
    ...
    <SoloV2GameShell
      ...
      soloV2Footer={{
        ...
        onDecreaseAmount: () => {
          ...
          setWagerInput(prev => {
            ...
            return String(Math.min(MAX_WAGER, Math.max(0, c - SOLO_LADDER_MIN_WAGER)));
          });
        },
        onIncreaseAmount: () => {
          clearPresetChain();
      ...
      gameplaySlot={
        <LadderGameplayPanel
          ...
          stepLabels={stepLabels}
          showBoardCashOut={
            uiState === UI_STATE.SESSION_ACTIVE && !terminalSession && Boolean(sl?.canCashOut)
          }
          boardCashOutDisabled={cashOutLoading || busyFooter || climbLoading}
