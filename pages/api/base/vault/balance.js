...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rateLimit = await checkArcadeRateLimit("base-vault-balance", deviceId, 60, 60_000);
    if (!rateLimit.allowed) {
      return res.status(429).json({ success: false, message: "Too many base balance requests" });
    }
    ...
    console.error("base/vault/balance failed", error);
    return res.status(500).json({ success: false, message: "Base vault balance API failed" });
  }
}
