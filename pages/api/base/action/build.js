...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return res.status(429).json({ success: false, code: "RATE_LIMIT_DEVICE", message: "Too many build requests" });
    }

    const { building_key } = req.body || {};
    if (!building_key || typeof building_key !== "string") {
      return res.status(400).json({ success: false, code: "BASE_INVALID_BUILDING_KEY", message: "Missing or invalid building_key" });
    }

    const supabase = getSupabaseAdmin();

    // Use atomic RPC function
    const { data: rpcData, error: rpcError } = await supabase.rpc("base_build_upgrade", {
      p_device_id: deviceId,
      p_building_key: building_key,
    });

    if (rpcError) {
      const errorMessage = rpcError.message || "Failed to build upgrade";
      
      // Map SQL exceptions to error codes
      if (errorMessage.includes("Invalid building key")) {
        return res.status(400).json({ success: false, code: "BASE_BUILDING_NOT_FOUND", message: errorMessage });
      }
      if (errorMessage.includes("max level")) {
        return res.status(400).json({ success: false, code: "BASE_BUILDING_MAX_LEVEL", message: errorMessage });
      }
      if (errorMessage.includes("Insufficient resources")) {
        return res.status(400).json({ success: false, code: "BASE_INSUFFICIENT_RESOURCES", message: errorMessage });
      }
      
      return res.status(400).json({ success: false, code: "BASE_BUILD_FAILED", message: errorMessage });
    }

    const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
    if (!result) {
      return res.status(400).json({ success: false, code: "BASE_BUILD_FAILED", message: "RPC returned no data" });
    }
    ...
    logError(error, { event: EVENTS.BASE_BUILD_FAIL, deviceId });
    return res.status(500).json({ success: false, code: "BASE_BUILD_INTERNAL_ERROR", message: "Build action failed" });
  }
}
