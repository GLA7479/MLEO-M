import { getArcadeDevice } from "../../../../lib/server/arcadeDeviceCookie";
import { checkArcadeRateLimit } from "../../../../lib/server/arcadeRateLimit";
import { getSupabaseAdmin } from "../../../../lib/server/supabaseAdmin";
import { checkIpRateLimit } from "../../../../lib/server/ipRateLimit";
import { validateCsrfToken } from "../../../../lib/server/csrf";
import {
  logCsrfFailure,
  logIpRateLimitExceeded,
} from "../../../../lib/server/securityLogger";

export default async function handler(req, res) {
  if (req.method !== "POST") {
    res.setHeader("Allow", "POST");
    return res.status(405).json({
      success: false,
      code: "METHOD_NOT_ALLOWED",
      message: "Method not allowed",
    });
  }

  try {
    if (!validateCsrfToken(req)) {
      logCsrfFailure(req);
      return res.status(403).json({
        success: false,
        code: "CSRF_INVALID",
        message: "Invalid CSRF token",
      });
    }

    const ipRate = await checkIpRateLimit(req, 60, 60_000);
    if (!ipRate.allowed) {
      logIpRateLimitExceeded(req, 60);
      return res.status(429).json({
        success: false,
        code: "RATE_LIMIT_IP",
        message: "Too many requests from this IP",
      });
    }

    const deviceId = getArcadeDevice(req);
    if (!deviceId) {
      return res.status(401).json({
        success: false,
        code: "DEVICE_NOT_INITIALIZED",
        message: "Device not initialized",
      });
    }

    const rateLimit = await checkArcadeRateLimit(
      "base-action-mission-claim",
      deviceId,
      20,
      60_000
    );
    if (!rateLimit.allowed) {
      return res.status(429).json({
        success: false,
        code: "RATE_LIMIT_DEVICE",
        message: "Too many mission claim requests",
      });
    }

    const { mission_key } = req.body || {};
    const missionKey = String(mission_key || "").trim();
    if (!missionKey) {
      return res.status(400).json({
        success: false,
        code: "BASE_INVALID_MISSION_KEY",
        message: "Missing or invalid mission_key",
      });
    }

    const supabase = getSupabaseAdmin();

    const { data, error } = await supabase.rpc("base_claim_mission_reward", {
      p_device_id: deviceId,
      p_mission_key: missionKey,
    });

    if (error) {
      return res.status(400).json({
        success: false,
        code: "BASE_MISSION_CLAIM_FAILED",
        message: error.message || "Failed to claim mission reward",
      });
    }

    const row = Array.isArray(data) ? data[0] : data;
    const state = row?.state || row || null;

    if (!state) {
      return res.status(400).json({
        success: false,
        code: "BASE_MISSION_CLAIM_FAILED",
        message: "RPC returned no data",
      });
    }

    return res.status(200).json({
      success: true,
      state,
      mission_key: missionKey,
    });
  } catch (error) {
    console.error("base/action/mission-claim failed", error);
    return res.status(500).json({
      success: false,
      code: "BASE_MISSION_CLAIM_INTERNAL_ERROR",
      message: "Mission claim action failed",
    });
  }
}
