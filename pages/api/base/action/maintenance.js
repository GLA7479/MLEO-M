...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rateLimit = await checkArcadeRateLimit("base-action-maintenance", deviceId, 20, 60_000);
    if (!rateLimit.allowed) {
      return res.status(429).json({ success: false, code: "RATE_LIMIT_DEVICE", message: "Too many maintenance requests" });
    }
    ...
    if (!canAfford(resources, MAINTENANCE_COST)) {
      return res.status(400).json({ success: false, code: "BASE_INSUFFICIENT_RESOURCES", message: "Need GOLD, SCRAP and DATA for maintenance" });
    }
    ...
    return res.status(500).json({ success: false, code: "BASE_MAINTENANCE_INTERNAL_ERROR", message: "Maintenance action failed" });
  }
}
