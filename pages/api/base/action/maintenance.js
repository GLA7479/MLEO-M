...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return res.status(429).json({ success: false, message: "Too many maintenance requests" });
    }
    ...
    if (!canAfford(resources, MAINTENANCE_COST)) {
      return res.status(400).json({ success: false, message: "Need GOLD, SCRAP and DATA for maintenance" });
    }
    ...
    return res.status(500).json({ success: false, message: "Maintenance action failed" });
  }
}
