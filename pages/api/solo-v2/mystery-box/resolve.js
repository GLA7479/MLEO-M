...
function entryCostFromSessionRow(sessionRow) {
  ...
  return stake >= MYSTERY_BOX_MIN_WAGER ? stake : QUICK_FLIP_CONFIG.entryCost;
}
...
function resolveWinningBoxIndex() {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sessionRead.error) {
      if (isMissingTable(sessionRead.error)) {
        return res.status(503).json({
          ...
          message: "Solo V2 Mystery Box resolve is not migrated yet.",
        });
      }
      return res.status(503).json({
        ok: false,
        ...
        status: "unavailable",
        message: "Mystery Box resolve is temporarily unavailable.",
      });
    }
    ...
    if (sessionRow.game_key !== "mystery_box") {
      return res.status(400).json({
        ok: false,
        category: "validation_error",
        status: "invalid_game",
        message: "Session game must be mystery_box",
      });
    }
    ...
    if (!snapshotResult.ok) {
      ...
      return res.status(503).json({
        ...
        category: "unavailable",
        status: "unavailable",
        message: "Pick lookup is temporarily unavailable.",
      });
    }

    const snapshot = snapshotResult.snapshot;
    const choice = snapshot.boxChoice;
    if (choice === null || choice === undefined) {
      return res.status(409).json({
        ok: false,
        category: "conflict",
        status: "choice_required",
        message: "No submitted mystery box pick found for this session.",
      });
    }

    const outcome = resolveWinningBoxIndex();
    const isWin = choice === outcome;
    const resolvedAt = new Date().toISOString();
    const entryCost = entryCostFromSessionRow(sessionRow);
    const fundingSource = fundingSourceFromSessionRow(sessionRow);
    const resolvedSummary = {
      phase: "mystery_box_resolved",
      choice,
      outcome,
      isWin,
      resolvedAt,
      settlementSummary: buildMysteryBoxSettlementSummary({ choice, outcome, isWin, entryCost, fundingSource }),
      stats: "deferred",
    };
    ...
    console.error("solo-v2/mystery-box/resolve failed", error);
    return res.status(500).json({
      ok: false,
      category: "unexpected_error",
      status: "server_error",
      message: "Mystery Box resolve failed",
    });
  }
}
