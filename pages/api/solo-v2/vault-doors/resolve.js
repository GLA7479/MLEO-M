...
import {
  ...
  parseVaultDoorsActiveSummary,
} from "../../../../lib/solo-v2/server/vaultDoorsSnapshot";
import { isTrapAtStage } from "../../../../lib/solo-v2/server/vaultDoorsEngine";
import {
...
function topLadderMultiplier() {
  return VAULT_DOORS_MULTIPLIER_LADDER[VAULT_DOORS_MULTIPLIER_LADDER.length - 1];
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sessionRead.error) {
      if (isMissingTable(sessionRead.error)) {
        return res.status(503).json({
          ...
          message: "Solo V2 Vault Doors resolve is not migrated yet.",
        });
      }
      return res.status(503).json({
        ok: false,
        ...
        status: "unavailable",
        message: "Vault Doors resolve is temporarily unavailable.",
      });
    }
    ...
    if (sessionRow.game_key !== "vault_doors") {
      return res.status(400).json({
        ok: false,
        category: "validation_error",
        status: "invalid_game",
        message: "Session game must be vault_doors",
      });
    }
    ...
    if (isCashOut) {
      ...
      const activeCash = parseVaultDoorsActiveSummary(sessionRow);
      if (!activeCash) {
        return res.status(409).json({
          ok: false,
          category: "conflict",
          status: "invalid_session_state",
          message: "Session has no active Vault Doors state.",
        });
      }

      const clearedLen = activeCash.clearedRows.length;
      const securedMultiplier =
        clearedLen > 0 ? VAULT_DOORS_MULTIPLIER_LADDER[clearedLen - 1] : 1;
      ...
      if (sessionUpdate.error) {
        return res.status(503).json({
          ...
          category: "unavailable",
          status: "unavailable",
          message: "Vault Doors cash out update failed.",
        });
      }
    ...
    const activeEarly = parseVaultDoorsActiveSummary(sessionRow);
    ...
    if (!Number.isFinite(pickEventId) || pickEventId <= 0 || rowIndex < 0 || rowIndex >= VAULT_DOORS_STAGE_COUNT) {
      return res.status(409).json({
        ok: false,
        category: "conflict",
        status: "choice_required",
    ...
    const trapHit = isTrapAtStage(active.trapDoors, rowIndex, column);
    if (trapHit === null) {
      return res.status(409).json({
        ok: false,
        category: "conflict",
    ...
    if (trapHit) {
      ...
      const bombCol = active.trapDoors[rowIndex];
      const resolvedSummary = {
        phase: "vault_doors_resolved",
        terminalKind: "bomb",
        payoutReturn,
    ...
    const securedMultiplier = VAULT_DOORS_MULTIPLIER_LADDER[rowIndex];
    ...
    const isFinalRow = rowIndex >= VAULT_DOORS_STAGE_COUNT - 1;
    if (isFinalRow) {
      ...
      if (sessionUpdate.error || !sessionUpdate.data) {
        ...
        return res.status(503).json({
          ...
          category: "unavailable",
          status: "unavailable",
          message: "Vault Doors full clear update failed.",
        });
      }
    ...
    if (sessionUpdate.error || !sessionUpdate.data) {
      return res.status(503).json({
        ...
        category: "unavailable",
        status: "unavailable",
        message: "Vault Doors turn update failed.",
      });
    }
    ...
    console.error("solo-v2/vault-doors/resolve failed", error);
    return res.status(500).json({
      ok: false,
      category: "unexpected_error",
      status: "server_error",
      message: "Vault Doors resolve failed",
    });
  }
}
