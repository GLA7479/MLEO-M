...
import {
  ...
  parseCrystalPathActiveSummary,
} from "../../../../lib/solo-v2/server/crystalPathSnapshot";
import { isPickSafe } from "../../../../lib/solo-v2/server/crystalPathEngine";
import {
...
function createTerminalResolvedPayload(sessionRow) {
  ...
  const safeLayout = Array.isArray(summary.safeColumns) ? summary.safeColumns : null;
  return {
...
function topLadderMultiplier() {
  return CRYSTAL_PATH_MULTIPLIER_LADDER[CRYSTAL_PATH_MULTIPLIER_LADDER.length - 1];
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sessionRead.error) {
      if (isMissingTable(sessionRead.error)) {
        return res.status(503).json({
          ...
          message: "Solo V2 Crystal Path resolve is not migrated yet.",
        });
      }
      return res.status(503).json({
        ok: false,
        ...
        status: "unavailable",
        message: "Crystal Path resolve is temporarily unavailable.",
      });
    }
    ...
    if (sessionRow.game_key !== "crystal_path") {
      return res.status(400).json({
        ok: false,
        category: "validation_error",
        status: "invalid_game",
        message: "Session game must be crystal_path",
      });
    }
    ...
    if (isCashOut) {
      ...
      const activeCash = parseCrystalPathActiveSummary(sessionRow);
      if (!activeCash) {
        return res.status(409).json({
          ok: false,
          category: "conflict",
          status: "invalid_session_state",
          message: "Session has no active Crystal Path state.",
        });
      }

      const clearedLen = activeCash.clearedRows.length;
      const securedMultiplier =
        clearedLen > 0 ? CRYSTAL_PATH_MULTIPLIER_LADDER[clearedLen - 1] : 1;
      ...
      if (sessionUpdate.error) {
        return res.status(503).json({
          ...
          category: "unavailable",
          status: "unavailable",
          message: "Crystal Path cash out update failed.",
        });
      }
    ...
    const activeEarly = parseCrystalPathActiveSummary(sessionRow);
    ...
    if (!snap0.canResolveTurn || !snap0.pendingPick) {
      return res.status(409).json({
        ...
        message: "No pending path pick to resolve for this session.",
      });
    }
    ...
    if (!Number.isFinite(pickEventId) || pickEventId <= 0 || rowIndex < 0 || rowIndex >= CRYSTAL_PATH_ROW_COUNT) {
      return res.status(409).json({
        ok: false,
        category: "conflict",
        status: "choice_required",
    ...
    const pickSafe = isPickSafe(active.safeColumns, rowIndex, column);
    if (pickSafe === null) {
      return res.status(409).json({
        ok: false,
        category: "conflict",
        status: "invalid_session_state",
        message: "Invalid sealed path layout.",
      });
    }
    ...
    if (!pickSafe) {
      ...
      const resolvedSummary = {
        ...
        bombColumn: column,
        safeColumns: active.safeColumns,
    ...
    const securedMultiplier = CRYSTAL_PATH_MULTIPLIER_LADDER[rowIndex];
    ...
    const isFinalRow = rowIndex >= CRYSTAL_PATH_ROW_COUNT - 1;
    if (isFinalRow) {
      ...
      if (sessionUpdate.error || !sessionUpdate.data) {
        ...
        return res.status(503).json({
          ...
          category: "unavailable",
          status: "unavailable",
          message: "Crystal Path full clear update failed.",
        });
      }
    ...
    if (sessionUpdate.error || !sessionUpdate.data) {
      return res.status(503).json({
        ...
        category: "unavailable",
        status: "unavailable",
        message: "Crystal Path turn update failed.",
      });
    }
    ...
    console.error("solo-v2/crystal-path/resolve failed", error);
    return res.status(500).json({
      ok: false,
      category: "unexpected_error",
      status: "server_error",
      message: "Crystal Path resolve failed",
    });
  }
}
