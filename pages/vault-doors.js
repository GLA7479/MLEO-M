...
import VaultDoorsBoard from "../components/solo-v2/VaultDoorsBoard";
...
import {
  ...
  payoutForMultiplier,
} from "../lib/solo-v2/vaultDoorsConfig";
import {
  applyVaultDoorsSettlementOnce,
...
const GAME_KEY = "vault_doors";
const PLAYER_HEADER = "vault-doors-client";
...
const STATS_KEY = "solo_v2_vault_doors_stats_v1";
...
function writeVaultDoorsStats(next) {
...
function vaultStripModel(rowCount, uiState, nCleared, terminalKind) {
...
function VaultDoorsGameplayPanel({
  session,
  uiState,
  pulseCell,
  shakeCell,
  ...
  const vd = session?.vaultDoors;
  const playing = vd?.playing;
  const rr = vd?.resolvedResult;
...
export default function VaultDoorsPage() {
  const [vaultBalance, setVaultBalance] = useState(0);
  const [vaultReady, setVaultReady] = useState(false);
  const [wagerInput, setWagerInput] = useState(String(VAULT_DOORS_MIN_WAGER));
  ...
  const [stats, setStats] = useState(readVaultDoorsStats);
  ...
  useEffect(() => {
    ...
    applyVaultDoorsSettlementOnce(sessionId, settlementSummary).then(settlementResult => {
  ...
  function applySessionReadState(sessionPayload, { resumed = false } = {}) {
    const vd = sessionPayload?.vaultDoors;
    const readState = String(vd?.readState || sessionPayload?.readState || "");
    const st = String(sessionPayload?.sessionStatus || "");

    if (st === "resolved" && vd?.resolvedResult) {
      setResolvedResult({
        ...vd.resolvedResult,
        sessionId: sessionPayload.id,
        settlementSummary: vd.resolvedResult.settlementSummary,
      });
    ...
    if (readState === "choice_required" || readState === "ready") {
      setResolvedResult(null);
      setUiState(UI_STATE.SESSION_ACTIVE);
      setSessionNotice(resumed ? "Resumed active run." : "Pick a safe door on the current row.");
      setErrorMessage("");
      return;
    }
  ...
  async function postResolve(sessionId, body, activeCycle) {
    const response = await fetch("/api/solo-v2/vault-doors/resolve", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-solo-v2-player": PLAYER_HEADER,
  ...
  async function handleDigColumn(col) {
      ...
      const playing = sessionRef.current?.vaultDoors?.playing;
  ...
  async function runStartRun() {
    ...
    if (!isGiftRound && wager < VAULT_DOORS_MIN_WAGER) return;
    ...
    const vdBoot = boot.session?.vaultDoors;
    if (vdBoot?.readState === "choice_submitted") {
  ...
  useEffect(() => {
    const sid = session?.id;
    const vdResume = session?.vaultDoors;
    if (!sid || !vdResume || uiState !== UI_STATE.SESSION_ACTIVE) return;
    if (vdResume.readState !== "choice_submitted" || !vdResume.canResolveTurn) return;
  ...
  }, [session?.id, session?.vaultDoors?.readState, session?.vaultDoors?.canResolveTurn, uiState]);

  const numericWager = parseWagerInput(wagerInput);
  const wagerPlayable =
    vaultReady && numericWager >= VAULT_DOORS_MIN_WAGER && vaultBalance >= numericWager;
  ...
  const stakeExceedsVault =
    vaultReady &&
    idleLike &&
    numericWager >= VAULT_DOORS_MIN_WAGER &&
  ...
  const vd = session?.vaultDoors;
  const playing = vd?.playing;
  const readState = String(vd?.readState || "");

  const runEntryFromSession =
    session != null &&
    Number(session.entryAmount) >= VAULT_DOORS_MIN_WAGER &&
  ...
  const firstStepWinPreview = payoutForMultiplier(
    Math.max(VAULT_DOORS_MIN_WAGER, numericWager),
    VAULT_DOORS_MULTIPLIER_LADDER[0],
  );
  ...
  const rrSnap = vd?.resolvedResult;
  ...
  const strip = vaultStripModel(rowTotal, uiState, nCleared, stripTerminalKind);
  ...
  if (terminalKind === "bomb") resultTitle = "Trap — run lost";
  ...
  if (terminalKind === "bomb") {
    ...
    popupLine2 = Number.isFinite(fr) ? `Trap · stage ${fr + 1}` : "Trap hit";
  ...
  return (
    <SoloV2GameShell
      title="Vault Doors"
      subtitle="Pick doors. Climb safely."
      ...
      soloV2Footer={{
        ...
        onDecreaseAmount: () => {
          ...
          setWagerInput(prev => {
            ...
            return String(Math.min(MAX_WAGER, Math.max(0, c - VAULT_DOORS_MIN_WAGER)));
          });
        },
        onIncreaseAmount: () => {
          clearPresetChain();
        ...
        onResetAmount: () => {
          clearPresetChain();
          setWagerInput(String(VAULT_DOORS_MIN_WAGER));
        },
      ...
      gameplaySlot={
        <VaultDoorsGameplayPanel
          ...
          stepLabels={stepLabels}
          showBoardCashOut={
            uiState === UI_STATE.SESSION_ACTIVE && !terminalSession && Boolean(vd?.canCashOut)
          }
      ...
      helpContent={
        <div className="space-y-2">
          <p>
            Vault Doors is a six-stage ladder: each stage has three doors, and exactly one is a trap. The server seals
            trap placement before you pick. A safe door advances you and raises your secured payout; the trap ends the run.
          </p>
          <p>
            After any safe stage you may cash out from the lower band under the grid and above the stake bar. Secured
