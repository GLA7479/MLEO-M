...
import {
  ...
  requestOv2MeldMatchDeclareFinish,
  requestOv2MeldMatchDiscard,
  requestOv2MeldMatchDraw,
  requestOv2MeldMatchMarkTurnTimeout,
  requestOv2MeldMatchOfferDouble,
  requestOv2MeldMatchRequestRematch,
  requestOv2MeldMatchCancelRematch,
  requestOv2MeldMatchResolveLayoff,
  requestOv2MeldMatchRespondDouble,
  requestOv2MeldMatchStartNextMatch,
...
export function useOv2MeldMatchSession(baseContext) {
  const preview = useOv2UiPreviewOptional("meldmatch");
  ...
  useEffect(() => {
    if (!roomId || roomProductId !== OV2_MELDMATCH_PRODUCT_GAME_ID) {
      setSnap(null);
      return undefined;
    }
    let cancelled = false;
    void (async () => {
      const s = await fetchOv2MeldMatchSnapshot(roomId, { participantKey: selfKey ?? "" });
      if (!cancelled) setSnap(prev => ov2PreferNewerSnapshot(prev, s ?? null));
    })();
    const unsub = subscribeOv2MeldMatchSnapshot(roomId, {
  ...
  useEffect(() => {
    if (!roomId || !selfKey || roomProductId !== OV2_MELDMATCH_PRODUCT_GAME_ID) return undefined;
    const s = snap;
    const phase = s ? String(s.phase || "").toLowerCase() : "";
    if (phase !== "playing" && phase !== "layoff") return undefined;
    ...
    const pd = phase === "playing" && s.pendingDouble && typeof s.pendingDouble === "object" ? s.pendingDouble : null;
    ...
    const turnKey = `${sid}|${dl}|${dlSeat}|${pd ? "dbl" : phase === "layoff" ? "lay" : "mv"}`;
  ...
  const draw = useCallback(
    async source => {
      ...
      setErr("");
      try {
        const r = await requestOv2MeldMatchDraw(roomId, selfKey, source, { revision: snap.revision });
        if (!r.ok) {
  ...
  const discard = useCallback(
    async cardId => {
      ...
      setErr("");
      try {
        const r = await requestOv2MeldMatchDiscard(roomId, selfKey, cardId, { revision: snap.revision });
        if (!r.ok) {
  ...
  const declareFinish = useCallback(
    async payload => {
      if (!roomId || !selfKey || !snap) return { ok: false };
      if (busy) return { ok: false };
      setBusy(true);
      setErr("");
      try {
        const r = await requestOv2MeldMatchDeclareFinish(roomId, selfKey, payload, { revision: snap.revision });
        if (!r.ok) {
          setErr(r.error || "Finish failed");
          return { ok: false };
        }
        if (r.snapshot) setSnap(prev => ov2PreferNewerSnapshot(prev, r.snapshot));
        return { ok: true };
  ...
  const resolveLayoff = useCallback(
    async assignments => {
      if (!roomId || !selfKey || !snap) return { ok: false };
      if (busy) return { ok: false };
      setBusy(true);
      setErr("");
      try {
        const r = await requestOv2MeldMatchResolveLayoff(roomId, selfKey, assignments, { revision: snap.revision });
        if (!r.ok) {
          setErr(r.error || "Layoff failed");
          return { ok: false };
        }
        if (r.snapshot) setSnap(prev => ov2PreferNewerSnapshot(prev, r.snapshot));
        return { ok: true };
  ...
  const offerDouble = useCallback(async () => {
    ...
    setErr("");
    try {
      const r = await requestOv2MeldMatchOfferDouble(roomId, selfKey, { revision: snap.revision });
      if (!r.ok) {
  ...
  const respondDouble = useCallback(
    async accept => {
      ...
      setErr("");
      try {
        const r = await requestOv2MeldMatchRespondDouble(roomId, selfKey, accept, { revision: snap.revision });
        if (!r.ok) {
  ...
  const requestRematch = useCallback(async () => {
    ...
    return requestOv2MeldMatchRequestRematch(roomId, selfKey);
  }, [roomId, selfKey]);

  const cancelRematch = useCallback(async () => {
    if (!roomId || !selfKey) return { ok: false };
    return requestOv2MeldMatchCancelRematch(roomId, selfKey);
  }, [roomId, selfKey]);

  const startNextMatch = useCallback(
    async expectedMatchSeq => {
      if (!roomId || !selfKey) return { ok: false };
      return requestOv2MeldMatchStartNextMatch(roomId, selfKey, expectedMatchSeq);
    },
    [roomId, selfKey]
  );
  ...
  const vm = useMemo(() => {
    ...
    const turnTimeLeftMs =
      (phase === "playing" || phase === "layoff") && turnDeadline != null ? Math.max(0, turnDeadline - nowMs) : null;
    const turnTimeLeftSec = turnTimeLeftMs != null ? Math.ceil(turnTimeLeftMs / 1000) : null;
    let layoffMelds = null;
    if (Array.isArray(snap?.layoffMelds)) {
      layoffMelds = snap.layoffMelds;
    }
    return {
      phase,
      ...
      discardTop: snap?.discardTop ?? null,
      handSizes: snap?.handSizes ?? [0, 0],
      myHand: Array.isArray(snap?.myHand) ? snap.myHand : [],
      opponentHandRevealed: Array.isArray(snap?.opponentHandRevealed) ? snap.opponentHandRevealed : [],
      opponentHandCount: snap?.opponentHandCount ?? null,
      layoffCloserSeat: snap?.layoffCloserSeat ?? null,
      layoffCloserDeadwood: snap?.layoffCloserDeadwood ?? null,
      layoffMelds,
