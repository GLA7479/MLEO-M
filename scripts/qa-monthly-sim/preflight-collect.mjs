#!/usr/bin/env node
/**
 * Gate 3.5 preflight metrics collector (read-only).
 * Usage: node scripts/qa-monthly-sim/preflight-collect.mjs --run-id=<uuid>
 */
import fs from "fs";
...
export async function collectPreflightReport(runId, extra = {}) {
  const db = getQaDb();
  ...
  const report = {
    ...
    paths: {
      ...
      report: path.join(REPORTS, `preflight-report-${runId}.json`),
    },
...
async function main() {
  ...
  if (!runId) {
    console.error("Usage: preflight-collect.mjs --run-id=<uuid>");
    process.exit(1);
  }
  const { report, outPath } = await collectPreflightReport(runId);
  console.log(JSON.stringify({ ok: true, reportPath: outPath, report }, null, 2));
}

if (process.argv[1]?.replace(/\\/g, "/").endsWith("preflight-collect.mjs")) {
  main().catch(e => {
    console.error(e);
    process.exit(1);
  });
