// ============================================================================
// MLEO Gold Rush Digger - Full-Screen Game Template (No-Scroll Auto-Scale)
// Dig for treasures, avoid the skulls!
// ============================================================================
...
function generateMap() {
  ...
  for (let i = 0; i < PRIZES.grandPrize.count; i++) map.push('grandPrize');
...
export default function GoldRushPage() {
  ...
  useEffect(() => {
    ...
    const gameId = router.pathname.replace('/', '') || 'goldrush';
...
useEffect(() => {
  ...
  const calc = () => {
    ...
    const gridWrap = wrapRef.current.querySelector("#goldrush-grid-wrap");
  ...
  const startGame = async (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      const startResult = await startPaidArcadeSession("goldrush", play);
  ...
  const dig = (index) => {
    ...
    if (prize === 'skull') {
      ...
      const prizeAmount = Math.floor(Number(playAmount) * prizeData.multiplier);
      const newTotal = totalPrize + prizeAmount;
  ...
  const endGame = async (cashout, treasures) => {
    ...
    const foundItems = dug
      .map((index) => map[index])
      .filter((item) => item && item !== "skull");
    try {
      ...
      const resolvedTreasures = Number(payload.treasures || treasures);
      ...
      const newStats = {
        ...
        treasuresFound: stats.treasuresFound + resolvedTreasures,
        lastPlay: play,
      };
      setStats(newStats);
    } catch (error) {
...
>
                  ...
                  <button
                    ...
                    className={`text-xl font-bold transition-all ${
                      isDug
                        ...
                        : "bg-amber-600/30 text-white hover:bg-amber-600/50"
                    } disabled:opacity-50`}
                  >
                    {isDug ? prizeData?.emoji : '⛏️'}
                  </button>
                );
              })}
            </div>
          ...
          >
            <button
              ...
              disabled={(gameActive && dug.length === 0) || (gameResult && !gameResult)}
              className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-amber-500 to-yellow-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {gameActive
                ? `💰 CASH OUT ${fmt(totalPrize)}`
                : gameResult
                ? "PLAY AGAIN"
                : "START"}
            </button>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            ...
            >
              ...
              <div className="text-2xl font-bold mb-1">
                {gameResult.win ? "CASHED OUT!" : "SKULL!"}
              </div>
              ...
              <div className="text-sm opacity-80 mt-2">
                Treasures Found: {gameResult.treasures}
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4">
            <div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto">
              ...
              <div className="space-y-3 text-sm">
                ...
                <p>
                  <strong>2. Start Digging:</strong> Click cells to dig for treasures!
                </p>
                <p>
                  <strong>3. Avoid Skulls:</strong> Hit a skull and you lose!
                </p>
                <p>
            ...
            <div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-black/30 border border-white/10 rounded-lg p-3">
                    <div className="text-xs text-white/60">Treasures</div>
