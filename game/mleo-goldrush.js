// ============================================================================
// MLEO Gold Rush Digger - Full-Screen Game Template
// Dig for gold and treasures!
// ============================================================================
...
export default function GoldRushPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalGames: 0, totalBet: 0, totalWon: 0, biggestWin: 0, treasuresFound: 0, lastBet: MIN_BET }));
  ...
  const startGame = (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      if (result.success) { bet = result.amount; setIsFreePlay(false); router.replace('/goldrush', undefined, { shallow: true }); }
  ...
  const dig = (index) => {
    if (!gameActive || dug.includes(index)) return;
    playSfx(clickSound.current);
    const newDug = [...dug, index];
    setDug(newDug);
    const prize = map[index];
    const prizeData = PRIZES[prize];

    if (prize === 'skull') {
      endGame(false, newDug.length - 1);
    } else {
      const bet = Number(betAmount);
      const addPrize = Math.floor(bet * prizeData.multiplier);
      const newTotal = totalPrize + addPrize;
      setTotalPrize(newTotal);
    }
  };

  const cashOut = () => {
    if (!gameActive || dug.length === 0) return;
  ...
  const endGame = (cashout, treasures) => {
    ...
    const resultData = { win, treasures, prize, profit: win ? prize - bet : -bet };
    setGameResult(resultData);
    setGameActive(false);
    
    const newStats = { ...stats, totalGames: stats.totalGames + 1, totalBet: stats.totalBet + bet, totalWon: win ? stats.totalWon + prize : stats.totalWon, biggestWin: Math.max(stats.biggestWin, win ? prize : 0), treasuresFound: stats.treasuresFound + treasures, lastBet: bet };
    setStats(newStats);
  };

  const resetGame = () => { setGameResult(null); setShowResultPopup(false); setMap([]); setDug([]); setTotalPrize(0); setGameActive(false); };
  const backSafe = () => { playSfx(clickSound.current); router.push('/arcade'); };

  if (!mounted) return <div className="min-h-screen bg-gradient-to-br from-yellow-900 via-black to-amber-900 flex items-center justify-center"><div className="text-white text-xl">Loading...</div></div>;

  const potentialWin = Math.floor(Number(betAmount) * 10);

  return (
    <Layout>
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-yellow-900 via-black to-amber-900" style={{ height: 'var(--app-100vh, 100vh)' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-4 pb-16 pt-14 overflow-y-auto" style={{ minHeight: '100%' }}>
          <div className="text-center mb-3"><h1 className="text-3xl md:text-4xl font-extrabold text-white mb-1">⛏️ Gold Rush</h1><p className="text-white/70 text-sm">Dig for gold • Avoid skulls!</p></div>
          <div className="grid grid-cols-3 gap-2 mb-3 w-full max-w-md">
            ...
            <div className="bg-black/30 border border-white/10 rounded-lg p-3 text-center"><div className="text-xs text-white/60 mb-1">Prize</div><div className="text-lg font-bold text-green-400">{fmt(totalPrize)}</div></div>
          </div>

          <div className="mb-2 text-center text-xs text-white/70">Dug: {dug.length}/25 • Current: {fmt(totalPrize)} MLEO</div>

          <div className="grid grid-cols-5 gap-1 mb-2 w-full max-w-xs" style={{ minHeight: '200px' }}>
            {[...Array(GRID_SIZE)].map((_, i) => {
              const isDug = dug.includes(i);
              const prize = isDug ? map[i] : null;
              const prizeData = prize ? PRIZES[prize] : null;
              return (<button key={i} onClick={() => dig(i)} disabled={!gameActive || isDug} className={`h-12 rounded text-xl font-bold transition-all ${isDug ? (prize === 'skull' ? 'bg-red-500 text-white' : 'bg-yellow-500 text-black') : 'bg-amber-600/30 text-white hover:bg-amber-600/50'} disabled:opacity-50`}>{isDug ? prizeData?.emoji : '⛏️'}</button>);
            })}
            </div>
          ...
          <div className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: '140px' }}>
            <button onClick={gameActive ? cashOut : (gameResult ? resetGame : () => startGame(false))} disabled={(gameActive && dug.length === 0) || (!gameActive && !gameResult && false)} className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-yellow-500 to-amber-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50">{gameActive ? `💰 CASH OUT ${fmt(totalPrize)}` : gameResult ? "PLAY AGAIN" : "START"}</button>
