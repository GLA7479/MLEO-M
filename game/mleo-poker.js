...
const PAYOUTS = { "Royal Flush": 1000, "Straight Flush": 200, "Four of a Kind": 50, "Full House": 20, "Flush": 10, "Straight": 8, "Three of a Kind": 5, "Two Pair": 3, "One Pair": 2, "High Card": 1 };
...
function evaluateHand(cards) {
  if (cards.length !== 5) return { hand: "High Card", rank: 1 };
  ...
  if (isFlush && isStraight && values[0] === 14) return { hand: "Royal Flush", rank: 10 };
  if (isFlush && isStraight) return { hand: "Straight Flush", rank: 9 };
  if (countsArray[0] === 4) return { hand: "Four of a Kind", rank: 8 };
  if (countsArray[0] === 3 && countsArray[1] === 2) return { hand: "Full House", rank: 7 };
  if (isFlush) return { hand: "Flush", rank: 6 };
  if (isStraight) return { hand: "Straight", rank: 5 };
  if (countsArray[0] === 3) return { hand: "Three of a Kind", rank: 4 };
  if (countsArray[0] === 2 && countsArray[1] === 2) return { hand: "Two Pair", rank: 3 };
  if (countsArray[0] === 2) return { hand: "One Pair", rank: 2 };
...
export default function PokerPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalHands: 0, wins: 0, losses: 0, totalBet: 0, totalWon: 0, biggestWin: 0, royalFlushes: 0, lastBet: MIN_BET }));
  ...
  const dealHand = (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      if (result.success) { bet = result.amount; setIsFreePlay(false); router.replace('/poker', undefined, { shallow: true }); }
  ...
  const potentialWin = Math.floor(Number(betAmount) * 1000);

  return (
    <Layout>
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-green-900 via-black to-blue-900" style={{ height: 'var(--app-100vh, 100vh)' }}>
        ...
        <div className="absolute top-0 left-0 right-0 z-50 pointer-events-none">
          <div className="relative px-2 py-3">
            <div className="absolute left-2 top-2 flex gap-2 pointer-events-auto">
              ...
              {freePlayTokens > 0 && (<button onClick={() => dealHand(true)} className="relative px-2 py-1 rounded-lg bg-amber-500/20 border border-amber-500/40 hover:bg-amber-500/30 transition-all" title={`${freePlayTokens} Free Play${freePlayTokens > 1 ? 's' : ''} Available`}><span className="text-base">🎁</span><span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">{freePlayTokens}</span></button>)}
            </div>
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-4 pb-16 pt-14 overflow-y-auto" style={{ minHeight: '100%' }}>
          <div className="text-center mb-3"><h1 className="text-3xl md:text-4xl font-extrabold text-white mb-1">🃏 Poker</h1><p className="text-white/70 text-sm">Texas Hold'em • Best hand wins!</p></div>
