...
export default function BombPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalGames: 0, wins: 0, losses: 0, totalBet: 0, totalWon: 0, biggestWin: 0, perfectDefuses: 0, lastBet: MIN_BET }));
  ...
  const startGame = (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      if (result.success) { bet = result.amount; setIsFreePlay(false); router.replace('/bomb', undefined, { shallow: true }); }
  ...
  const endGame = (win, finalLevel) => {
    ...
    const resultData = { win, level: finalLevel, multiplier, prize, profit: win ? prize - bet : -bet, perfect: finalLevel >= TOTAL_LEVELS };
    setGameResult(resultData);
    setGameActive(false);
    
    const newStats = { ...stats, totalGames: stats.totalGames + 1, wins: win ? stats.wins + 1 : stats.wins, losses: win ? stats.losses : stats.losses + 1, totalBet: stats.totalBet + bet, totalWon: win ? stats.totalWon + prize : stats.totalWon, biggestWin: Math.max(stats.biggestWin, win ? prize : 0), perfectDefuses: resultData.perfect ? stats.perfectDefuses + 1 : stats.perfectDefuses, lastBet: bet };
    setStats(newStats);
  };
  ...
  return (
    <Layout>
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-red-900 via-black to-orange-900" style={{ height: '100svh' }}>
        ...
        {showHowToPlay && (<div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4"><div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto"><h2 className="text-2xl font-extrabold mb-4">💣 How to Play</h2><div className="space-y-3 text-sm"><p><strong>1. Start Defusing:</strong> Min {MIN_BET} MLEO</p><p><strong>2. Cut Right Wire:</strong> 1 out of 4 wires is correct!</p><p><strong>3. Cash Out or Continue:</strong> Take your prize or risk it for more!</p><div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3"><p className="text-red-300 font-semibold mb-2">💥 Level Prizes:</p><div className="text-xs text-white/80 space-y-1"><p>• <strong>Level 1:</strong> ×2.65</p><p>• <strong>Level 2:</strong> ×4.2</p><p>• <strong>Level 3:</strong> ×6.4</p><p>• <strong>Level 4:</strong> ×10.5</p><p>• <strong>Level 5:</strong> ×20 🏆</p></div></div><div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 mt-2"><p className="text-green-300 font-semibold text-xs">💡 Strategy: Cash out early for safer wins, or risk it all for ×20!</p></div></div><button onClick={() => setShowHowToPlay(false)} className="w-full mt-6 py-3 rounded-lg bg-white/10 hover:bg-white/20 font-bold">Close</button></div></div>)}
