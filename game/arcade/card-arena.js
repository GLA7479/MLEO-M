...
// Classic Card Arena! Best 5-card hand wins!
// ============================================================================
...
function evaluateHand(cards) {
  ...
  if (countsArray[0] === 3 && countsArray[1] === 2) return { hand: "Full Platform", rank: 7 };
...
export default function CardArenaPage() {
  ...
  // Only show potentialWin when game is not active (no gameResult or gameResult is dealing)
  const potentialWin = (!gameResult || gameResult.dealing) ? Math.floor(Number(playAmount) * 1000) : 0;
      ...
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-green-900 via-black to-blue-900" style={{ height: '100svh' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          <div className="text-center mb-1">
            <h1 className="text-2xl font-extrabold text-white mb-0.5">🎴 Card Arena</h1>
          ...
          <div ref={betRef} className="flex items-center justify-center gap-1 mb-1 flex-wrap">
            ...
            <button type="button" onClick={() => { const current = Number(playAmount) || MIN_PLAY; const newBet = Math.min(vault, current + 1000); setPlayAmount(String(newBet)); playSfx(clickSound.current); }} disabled={!!gameResult} className="h-8 w-8 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold text-sm disabled:opacity-50 disabled:pointer-events-none">+</button>
          </div>
        ...
        {showResultPopup && gameResult && !gameResult.dealing && gameResult.hand && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            <div className={`${gameResult.win ? 'bg-green-500' : 'bg-gray-600'} text-white px-8 py-6 rounded-2xl shadow-2xl text-center pointer-events-auto`} style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
              ...
              <div className="text-lg font-bold">{gameResult.win ? `+${fmt(gameResult.prize)} MLEO` : `Hand: ${gameResult.hand}`}</div>
