...
export default function DiamondsPage() {
  ...
  useEffect(() => {
    ...
    const gameId = router.pathname.replace('/', '') || 'diamonds';
...
useEffect(() => {
  ...
  const collectToWallet = async () => {
    ...
    try {
      ...
      const debitResult = await debitSharedVault(wholeCollectAmount, "diamonds-claim");
      if (!debitResult.ok) {
  ...
  const startGame = async (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      const startResult = await startPaidArcadeSession("diamonds", play);
  ...
  const endGame = async (cashout, gems) => {
    if (!sessionId) return;
    const play = Number(playAmount);
    try {
      const finishResult = await finishArcadeSession(sessionId, { cashout, gems, difficulty });
      ...
      const resolvedGems = Number(payload.gems || gems);
      ...
      const newStats = {
        ...
        gemsFound: stats.gemsFound + resolvedGems,
        lastPlay: play,
      };
      setStats(newStats);
    } catch (error) {
...
>
          ...
          >
            <button
              ...
              disabled={(gameActive && revealed.length === 0) || (!gameActive && !gameResult && Number(playAmount) < MIN_PLAY)}
              className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-purple-500 to-pink-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {gameActive
