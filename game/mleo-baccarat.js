...
export default function CardDuelPage() {
  ...
  const checkResult = async (finishResult, play) => {
    ...
    const resultData = { win, playerScore: pScore, bankerScore: bScore, playType: playType.name, prizeMultiplier: Number(payload.multiplier || playType.prize), prize, multiplier: multiplier, profit: win ? prize - play : -play, tie: isTie };
    setGameResult(resultData);

    const newStats = { ...stats, totalGames: stats.totalGames + 1, wins: win ? stats.wins + 1 : stats.wins, losses: win ? stats.losses : stats.losses + 1, totalPlay: stats.totalPlay + play, totalWon: win ? stats.totalWon + prize : stats.totalWon, biggestWin: Math.max(stats.biggestWin, win ? prize : 0), ties: isTie ? stats.ties + 1 : stats.ties, lastPlay: play };
    setStats(newStats);
  };
  ...
  // Only show potentialWin when game is not active (no gameResult and not dealing)
  const potentialWin = (!gameResult && !dealing) ? Math.floor(Number(playAmount) * PLAY_TYPES[selectedPlay].prize) : 0;
      ...
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-purple-900 via-black to-purple-900" style={{ height: '100svh' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          ...
          <div className="mb-1 w-full max-w-md flex flex-col items-center justify-center" style={{ height: "var(--chart-h, 300px)" }}>
            ...
            <div className={`text-base font-bold transition-opacity ${gameResult ? 'opacity-100' : 'opacity-0'} ${gameResult?.win ? 'text-green-400' : gameResult?.tie ? 'text-yellow-400' : 'text-red-400'}`}>{gameResult ? (gameResult.win ? 'YOU WIN!' : gameResult.tie ? 'BOARD: TIE' : 'YOU LOSE') : 'waiting'}</div>
            </div>

            <div className="flex gap-2 mt-4 w-full">
            {Object.entries(PLAY_TYPES).map(([key, play]) => (
              <button key={key} onClick={() => { setSelectedPlay(key); playSfx(clickSound.current); }} disabled={dealing} className={`flex-1 py-2 rounded-lg font-bold text-xs transition-all ${selectedPlay === key ? 'bg-gradient-to-r from-purple-600 to-purple-500 text-white ring-2 ring-purple-300' : 'bg-white/10 text-white hover:bg-white/20'} disabled:opacity-50`}>
          ...
          <div ref={betRef} className="flex items-center justify-center gap-1 mb-1 flex-wrap">
            ...
            <div className="relative">
              ...
              <button onClick={() => { setPlayAmount(String(MIN_PLAY)); setActiveAmountButton("100"); playSfx(clickSound.current); }} disabled={dealing || gameResult} className="absolute right-1 top-1/2 transform -translate-y-1/2 h-6 w-6 rounded bg-red-500/20 hover:bg-red-500/30 text-red-400 font-bold text-xs disabled:opacity-50 flex items-center justify-center" title="Reset to minimum play">↺</button>
            </div>
            <button onClick={() => { const current = Number(playAmount) || MIN_PLAY; const newPlay = Math.min(vault, current + 1000); setPlayAmount(String(newPlay)); playSfx(clickSound.current); }} disabled={dealing || gameResult} className="h-8 w-8 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold text-sm disabled:opacity-50">+</button>
          </div>
        ...
        {showResultPopup && gameResult && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            <div className={`${gameResult.win ? 'bg-green-500' : 'bg-red-500'} text-white px-8 py-6 rounded-2xl shadow-2xl text-center pointer-events-auto`} style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
              ...
              <div className="text-2xl font-bold mb-1">{gameResult.win ? 'YOU WIN!' : gameResult.tie ? 'BOARD TIE' : 'YOU LOSE'}</div>
          ...
          <div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4">
            <div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto">
              ...
              <div className="space-y-3 text-sm">
                <p><strong>1. Select Play:</strong> Player, Banker, or Tie</p>
