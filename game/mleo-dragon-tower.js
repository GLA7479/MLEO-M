...
export default function DragonTowerPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalGames: 0, wins: 0, losses: 0, totalPlay: 0, totalWon: 0, biggestWin: 0, maxFloor: 0, lastPlay: MIN_PLAY }));
  ...
  const collectToWallet = async () => {
    ...
    try {
      ...
      const debitResult = await debitSharedVault(wholeAmount, "dragon-tower");
  ...
  const resetGame = () => { setGameResult(null); setShowResultPopup(false); setFloor(0); setSafeCards([]); setGameActive(false); setSessionId(null); setActiveAmountButton("100"); };
  ...
  const currentPrize = gameActive && floor > 0 ? Math.floor(Number(playAmount) * multipliers[floor - 1]) : 0;
  // Only show potentialWin when game is not active (no gameResult and not gameActive)
  const potentialWin = (!gameResult && !gameActive) ? Math.floor(Number(playAmount) * currentMultiplier) : 0;
      ...
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-purple-900 via-black to-red-900" style={{ height: '100svh' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          <div className="text-center mb-1">
            <h1 className="text-2xl font-extrabold text-white mb-0.5">🐉 Dragon Tower</h1>
            <p className="text-white/70 text-xs">Climb the tower • Avoid dragons!</p>
          </div>
          ...
          <div className="mb-1 w-full max-w-md flex flex-col items-center justify-center" style={{ height: "var(--chart-h, 300px)" }}>
            <div className="mb-2 text-center text-xs text-white/70">Floor {floor + 1}/{TOTAL_FLOORS} • ×{currentMultiplier.toFixed(1)}</div>
            {gameActive && (<div className="flex gap-2 justify-center">{[...Array(CARDS_PER_FLOOR)].map((_, i) => (<button key={i} onClick={() => selectCard(i)} className="w-20 h-24 rounded-lg bg-gradient-to-br from-purple-600 to-purple-800 border-2 border-white/20 text-white text-3xl font-bold hover:brightness-110 transition-all">🃏</button>))}</div>)}
            {!gameActive && (<div className="text-center text-5xl mb-2">🐉</div>)}
            </div>
          ...
          <div ref={ctaRef} className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: '140px' }}>
            <button 
              ...
              disabled={(gameActive && floor === 0) || (gameResult && !gameResult)}
              className={`w-full py-3 rounded-lg font-bold text-base shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                gameActive 
        ...
        {showHowToPlay && (<div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4"><div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto"><h2 className="text-2xl font-extrabold mb-4">🐉 How to Play</h2><div className="space-y-3 text-sm"><p><strong>1. Start:</strong> Min {MIN_PLAY} MLEO</p><p><strong>2. Pick Cards:</strong> Only 1 safe card per floor - choose wisely!</p><p><strong>3. Cash Out or Climb:</strong> Take your prize or risk more!</p><div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-3"><p className="text-purple-300 font-semibold mb-2">🏰 Floor Prizes:</p><div className="text-xs text-white/80 space-y-1"><p>• Floors 1-2: ×1.65, ×2.3</p><p>• Floors 3-4: ×3.1, ×4.5</p><p>• Floors 5-6: ×6.7, ×9.5</p><p>• Floors 7-8: ×14, ×22 🏆</p></div></div><div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 mt-2"><p className="text-green-300 font-semibold text-xs">💡 Strategy: Cash out early for safer wins, or climb to ×22!</p></div></div><button onClick={() => setShowHowToPlay(false)} className="w-full mt-6 py-3 rounded-lg bg-white/10 hover:bg-white/20 font-bold">Close</button></div></div>)}
