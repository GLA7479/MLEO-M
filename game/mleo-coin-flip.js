...
const MIN_PLAY = 100;
const WIN_MULTIPLIER = 1.92; // 50% win chance = RTP 96%
        ...
        >
          ...
          >
            <button
              ...
              disabled={flipping || Number(playAmount) < MIN_PLAY}
              className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <div className={`${gameResult.win ? 'bg-green-500' : 'bg-red-500'} text-white px-8 py-6 rounded-2xl shadow-2xl text-center pointer-events-auto animate-fade-in`} style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
              ...
              {gameResult.win && (
                <div className="text-xs opacity-90 mt-1">
                  Prize: {fmt(gameResult.prize)} MLEO (×{gameResult.multiplier?.toFixed(2) || WIN_MULTIPLIER.toFixed(2)})
                </div>
              )}
              <div className="text-sm opacity-80 mt-2">
                Result: {gameResult.result?.toUpperCase() || 'N/A'}
              </div>
            </div>
          </div>
        )}
