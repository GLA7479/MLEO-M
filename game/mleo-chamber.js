// ============================================================================
// MLEO Mystery Chamber - Full-Screen Game Template
...
export default function ChamberPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalGames: 0, totalPlay: 0, totalWon: 0, biggestWin: 0, chambersOpened: 0, lastPlay: MIN_PLAY }));
  ...
  useEffect(() => {
    ...
    const gameId = router.pathname.replace('/', '') || 'chamber';
  ...
  const collectToWallet = async () => {
    ...
    try {
      ...
      const debitResult = await debitSharedVault(wholeCollectAmount, "chamber-claim");
  ...
  const startGame = async (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      try {
        ...
        if (result.success) { play = result.amount; nextSessionId = result.sessionId; setFreePlayTokens(result.remainingTokens); setIsFreePlay(false); router.replace('/chamber', undefined, { shallow: true }); }
      ...
      const startResult = await startPaidArcadeSession("chamber", play);
  ...
  const endGame = async (cashout, chambers) => {
    ...
    try {
      const finishResult = await finishArcadeSession(sessionId, { cashout, chambers });
      ...
      const resolvedChambers = Number(payload.chambers || chambers);
      ...
      const resultData = { win, chambers: resolvedChambers, prize, profit: win ? prize - play : -play, perfect: Boolean(payload.perfect), multiplier };
      setGameResult(resultData);
      setGameActive(false);
      setSessionId(null);
      
      const newStats = { ...stats, totalGames: stats.totalGames + 1, totalPlay: stats.totalPlay + play, totalWon: win ? stats.totalWon + prize : stats.totalWon, biggestWin: Math.max(stats.biggestWin, win ? prize : 0), chambersOpened: stats.chambersOpened + resolvedChambers, lastPlay: play };
      setStats(newStats);
    } catch (error) {
  ...
  const resetGame = () => { setGameResult(null); setShowResultPopup(false); setSelectedChambers([]); setDangerChamber(null); setGameActive(false); setSessionId(null); };
  ...
  const currentPrize = selectedChambers.length > 0 ? Math.floor(Number(playAmount) * Math.pow(1.13, selectedChambers.length)) : 0;
      ...
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-slate-900 via-black to-slate-900" style={{ height: '100svh' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          <div className="text-center mb-1">
            <h1 className="text-2xl font-extrabold text-white mb-0.5">🔫 Mystery Chamber</h1>
            <p className="text-white/70 text-xs">6 chambers • 1 danger • Secure your progress before the danger appears!</p>
          </div>
          ...
          <div ref={ctaRef} className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: '140px' }}>
            <button
              ...
              disabled={(gameActive && selectedChambers.length === 0) || (!gameActive && !gameResult && Number(playAmount) < MIN_PLAY)}
              className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-slate-600 to-slate-700 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {showResultPopup && gameResult && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            <div className={`${gameResult.win ? 'bg-green-500' : 'bg-red-500'} text-white px-8 py-6 rounded-2xl shadow-2xl text-center pointer-events-auto`} style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
              <div className="text-4xl mb-2">{gameResult.win ? '🔫' : '☠️'}</div>
              <div className="text-2xl font-bold mb-1">{gameResult.win ? 'CASHED OUT!' : 'BOOM!'}</div>
