...
const MIN_BET = 1000;
const SYMBOLS = ['💎', '⭐', '🍒', '🍋', '🍊', '🍉', '🎰', '7️⃣', '🔔'];
const PAYOUTS = { '💎': { 5: 500, 4: 100, 3: 20 }, '7️⃣': { 5: 200, 4: 50, 3: 15 }, '⭐': { 5: 100, 4: 30, 3: 10 }, '🔔': { 5: 80, 4: 20, 3: 8 }, '🎰': { 5: 60, 4: 15, 3: 6 }, '🍒': { 5: 40, 4: 10, 3: 5 }, '🍉': { 5: 30, 4: 8, 3: 4 }, '🍊': { 5: 20, 4: 6, 3: 3 }, '🍋': { 5: 15, 4: 5, 3: 2 } };
...
function checkWin(reels) {
  ...
  for (const [symbol, count] of Object.entries(counts)) {
    if (count >= 3 && PAYOUTS[symbol]) {
      return { symbol, count, multiplier: PAYOUTS[symbol][count] || 0 };
    }
  }
  return null;
}
...
export default function SlotsPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalSpins: 0, wins: 0, totalBet: 0, totalWon: 0, biggestWin: 0, jackpots: 0, lastBet: MIN_BET }));
  ...
  const spin = (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      if (result.success) { bet = result.amount; setIsFreePlay(false); router.replace('/slots-upgraded', undefined, { shallow: true }); }
    ...
    const spinInterval = setInterval(() => {
      ...
      if (count >= 15) {
        ...
        setReels(finalReels);
        setSpinning(false);
        checkResult(finalReels, bet);
      }
    }, 100);
  };

  const checkResult = (finalReels, bet) => {
    const winData = checkWin(finalReels);
    const prize = winData ? bet * winData.multiplier : 0;
    const win = prize > 0;
    ...
    const resultData = { win, symbol: winData?.symbol, count: winData?.count, multiplier: winData?.multiplier || 0, prize, profit: win ? prize - bet : -bet, jackpot: winData?.count === 5 };
    setGameResult(resultData);

    const newStats = { ...stats, totalSpins: stats.totalSpins + 1, wins: win ? stats.wins + 1 : stats.wins, totalBet: stats.totalBet + bet, totalWon: win ? stats.totalWon + prize : stats.totalWon, biggestWin: Math.max(stats.biggestWin, win ? prize : 0), jackpots: resultData.jackpot ? stats.jackpots + 1 : stats.jackpots, lastBet: bet };
    setStats(newStats);
  };

  const resetGame = () => { setGameResult(null); setShowResultPopup(false); setReels(['🎰', '🎰', '🎰', '🎰', '🎰']); setSpinning(false); };
  ...
  const potentialWin = Math.floor(Number(betAmount) * 500);

  return (
    <Layout>
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-yellow-900 via-black to-purple-900" style={{ height: 'var(--app-100vh, 100vh)' }}>
        ...
        <div className="absolute top-0 left-0 right-0 z-50 pointer-events-none">
          <div className="relative px-2 py-3">
            <div className="absolute left-2 top-2 flex gap-2 pointer-events-auto">
              ...
              {freePlayTokens > 0 && (<button onClick={() => spin(true)} disabled={spinning} className="relative px-2 py-1 rounded-lg bg-amber-500/20 border border-amber-500/40 hover:bg-amber-500/30 transition-all disabled:opacity-50" title={`${freePlayTokens} Free Play${freePlayTokens > 1 ? 's' : ''} Available`}><span className="text-base">🎁</span><span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">{freePlayTokens}</span></button>)}
            </div>
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-4 pb-16 pt-14 overflow-y-auto" style={{ minHeight: '100%' }}>
          <div className="text-center mb-3"><h1 className="text-3xl md:text-4xl font-extrabold text-white mb-1">🎰 Slots</h1><p className="text-white/70 text-sm">5 Reels • Massive wins!</p></div>
          ...
          <div className="mb-3" style={{ minHeight: '150px' }}>
            <div className="flex gap-2 justify-center mb-2">{reels.map((symbol, i) => (<div key={i} className={`w-16 h-20 rounded-lg bg-gradient-to-br from-purple-600 to-purple-800 border-2 border-yellow-400 flex items-center justify-center text-4xl ${spinning ? 'animate-pulse' : ''}`}>{symbol}</div>))}</div>
            <div className="text-center" style={{ height: '28px' }}><div className={`text-base font-bold transition-opacity ${gameResult ? 'opacity-100' : 'opacity-0'} ${gameResult?.win ? 'text-green-400' : 'text-white'}`}>{gameResult?.win ? `${gameResult.count} ${gameResult.symbol}!` : gameResult ? 'No Match' : 'waiting'}</div></div>
          </div>
        ...
        {showResultPopup && gameResult && gameResult.win && (<div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none"><div className="bg-gradient-to-r from-yellow-500 to-purple-600 text-white px-8 py-6 rounded-2xl shadow-2xl text-center pointer-events-auto" style={{ animation: 'fadeIn 0.3s ease-in-out' }}><div className="text-4xl mb-2">{gameResult.jackpot ? '💎' : '🎉'}</div><div className="text-2xl font-bold mb-1">{gameResult.jackpot ? 'JACKPOT!' : 'YOU WIN!'}</div><div className="text-lg">+{fmt(gameResult.prize)} MLEO</div><div className="text-sm opacity-80 mt-2">{gameResult.count} {gameResult.symbol}</div></div></div>)}
        ...
        {showHowToPlay && (<div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4"><div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto"><h2 className="text-2xl font-extrabold mb-4">🎰 How to Play</h2><div className="space-y-3 text-sm"><p><strong>1. Place Bet:</strong> Min {MIN_BET} MLEO</p><p><strong>2. Spin:</strong> 5 reels spin!</p><p><strong>3. Win:</strong> Match 3+ symbols!</p><div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3"><p className="text-yellow-300 font-semibold">Top Prizes:</p><div className="text-xs text-white/80 mt-2 space-y-1"><p>• 5 💎: ×500 💎</p><p>• 5 7️⃣: ×200</p><p>• 5 ⭐: ×100</p></div></div></div><button onClick={() => setShowHowToPlay(false)} className="w-full mt-6 py-3 rounded-lg bg-white/10 hover:bg-white/20 font-bold">Close</button></div></div>)}

        {showStats && (<div className="fixed inset-0 z-[10000] bg-black/80 flex items-center justify-center p-4"><div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto"><h2 className="text-2xl font-extrabold mb-4">📊 Your Statistics</h2><div className="space-y-3"><div className="grid grid-cols-2 gap-3"><div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Total Spins</div><div className="text-xl font-bold">{stats.totalSpins}</div></div><div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Win Rate</div><div className="text-xl font-bold text-green-400">{stats.totalSpins > 0 ? ((stats.wins / stats.totalSpins) * 100).toFixed(1) : 0}%</div></div><div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Total Bet</div><div className="text-lg font-bold text-amber-400">{fmt(stats.totalBet)}</div></div><div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Total Won</div><div className="text-lg font-bold text-emerald-400">{fmt(stats.totalWon)}</div></div><div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Biggest Win</div><div className="text-lg font-bold text-yellow-400">{fmt(stats.biggestWin)}</div></div><div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Jackpots</div><div className="text-lg font-bold text-purple-400">{stats.jackpots}</div></div></div></div><button onClick={() => setShowStats(false)} className="w-full mt-6 py-3 rounded-lg bg-white/10 hover:bg-white/20 font-bold">Close</button></div></div>)}
