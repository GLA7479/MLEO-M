...
export function BaseMap({ state, onSelectBuilding }) {
  return (
    <div className="relative h-full w-full overflow-hidden rounded-lg border border-slate-800 bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900">
      ...
      {BUILDINGS.map((def) => {
        ...
        return (
          ...
          >
            <div
              className={`
                relative flex h-11 w-16 items-center justify-center rounded-md border
                bg-slate-900/80 text-[11px] font-medium
                ...
                group-hover:border-emerald-400/80 group-hover:text-emerald-50 group-hover:shadow-[0_0_0_1px_rgba(16,185,129,0.8),0_18px_40px_rgba(16,185,129,0.45)]
                transition-all duration-200
              `}
            >
              <span className="truncate px-1">{def.name}</span>
            ...
            <span className="mt-0.5 max-w-[72px] truncate text-[10px] text-slate-500 group-hover:text-slate-300">
              {def.desc}
            </span>
            {showDrone && (
              <span
