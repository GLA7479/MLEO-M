  ...
  if (!selectedKey) {
    return (
      <div className="text-xs text-slate-400">
        Select a building on the map to see details and upgrade options.
      </div>
    );
  }
    ...
    <div className="space-y-3 text-xs">
      <div>
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="text-right">
            <div className="inline-flex items-center rounded-full border border-emerald-400/60 bg-emerald-500/10 px-2 py-0.5 text-[11px] text-emerald-200">
              Lv {Math.max(level, 0)}
            </div>
          </div>
        </div>
        <p className="mt-1 text-[11px] text-slate-300">{def.desc}</p>
      </div>
      ...
      <div className="rounded border border-slate-800 bg-slate-900/80 p-2">
        <div className="mb-1 flex items-center justify-between">
          ...
          >
            {affordable ? "You can afford this upgrade" : "Not enough resources"}
          </div>
        </div>
        ...
        <button
          ...
          className="mt-2 w-full rounded bg-emerald-600/90 px-2 py-1 text-[11px] font-medium hover:bg-emerald-500 disabled:cursor-not-allowed disabled:bg-slate-700"
        >
