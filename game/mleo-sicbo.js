...
export default function SicBoPage() {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const used = headH + (metersRef.current?.offsetHeight || 0) + (betRef.current?.offsetHeight || 0) + (ctaRef.current?.offsetHeight || 0) + topPad + 48 + safeBottom + 24;
  ...
  // Only show potentialWin when game is not active (no gameResult and not rolling)
  const potentialWin = (!gameResult && !rolling && selectedPlay) ? Math.floor(Number(playAmount) * PLAY_TYPES[selectedPlay].prize) : 0;

  return (
    <Layout>
      <div
        ref={wrapRef}
        className="relative w-full overflow-hidden bg-gradient-to-br from-red-900 via-black to-red-900"
        style={{ height: '100svh' }}
      >
        {/* Background pattern */}
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          ...
          <div className="mb-1 w-full max-w-md flex flex-col items-center justify-center" style={{ height: "var(--chart-h, 300px)" }}>
            <div className="flex gap-2 justify-center mb-2">
              {dice.map((d, i) => (
                <div key={i} className={`w-16 h-16 rounded-lg bg-white text-black flex items-center justify-center text-3xl font-bold shadow-lg ${rolling ? 'animate-bounce' : ''}`}>
                  {d}
                </div>
              ))}
            </div>
            {/* Result text - always present */}
            <div className="text-center" style={{ height: '28px' }}>
              <div className={`text-base font-bold transition-opacity ${gameResult ? 'opacity-100' : 'opacity-0'} ${gameResult?.win ? 'text-green-400' : 'text-red-400'}`}>
                {gameResult ? `Sum: ${gameResult.sum}` : 'waiting'}
              </div>
            </div>
          ...
          <div ref={betRef} className="flex items-center justify-center gap-1 mb-1 flex-wrap">
            ...
            <button onClick={() => { const current = Number(playAmount) || MIN_PLAY; const newBet = Math.max(MIN_PLAY, current - 100); setPlayAmount(String(newBet)); playSfx(clickSound.current); }} disabled={rolling} className="h-8 w-8 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold text-sm disabled:opacity-50">−</button>
          ...
          <div ref={ctaRef} className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: '140px' }}>
            <button
              ...
              disabled={rolling || !selectedPlay || gameResult}
              className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {rolling ? "Rolling..." : gameResult ? "NEW GAME" : "ROLL DICE"}
            </button>
        ...
        {showResultPopup && gameResult && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            <div className={`${gameResult.win ? 'bg-green-500' : 'bg-red-500'} text-white px-8 py-6 rounded-2xl shadow-2xl text-center pointer-events-auto`} style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
              <div className="text-4xl mb-2">{gameResult.triple ? '🎲🎲🎲' : gameResult.win ? '🎉' : '😔'}</div>
              <div className="text-2xl font-bold mb-1">
                {gameResult.triple ? 'TRIPLE!' : gameResult.win ? 'YOU WIN!' : 'YOU LOSE'}
              </div>
            ...
            <div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto">
              ...
              <div className="space-y-3">
                ...
                <div className="bg-gradient-to-r from-red-500/10 to-red-600/10 border border-red-500/30 rounded-lg p-4">
                  <div className="text-sm font-semibold mb-2">🎲 Triples Hit</div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-red-300">{stats.triples}</div>
                    <div className="text-xs text-white/60 mt-1">All Three Dice Match</div>
                  </div>
                </div>
              </div>
              <button
