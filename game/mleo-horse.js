// ============================================================================
// MLEO Horse Racing - Full-Screen Game Template
...
export default function HorseRacePage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalRaces: 0, wins: 0, losses: 0, totalBet: 0, totalWon: 0, biggestWin: 0, lastBet: MIN_BET }));
  ...
  const startRace = (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      if (result.success) { bet = result.amount; setIsFreePlay(false); router.replace('/horse', undefined, { shallow: true }); }
  ...
  return (
    <Layout>
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-green-900 via-black to-emerald-900" style={{ height: 'var(--app-100vh, 100vh)' }}>
        ...
        <div className="absolute top-0 left-0 right-0 z-50 pointer-events-none">
          <div className="relative px-2 py-3">
            <div className="absolute left-2 top-2 flex gap-2 pointer-events-auto">
              ...
              {freePlayTokens > 0 && (<button onClick={() => startRace(true)} disabled={racing} className="relative px-2 py-1 rounded-lg bg-amber-500/20 border border-amber-500/40 hover:bg-amber-500/30 transition-all disabled:opacity-50" title={`${freePlayTokens} Free Play${freePlayTokens > 1 ? 's' : ''} Available`}><span className="text-base">🎁</span><span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">{freePlayTokens}</span></button>)}
            </div>
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-4 pb-16 pt-14 overflow-y-auto" style={{ minHeight: '100%' }}>
          <div className="text-center mb-3"><h1 className="text-3xl md:text-4xl font-extrabold text-white mb-1">🏇 Horse Racing</h1><p className="text-white/70 text-sm">Bet on your horse • Win ×5!</p></div>
          ...
          <div className="flex items-center gap-2 mb-3">
            <button onClick={() => { const current = Number(betAmount) || MIN_BET; const newBet = Math.max(MIN_BET, current - 1000); setBetAmount(String(newBet)); playSfx(clickSound.current); }} disabled={racing} className="h-10 w-10 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold disabled:opacity-50">−</button>
            <input type="number" value={betAmount} onChange={(e) => setBetAmount(e.target.value)} disabled={racing} className="w-28 h-10 bg-black/30 border border-white/20 rounded-lg text-center text-white font-bold disabled:opacity-50 text-sm" min={MIN_BET} />
            <button onClick={() => { const current = Number(betAmount) || MIN_BET; const newBet = Math.min(vault, current + 1000); setBetAmount(String(newBet)); playSfx(clickSound.current); }} disabled={racing} className="h-10 w-10 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold disabled:opacity-50">+</button>
            </div>
            
          <div className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: '100px' }}>
            {!gameResult && <button onClick={() => startRace(false)} disabled={racing || selectedHorse === null} className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50">{racing ? "Racing..." : "START RACE"}</button>}
            {gameResult && <button onClick={resetGame} className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg hover:brightness-110 transition-all">RACE AGAIN</button>}
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-[10000] bg-black/60 flex items-center justify-center p-3" onClick={() => setMenuOpen(false)}>
            <div className="w-[86vw] max-w-[250px] max-h-[70vh] bg-[#0b1220] text-white shadow-2xl rounded-2xl p-4 md:p-5 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mt-4 text-xs opacity-70"><p>Horse Racing v2.0</p></div>
            </div>
          </div>
        )}
            ...
            <div className="bg-zinc-900 text-white max-w-md w-full rounded-2xl p-6 shadow-2xl max-h-[85vh] overflow-auto">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Total Races</div><div className="text-xl font-bold">{stats.totalRaces}</div></div>
                  <div className="bg-black/30 border border-white/10 rounded-lg p-3"><div className="text-xs text-white/60">Win Rate</div><div className="text-xl font-bold text-green-400">{stats.totalRaces > 0 ? ((stats.wins / stats.totalRaces) * 100).toFixed(1) : 0}%</div></div>
