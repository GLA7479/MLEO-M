      ...
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-red-900 via-black to-green-900" style={{ height: '100svh' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          ...
          <div ref={betRef} className="flex items-center justify-center gap-1 mb-1 flex-wrap">
            ...
            <button onClick={() => { setBetAmount(String(MIN_BET)); playSfx(clickSound.current); }} disabled={gameState !== "betting"} className="h-8 w-8 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400 font-bold text-xs disabled:opacity-50" title="Reset to minimum bet">↺</button>
          </div>

          <div ref={ctaRef} className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: "140px" }}>
            {gameState === "playing" ? (
              <div className="w-full flex gap-1 py-3 rounded-lg">
                <button onClick={hit} className="flex-1 rounded-lg font-bold text-xs bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg hover:brightness-110">HIT</button>
                <button onClick={() => stand()} className="flex-1 rounded-lg font-bold text-xs bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-lg hover:brightness-110">STAND</button>
                <button onClick={doubleDown} disabled={!canDouble} className="flex-1 rounded-lg font-bold text-[10px] bg-gradient-to-r from-purple-500 to-purple-600 text-white shadow-lg hover:brightness-110 disabled:opacity-30 disabled:cursor-not-allowed">DOUBLE</button>
                <button onClick={split} disabled={!canSplit} className="flex-1 rounded-lg font-bold text-[10px] bg-gradient-to-r from-pink-500 to-pink-600 text-white shadow-lg hover:brightness-110 disabled:opacity-30 disabled:cursor-not-allowed">SPLIT</button>
                <button onClick={surrender} disabled={!canSurrender} className="flex-1 rounded-lg font-bold text-[10px] bg-gradient-to-r from-gray-500 to-gray-600 text-white shadow-lg hover:brightness-110 disabled:opacity-30 disabled:cursor-not-allowed">SURR</button>
              </div>
            ) : (
