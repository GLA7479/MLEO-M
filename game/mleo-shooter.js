...
export default function ShooterPage() {
  ...
  const startGame = async (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      try {
        ...
        if (result.success) { play = result.amount; setIsFreePlay(false); router.replace('/shooter', undefined, { shallow: true }); }
      ...
      const debitResult = await debitSharedVault(play, "shooter");
      setVaultState(debitResult.balance);
    }
  ...
  const endGame = async () => {
    ...
    const multiplier = score * 0.20;
    const prize = Math.floor(play * multiplier);
    const win = prize > play; // Win if prize > play

    if (prize > 0) {
      const creditResult = await creditSharedVault(prize, "shooter");
      setVaultState(creditResult.balance);
      if (win) playSfx(winSound.current);
    }

    const resultData = { win, score, multiplier, prize, profit: prize - play, perfect: score >= TOTAL_TARGETS };
    setGameResult(resultData);
    setGameActive(false);

    const newStats = { ...stats, totalGames: stats.totalGames + 1, totalPlay: stats.totalPlay + play, totalWon: stats.totalWon + prize, biggestWin: Math.max(stats.biggestWin, prize), bestScore: Math.max(stats.bestScore, score), lastPlay: play };
    setStats(newStats);
  };
