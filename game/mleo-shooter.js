...
export default function ShooterPage() {
  ...
  const [stats, setStats] = useState(() => safeRead(LS_KEY, { totalGames: 0, totalPlay: 0, totalWon: 0, biggestWin: 0, bestScore: 0, lastPlay: MIN_PLAY }));
  ...
  useEffect(() => {
    ...
    const gameId = router.pathname.replace('/', '') || 'shooter';
  ...
  const collectToWallet = async () => {
    ...
    try {
      ...
      const debitResult = await debitSharedVault(wholeAmount, "shooter");
  ...
  const startGame = async (isFreePlayParam = false) => {
    ...
    if (isFreePlay || isFreePlayParam) {
      ...
      try {
        ...
        if (result.success) { play = result.amount; nextSessionId = result.sessionId; setFreePlayTokens(result.remainingTokens); setIsFreePlay(false); router.replace('/shooter', undefined, { shallow: true }); }
      ...
      if (play < MIN_PLAY) { alert(`Minimum play is ${MIN_PLAY} MLEO`); setGameActive(false); return; }
      const startResult = await startPaidArcadeSession("shooter", play);
      if (!startResult.success) { alert(startResult.message || 'Failed to start session'); setGameActive(false); return; }
  ...
  const endGame = async () => {
    ...
    try {
      const finishResult = await finishArcadeSession(sessionId, { score });
      ...
      const resolvedScore = Number(payload.score || score);
      ...
      const resultData = { win, score: resolvedScore, multiplier, prize, profit: prize - play, perfect: Boolean(payload.perfect) };
      setGameResult(resultData);
      setGameActive(false);
      setSessionId(null);

      const newStats = { ...stats, totalGames: stats.totalGames + 1, totalPlay: stats.totalPlay + play, totalWon: stats.totalWon + prize, biggestWin: Math.max(stats.biggestWin, prize), bestScore: Math.max(stats.bestScore, resolvedScore), lastPlay: play };
      setStats(newStats);
    } catch (error) {
  ...
  const potentialWin = Math.floor(Number(playAmount) * 3);
      ...
      <div ref={wrapRef} className="relative w-full overflow-hidden bg-gradient-to-br from-blue-900 via-black to-cyan-900" style={{ height: '100svh' }}>
        ...
        <div className="relative h-full flex flex-col items-center justify-start px-4 pb-4" style={{ minHeight: "100%", paddingTop: "calc(var(--head-h, 56px) + 8px)" }}>
          <div className="text-center mb-1">
            <h1 className="text-2xl font-extrabold text-white mb-0.5">🎯 Target Shooter</h1>
          ...
          <div ref={ctaRef} className="flex flex-col gap-3 w-full max-w-sm" style={{ minHeight: '140px' }}>
            <button onClick={gameResult ? resetGame : () => startGame(false)} disabled={gameActive || (!gameResult && Number(playAmount) < MIN_PLAY)} className="w-full py-3 rounded-lg font-bold text-base bg-gradient-to-r from-blue-500 to-cyan-600 text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed">{gameResult ? "PLAY AGAIN" : "START"}</button>
