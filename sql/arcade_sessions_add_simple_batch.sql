-- ============================================================================
-- EXTEND ARCADE SESSION RESOLUTION WITH SIMPLE ONE-SHOT GAMES
-- Supports: coin-flip, dice-over-under, baccarat, mega-wheel, keno
...
BEGIN
  ...
  ELSIF coalesce(v_session.game_id, '') = 'baccarat' THEN
    ...
    v_reward := CASE
      WHEN v_won THEN floor(v_session.stake * v_baccarat_multiplier)::bigint
      ELSE 0
    END;
  ...
  ELSIF coalesce(v_session.game_id, '') = 'keno' THEN
    ...
    IF EXISTS (
      SELECT 1
      FROM unnest(v_keno_selected) AS n
      WHERE n < 1 OR n > 40
    ) THEN
    ...
    v_reward := CASE
      WHEN v_keno_multiplier > 0 THEN floor(v_session.stake * v_keno_multiplier)::bigint
      ELSE 0
    END;
    v_won := v_reward > 0;
