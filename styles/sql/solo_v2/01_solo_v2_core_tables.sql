...
-- Additive and isolated: does not modify legacy arcade objects.

create extension if not exists pgcrypto;

-- Ensure baseline catalog/session/event tables exist even if 00 was skipped.
...
-- Align from foundation baseline if table already existed with smaller schema.
...
-- Backfill player_ref safely for existing rows created by 00 script shape.
do $$
begin
  if exists (
    select 1
