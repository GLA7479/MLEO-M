-- Wire OV2 Chess into shared-room economy, Quick Match allowlist, and leave/forfeit.
-- Apply after 094_ov2_chess_rpcs.sql.
--
-- ov2_shared_leave_room: canonical shared body is migrations/online-v2/checkers/089_ov2_checkers_shared_integration.sql
-- (latest migration in this repo that defines this RPC before Chess). This file keeps that body in lockstep: only add
-- v_in_ch_match and ov2_chess_voluntary_forfeit. If 089 changes, re-copy from 089 then re-apply the Chess branch.
...
-- Same PL/pgSQL as 089_ov2_checkers_shared_integration.sql except: DECLARE adds v_in_ch_match; IN_GAME adds
-- v_in_ch_match assignment, OR term, and ELSIF ov2_chess_voluntary_forfeit.
...
BEGIN
  ...
  IF v_room.status = 'IN_GAME' THEN
    ...
    IF v_in_ludo_match OR v_in_r51_match OR v_in_bg_match OR v_in_ck_match OR v_in_ch_match THEN
