-- Follow-up: require room lifecycle `active` and committed stakes for all seated members
-- before opening a Ludo session (aligns UI contract with server).
...
BEGIN
  ...
  IF v_room.lifecycle_phase IS DISTINCT FROM 'active' THEN
    RETURN jsonb_build_object(
      ...
      'message', 'Room must be active (all members committed stakes) before opening a Ludo session.'
    );
  END IF;
  ...
  IF EXISTS (
    ...
    RETURN jsonb_build_object(
      'ok', false,
      'code', 'SEATED_STAKES_NOT_COMMITTED',
      'message', 'All seated players must commit stakes before opening the Ludo session.'
    );
  END IF;
