-- Ludo: when __result__.prize is degenerate (equals a single per-seat stake, e.g. strike walkover in 023),
-- recompute full pot from committed shared-room seats so winner settlement matches full-pot economics (071).
-- Apply after 071. Run manually after review.
...
BEGIN
  ...
  -- At finish, ov2_ludo_seats may only list the winner; prize in JSON may be one stake. Derive pot from room.
  IF v_prize > 0 AND v_loss > 0 AND v_prize <= v_loss THEN
    ...
    IF COALESCE(v_seat_count, 0) >= 2 THEN
      v_prize := v_loss * v_seat_count;
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.ov2_ludo_after_finish_emit_settlement() IS
  'Ludo finish: winner credit = full pot; expands degenerate __result__.prize (single stake) using committed shared-room seat count.';
