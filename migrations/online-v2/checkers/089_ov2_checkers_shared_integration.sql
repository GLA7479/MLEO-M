-- Wire OV2 Checkers into shared-room economy policy, Quick Match, and leave/forfeit.
-- Apply after 088_ov2_checkers_rpcs.sql (extends 082 Backgammon shared bridge).
--
-- ov2_shared_leave_room: same body as 082_ov2_backgammon_shared_integration.sql, plus only the
-- Checkers IN_GAME branch (v_in_ck_match -> ov2_checkers_voluntary_forfeit). Re-sync here if 082 changes.
...
BEGIN
  ...
  IF v_room.status = 'IN_GAME' THEN
    ...
    IF v_in_ludo_match OR v_in_r51_match OR v_in_bg_match OR v_in_ck_match THEN
