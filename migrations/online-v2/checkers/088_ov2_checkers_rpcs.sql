-- OV2 Checkers RPCs: open, apply step, snapshot, timer, forfeit, rematch, settlement, claim.
-- Apply after 087_ov2_checkers_engine.sql.

BEGIN;

CREATE OR REPLACE FUNCTION public.ov2_checkers_parity_bump_timer(
  p_parity jsonb,
...
REVOKE ALL ON FUNCTION public.ov2_checkers_parity_bump_timer(jsonb, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_parity_bump_timer(jsonb, integer, integer) TO anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.ov2_checkers_jump_chain_at(p_board jsonb)
RETURNS int
LANGUAGE plpgsql
STABLE
SET search_path = public
...
BEGIN
  ...
  IF v_jc IS NULL OR jsonb_typeof(v_jc) <> 'object' OR NOT (v_jc ? 'at') THEN
    RETURN NULL;
  END IF;
  BEGIN
    v_at := (v_jc ->> 'at')::int;
...
BEGIN
  ...
  IF p_session.status = 'live' AND NOT v_finished AND v_my_seat IS NOT NULL AND v_turn IN (0, 1) AND v_my_seat = v_turn
     AND v_cells IS NOT NULL AND jsonb_typeof(v_cells) = 'array' AND jsonb_array_length(v_cells) = 64 THEN
...
CREATE OR REPLACE FUNCTION public.ov2_checkers_get_snapshot(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
CREATE OR REPLACE FUNCTION public.ov2_checkers_after_finish_emit_settlement()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  FROM public.ov2_checkers_seats
  ...
  v_idem := 'ov2:settle:' || v_room_id::text || ':' || v_sess_id::text || ':' || v_winner_pk || ':ck_win:';
  ...
  LOOP
    IF r.pk IS NULL OR length(r.pk) = 0 THEN
      CONTINUE;
    END IF;
    v_idem := 'ov2:settle:' || v_room_id::text || ':' || v_sess_id::text || ':' || r.pk || ':ck_loss:';
    INSERT INTO public.ov2_settlement_lines (
      room_id, match_seq, recipient_participant_key, line_kind, amount, idempotency_key, game_session_id, meta
    ) VALUES (
      v_room_id,
...
DROP TRIGGER IF EXISTS trg_ov2_checkers_finish_settlement ON public.ov2_checkers_sessions;
CREATE TRIGGER trg_ov2_checkers_finish_settlement
AFTER UPDATE OF phase ON public.ov2_checkers_sessions
FOR EACH ROW
WHEN (NEW.phase IS NOT DISTINCT FROM 'finished' AND OLD.phase IS DISTINCT FROM 'finished')
EXECUTE FUNCTION public.ov2_checkers_after_finish_emit_settlement();

CREATE OR REPLACE FUNCTION public._ov2_ck_member_rematch_requested(p_meta jsonb)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT
    (p_meta -> 'ck' ->> 'rematch_requested') IN ('true', 't', '1')
    OR (p_meta -> 'ck' -> 'rematch_requested') IS NOT DISTINCT FROM 'true'::jsonb;
$$;

CREATE OR REPLACE FUNCTION public.ov2_checkers_request_rematch(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
BEGIN
  ...
  IF v_room.active_session_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'code', 'NO_ACTIVE_SESSION', 'message', 'No active session');
  END IF;
  SELECT * INTO v_sess FROM public.ov2_checkers_sessions WHERE id = v_room.active_session_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'code', 'SESSION_NOT_FOUND', 'message', 'Session not found');
  END IF;
  ...
  END IF;
  IF public._ov2_ck_member_rematch_requested(v_member.meta) THEN
...
CREATE OR REPLACE FUNCTION public.ov2_checkers_cancel_rematch(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
BEGIN
  ...
  END IF;
  IF NOT public._ov2_ck_member_rematch_requested(v_member.meta) THEN
    RETURN jsonb_build_object('ok', true, 'idempotent', true);
  END IF;
  UPDATE public.ov2_room_members
  SET
    meta = CASE
      WHEN meta ? 'ck' THEN jsonb_set(meta, '{ck}', (meta -> 'ck') - 'rematch_requested' - 'rematch_at', true)
      ELSE meta
    END,
    updated_at = now()
  WHERE room_id = p_room_id AND participant_key = v_pk;
...
CREATE OR REPLACE FUNCTION public.ov2_checkers_start_next_match(
  p_room_id uuid,
  p_participant_key text,
...
BEGIN
  ...
  WHERE m.room_id = p_room_id
    ...
    AND public._ov2_ck_member_rematch_requested(m.meta);
  ...
  SET
    meta = CASE
      WHEN m.meta ? 'ck' THEN jsonb_set(m.meta, '{ck}', (m.meta -> 'ck') - 'rematch_requested' - 'rematch_at', true)
      ELSE m.meta
    END,
...
REVOKE ALL ON FUNCTION public.ov2_checkers_open_session(uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_open_session(uuid, text, text) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_checkers_apply_step(uuid, text, integer, integer, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_apply_step(uuid, text, integer, integer, bigint) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_checkers_get_snapshot(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_get_snapshot(uuid, text) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_checkers_voluntary_forfeit(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_voluntary_forfeit(uuid, text) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_checkers_mark_turn_timeout(uuid, text, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_mark_turn_timeout(uuid, text, bigint) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public._ov2_ck_member_rematch_requested(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.ov2_checkers_request_rematch(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_request_rematch(uuid, text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.ov2_checkers_cancel_rematch(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_cancel_rematch(uuid, text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.ov2_checkers_start_next_match(uuid, text, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_start_next_match(uuid, text, integer) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_checkers_claim_settlement(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_checkers_claim_settlement(uuid, text) TO anon, authenticated, service_role;
