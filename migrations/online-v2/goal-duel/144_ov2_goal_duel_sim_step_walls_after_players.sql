...
BEGIN
  ...
  -- ball floor (initial ground contact after integration)
  ...
  -- circle vs AABB separation before arena walls so keepers/players resolve first
  FOR i IN 1..2 LOOP
  ...
  -- ball floor again (separation can push the ball through the ground line)
  IF v_by + v_br > v_gy THEN
    v_by := v_gy - v_br;
    v_bvy := -abs(v_bvy) * v_rest;
    v_bvx := v_bvx * 0.94;
  ...
  -- side walls + ceiling last so goal detection (RPC, on this final state) matches resolved physics
  IF v_bx - v_br < 0 THEN
    v_bx := v_br;
