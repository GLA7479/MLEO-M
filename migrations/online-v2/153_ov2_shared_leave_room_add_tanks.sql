-- OV2 Tanks V1: extend shared leave_room for mid-match Tanks forfeit (apply after 156 + Tanks RPCs that define ov2_tanks_voluntary_forfeit).
-- Replaces only public.ov2_shared_leave_room.
...
DECLARE
  ...
  x_in_tanks_match boolean := false;
...
BEGIN
  ...
  IF x_status = 'IN_GAME' THEN
    ...
    x_in_tanks_match := x_product_game_id IS NOT DISTINCT FROM 'ov2_tanks'
      AND x_active_session_id IS NOT NULL
      AND COALESCE((SELECT s.phase FROM public.ov2_tanks_sessions s WHERE s.id = x_active_session_id), '') = 'playing'
      AND EXISTS (
        SELECT 1
        FROM public.ov2_tanks_seats ts
        WHERE ts.session_id = x_active_session_id
          AND ts.participant_key = x_pk
      );
    ...
    IF x_in_ludo_match OR x_in_r51_match OR x_in_bg_match OR x_in_ck_match OR x_in_ch_match
       ...
       OR x_in_cc_match OR x_in_fh_match OR x_in_gd_match OR x_in_snakes_match OR x_in_tanks_match THEN
      ...
      ELSIF x_in_tanks_match THEN
        x_ff := public.ov2_tanks_voluntary_forfeit(p_room_id, x_pk);
