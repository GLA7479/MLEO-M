-- Fix private/hidden room password hashing: gen_salt('bf') resolves as unknown without ::text on some Postgres builds.
-- Scope: ov2_shared_create_room only (same body as 051, one-line hash fix).
...
BEGIN
  ...
  IF v_pw IS NOT NULL THEN
    v_hash := crypt(v_pw::text, gen_salt('bf'::text));
  END IF;
