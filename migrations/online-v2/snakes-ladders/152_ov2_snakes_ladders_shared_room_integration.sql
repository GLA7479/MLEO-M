-- OV2 Snakes & Ladders: shared-room RPC surface (must apply AFTER 151 — references ov2_snakes_ladders_voluntary_forfeit).
-- Forked from goal-duel/141_ov2_goal_duel_shared_integration.sql + ov2_snakes_ladders branches.
...
AS $$
  SELECT CASE trim(COALESCE(p_product_game_id, ''))
    ...
    WHEN 'ov2_snakes_ladders' THEN 'ON_HOST_START'
...
AS $$
  SELECT CASE trim(COALESCE(p_game, ''))
    ...
    WHEN 'ov2_snakes_ladders' THEN 4
    ELSE 4
  END;
$$;
...
BEGIN
  ...
  IF v_room.status = 'IN_GAME' THEN
    ...
    v_in_mm_match := v_room.product_game_id IS NOT DISTINCT FROM 'ov2_meldmatch'
      AND v_room.active_session_id IS NOT NULL
      AND coalesce((SELECT phase FROM public.ov2_meldmatch_sessions WHERE id = v_room.active_session_id), '') IN ('playing', 'layoff')
      AND EXISTS (
        SELECT 1 FROM public.ov2_meldmatch_seats mms
        WHERE mms.session_id = v_room.active_session_id AND mms.participant_key = v_pk
      );
    v_in_cc_match := v_room.product_game_id IS NOT DISTINCT FROM 'ov2_colorclash'
      AND v_room.active_session_id IS NOT NULL
      AND coalesce((SELECT phase FROM public.ov2_colorclash_sessions WHERE id = v_room.active_session_id), '') = 'playing'
      AND EXISTS (
        SELECT 1 FROM public.ov2_colorclash_seats ccs
        WHERE ccs.session_id = v_room.active_session_id AND ccs.participant_key = v_pk
      );
    v_in_fh_match := v_room.product_game_id IS NOT DISTINCT FROM 'ov2_fleet_hunt'
      AND v_room.active_session_id IS NOT NULL
      AND coalesce((SELECT phase FROM public.ov2_fleet_hunt_sessions WHERE id = v_room.active_session_id), '') IN ('placement', 'battle')
      AND EXISTS (
    ...
    v_in_sl_match := v_room.product_game_id IS NOT DISTINCT FROM 'ov2_snakes_ladders'
      AND v_room.active_session_id IS NOT NULL
      AND coalesce((SELECT phase FROM public.ov2_snakes_ladders_sessions WHERE id = v_room.active_session_id), '') = 'playing'
      AND EXISTS (
        SELECT 1 FROM public.ov2_snakes_ladders_seats sls
        WHERE sls.session_id = v_room.active_session_id AND sls.participant_key = v_pk
      );

    IF v_in_ludo_match OR v_in_r51_match OR v_in_bg_match OR v_in_ck_match OR v_in_ch_match OR v_in_dom_match OR v_in_fl_match OR v_in_fg_match OR v_in_mm_match OR v_in_cc_match OR v_in_fh_match OR v_in_gd_match OR v_in_sl_match THEN
      ...
      ELSIF v_in_gd_match THEN
        ...
        IF coalesce((v_ff ->> 'ok')::boolean, false) IS NOT TRUE THEN
          RETURN v_ff;
        END IF;
      ELSIF v_in_sl_match THEN
        v_ff := public.ov2_snakes_ladders_voluntary_forfeit(p_room_id, v_pk);
        IF coalesce((v_ff ->> 'ok')::boolean, false) IS NOT TRUE THEN
          RETURN v_ff;
        END IF;
      END IF;
