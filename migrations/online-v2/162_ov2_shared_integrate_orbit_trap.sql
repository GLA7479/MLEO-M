-- OV2 Orbit Trap: shared-room economy policy + leave-room forfeit dispatch (Phase 4).
-- Neutral placement: migrations/online-v2/ root (shared functions; same pattern as 156_ov2_shared_integrate_snakes.sql).
...
AS $$
  SELECT CASE trim(COALESCE(p_product_game_id, ''))
    ...
    WHEN 'ov2_orbit_trap' THEN 'ON_HOST_START'
...
BEGIN
  ...
  IF x_status = 'IN_GAME' THEN
    ...
    x_in_ot_match := x_product_game_id IS NOT DISTINCT FROM 'ov2_orbit_trap'
      AND x_active_session_id IS NOT NULL
      AND COALESCE((SELECT s.phase FROM public.ov2_orbit_trap_sessions s WHERE s.id = x_active_session_id), '') = 'playing'
      AND EXISTS (
        SELECT 1
        FROM public.ov2_orbit_trap_seats ss
        WHERE ss.session_id = x_active_session_id
          AND ss.participant_key = x_pk
      );
    ...
    IF x_in_ludo_match OR x_in_r51_match OR x_in_bg_match OR x_in_ck_match OR x_in_ch_match
       ...
       OR x_in_cc_match OR x_in_fh_match OR x_in_gd_match OR x_in_snakes_match OR x_in_ot_match THEN
      ...
      ELSIF x_in_ot_match THEN
        x_ff := public.ov2_orbit_trap_leave_game(p_room_id, x_pk);
      END IF;
