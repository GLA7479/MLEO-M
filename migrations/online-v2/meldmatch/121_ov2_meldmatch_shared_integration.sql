-- Wire OV2 MeldMatch into shared-room economy, Quick Match allowlist, and leave/forfeit.
-- Apply after 120_ov2_meldmatch_rpcs_actions.sql (and after any migration that last replaced ov2_shared_leave_room).
-- Replaces the integration surface from 116_ov2_flipgrid_shared_integration.sql with MeldMatch merged in.
...
BEGIN
  ...
  IF v_room.status = 'IN_GAME' THEN
    ...
    IF v_in_ludo_match OR v_in_r51_match OR v_in_bg_match OR v_in_ck_match OR v_in_ch_match OR v_in_dom_match OR v_in_fl_match OR v_in_fg_match OR v_in_mm_match THEN
