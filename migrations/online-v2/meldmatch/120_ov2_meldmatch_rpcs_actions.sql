-- MeldMatch: doubles, timer (playing + layoff pass), forfeit, settlement, rematch, claim.
-- Apply after 119_ov2_meldmatch_rpcs.sql.
...
CREATE OR REPLACE FUNCTION public.ov2_meldmatch_respond_double(
  p_room_id uuid,
  p_participant_key text,
  p_accept boolean,
  p_expected_revision bigint DEFAULT NULL
...
BEGIN
  ...
  IF coalesce(p_accept, false) THEN
    ...
    v_ps := public.ov2_mm_parity_bump_timer(v_ps, v_from, v_from);
    UPDATE public.ov2_meldmatch_sessions
    SET
      parity_state = v_ps,
      revision = v_sess.revision + 1,
...
CREATE OR REPLACE FUNCTION public.ov2_meldmatch_mark_turn_timeout(
  p_room_id uuid,
  p_participant_key text,
  p_expected_revision bigint DEFAULT NULL
)
...
BEGIN
  ...
  IF v_sess.status IS DISTINCT FROM 'live' OR v_sess.phase NOT IN ('playing', 'layoff') THEN
    RETURN jsonb_build_object('ok', true, 'idempotent', true, 'snapshot', public.ov2_meldmatch_build_client_snapshot(v_sess, v_pk));
  END IF;
  ...
  IF NOT (v_ps ? 'turn_deadline_at') OR NOT (v_ps ? 'turn_deadline_seat') THEN
    v_ps := public.ov2_mm_parity_bump_timer(v_ps, v_turn, NULL);
    UPDATE public.ov2_meldmatch_sessions
    SET
      parity_state = v_ps,
      revision = v_sess.revision + 1,
  ...
  -- Layoff phase: timeout = pass with no layoffs (no missed-turn strike)
  IF v_sess.phase = 'layoff' THEN
  ...
  v_other := CASE WHEN v_turn = 0 THEN 1 ELSE 0 END;
  v_ps := public.ov2_mm_parity_bump_timer(v_ps, v_other, v_turn);
...
CREATE OR REPLACE FUNCTION public.ov2_meldmatch_voluntary_forfeit(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
BEGIN
  ...
  SELECT * INTO v_room FROM public.ov2_rooms WHERE id = p_room_id FOR UPDATE;
  IF NOT FOUND OR v_room.product_game_id IS DISTINCT FROM 'ov2_meldmatch' THEN
    RETURN jsonb_build_object('ok', false, 'code', 'ROOM_NOT_FOUND', 'message', 'Not a MeldMatch room');
  END IF;
  IF v_room.active_session_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'code', 'NO_ACTIVE_SESSION', 'message', 'No active session');
  END IF;
  SELECT * INTO v_sess FROM public.ov2_meldmatch_sessions WHERE id = v_room.active_session_id FOR UPDATE;
  IF NOT FOUND OR v_sess.phase NOT IN ('playing', 'layoff') THEN
  ...
  END IF;
  v_other := CASE WHEN v_seat = 0 THEN 1 ELSE 0 END;
  v_mult := public.ov2_mm_parity_stake_mult(v_sess.parity_state);
...
CREATE OR REPLACE FUNCTION public.ov2_meldmatch_after_finish_emit_settlement()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF coalesce((v_res ->> 'draw')::boolean, false) THEN
    ...
    LOOP
      IF r.pk IS NULL OR length(r.pk) = 0 THEN
        CONTINUE;
      END IF;
      v_idem := 'ov2:settle:' || v_room_id::text || ':' || v_sess_id::text || ':' || r.pk || ':mm_draw_refund:';
  ...
  IF v_loss IS NULL OR v_loss <= 0 THEN
    v_loss := v_entry * public.ov2_mm_parity_stake_mult(NEW.parity_state);
  END IF;
  IF v_prize IS NULL OR v_prize <= 0 THEN
    v_prize := v_loss * 2;
  END IF;

  v_idem := 'ov2:settle:' || v_room_id::text || ':' || v_sess_id::text || ':' || v_winner_pk || ':mm_win:';
  INSERT INTO public.ov2_settlement_lines (
    room_id, match_seq, recipient_participant_key, line_kind, amount, idempotency_key, game_session_id, meta
  ) VALUES (
    v_room_id,
  ...
  LOOP
    IF r.pk IS NULL OR length(r.pk) = 0 THEN
      CONTINUE;
    END IF;
    v_idem := 'ov2:settle:' || v_room_id::text || ':' || v_sess_id::text || ':' || r.pk || ':mm_loss:';
    INSERT INTO public.ov2_settlement_lines (
      room_id, match_seq, recipient_participant_key, line_kind, amount, idempotency_key, game_session_id, meta
    ) VALUES (
      v_room_id,
...
DROP TRIGGER IF EXISTS trg_ov2_meldmatch_finish_settlement ON public.ov2_meldmatch_sessions;
CREATE TRIGGER trg_ov2_meldmatch_finish_settlement
AFTER UPDATE OF phase ON public.ov2_meldmatch_sessions
FOR EACH ROW
WHEN (NEW.phase IS NOT DISTINCT FROM 'finished' AND OLD.phase IS DISTINCT FROM 'finished')
EXECUTE FUNCTION public.ov2_meldmatch_after_finish_emit_settlement();

CREATE OR REPLACE FUNCTION public._ov2_mm_member_rematch_requested(p_meta jsonb)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT
    (p_meta -> 'mm' ->> 'rematch_requested') IN ('true', 't', '1')
    OR (p_meta -> 'mm' -> 'rematch_requested') IS NOT DISTINCT FROM 'true'::jsonb;
$$;

CREATE OR REPLACE FUNCTION public.ov2_meldmatch_request_rematch(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
BEGIN
  ...
  IF v_room.active_session_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'code', 'NO_ACTIVE_SESSION', 'message', 'No active session');
  END IF;
  SELECT * INTO v_sess FROM public.ov2_meldmatch_sessions WHERE id = v_room.active_session_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'code', 'SESSION_NOT_FOUND', 'message', 'Session not found');
  END IF;
  ...
  END IF;
  IF public._ov2_mm_member_rematch_requested(v_member.meta) THEN
...
CREATE OR REPLACE FUNCTION public.ov2_meldmatch_cancel_rematch(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
BEGIN
  ...
  END IF;
  IF NOT public._ov2_mm_member_rematch_requested(v_member.meta) THEN
    RETURN jsonb_build_object('ok', true, 'idempotent', true);
  END IF;
  UPDATE public.ov2_room_members
  SET
    meta = CASE
      WHEN meta ? 'mm' THEN jsonb_set(meta, '{mm}', (meta -> 'mm') - 'rematch_requested' - 'rematch_at', true)
      ELSE meta
    END,
    updated_at = now()
  WHERE room_id = p_room_id AND participant_key = v_pk;
...
CREATE OR REPLACE FUNCTION public.ov2_meldmatch_start_next_match(
  p_room_id uuid,
  p_participant_key text,
...
BEGIN
  ...
  WHERE m.room_id = p_room_id
    ...
    AND public._ov2_mm_member_rematch_requested(m.meta);
  ...
  SET
    meta = CASE
      WHEN m.meta ? 'mm' THEN jsonb_set(m.meta, '{mm}', (m.meta -> 'mm') - 'rematch_requested' - 'rematch_at', true)
      ELSE m.meta
    END,
...
REVOKE ALL ON FUNCTION public.ov2_meldmatch_respond_double(uuid, text, boolean, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_respond_double(uuid, text, boolean, bigint) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_meldmatch_mark_turn_timeout(uuid, text, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_mark_turn_timeout(uuid, text, bigint) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_meldmatch_voluntary_forfeit(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_voluntary_forfeit(uuid, text) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_meldmatch_after_finish_emit_settlement() FROM PUBLIC;
REVOKE ALL ON FUNCTION public._ov2_mm_member_rematch_requested(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.ov2_meldmatch_request_rematch(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_request_rematch(uuid, text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.ov2_meldmatch_cancel_rematch(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_cancel_rematch(uuid, text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.ov2_meldmatch_start_next_match(uuid, text, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_start_next_match(uuid, text, integer) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_meldmatch_claim_settlement(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_meldmatch_claim_settlement(uuid, text) TO anon, authenticated, service_role;
