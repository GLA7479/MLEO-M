-- OV2 FlipGrid RPCs: snapshot, open, play. Apply after 113_ov2_flipgrid_engine.sql.

BEGIN;

CREATE OR REPLACE FUNCTION public.ov2_flipgrid_legal_move_count(p_cells jsonb, p_seat int)
RETURNS int
LANGUAGE plpgsql
IMMUTABLE
  ...
  FOR v_r IN 0..7 LOOP
    FOR v_c IN 0..7 LOOP
      ...
      IF public.ov2_fg_cell_value(p_cells, v_idx) IS NOT NULL THEN
        CONTINUE;
      END IF;
      IF jsonb_array_length(public.ov2_fg_all_flip_indices(p_cells, v_r, v_c, p_seat)) > 0 THEN
        v_n := v_n + 1;
      END IF;
    END LOOP;
  END LOOP;
...
CREATE OR REPLACE FUNCTION public.ov2_flipgrid_get_snapshot(
  p_room_id uuid,
  p_participant_key text
)
RETURNS jsonb
...
CREATE OR REPLACE FUNCTION public.ov2_flipgrid_play_move(
  p_room_id uuid,
  p_participant_key text,
  p_row integer,
  p_col integer,
...
DECLARE
  ...
  v_entry bigint;
BEGIN
  IF p_room_id IS NULL OR p_row IS NULL OR p_col IS NULL THEN
  ...
  v_cells := COALESCE(v_board -> 'cells', public.ov2_fg_empty_cells());
  SELECT ok, new_cells, flip_count, err
  INTO v_ok, v_new_cells, v_fc, v_err
  FROM public.ov2_fg_apply_move(v_cells, p_row::int, p_col::int, v_seat);
  ...
  v_board := jsonb_set(COALESCE(v_board, '{}'::jsonb), '{cells}', to_jsonb(v_new_cells), true);
  v_ps := v_sess.parity_state;

  SELECT f.o_board, f.o_parity, f.o_turn, f.o_finished, f.o_winner
  INTO v_fin_board, v_fin_parity, v_fin_turn, v_fin_done, v_fin_w
  FROM public.ov2_flipgrid_finalize_turn_state(v_board, v_ps, v_seat) AS f;
...
REVOKE ALL ON FUNCTION public.ov2_flipgrid_legal_move_count(jsonb, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_flipgrid_legal_move_count(jsonb, integer) TO anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.ov2_flipgrid_get_snapshot(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_flipgrid_get_snapshot(uuid, text) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.ov2_flipgrid_play_move(uuid, text, integer, integer, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ov2_flipgrid_play_move(uuid, text, integer, integer, bigint) TO anon, authenticated, service_role;
