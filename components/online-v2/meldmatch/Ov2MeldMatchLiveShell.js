...
import { requestOv2MeldMatchOpenSession } from "../../../lib/online-v2/meldmatch/ov2MeldMatchSessionAdapter";
import {
...
import Ov2MeldMatchScreen from "./Ov2MeldMatchScreen";
...
export default function Ov2MeldMatchLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.MELDMATCH) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a MeldMatch table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_mm_shell_room:${roomId}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_mm_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.MELDMATCH &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.MELDMATCH) return "";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const res = await requestOv2MeldMatchOpenSession(roomId, participantId, {
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="MeldMatch" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
      ...
      {roomId && loadError && !room ? (
        ...
        <div className="flex w-full min-h-min flex-col overflow-x-hidden">
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                When the room is active, the host can open MeldMatch here.
              </p>
              <Link
                href="/online-v2/rooms"
          ...
          <div className="flex w-full min-h-min flex-col">
            <Ov2MeldMatchScreen
              key={room?.active_session_id ? String(room.active_session_id) : "ov2-mm-no-session"}
