...
import { requestOv2SnakesOpenSession } from "../../../lib/online-v2/snakes-and-ladders/ov2SnakesSessionApi";
import {
...
import Ov2SnakesScreen from "./Ov2SnakesScreen";
...
export default function Ov2SnakesLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.SNAKES_AND_LADDERS) {
        setRoom(null);
        setMembers([]);
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.SNAKES_AND_LADDERS &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.SNAKES_AND_LADDERS) return "";
    ...
    if (seatedCount > 4) return "Snakes & Ladders supports at most four seated players.";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const ms = room?.match_seq != null ? Number(room.match_seq) : NaN;
      if (!Number.isFinite(ms)) {
        setOpenErr("Could not read room match sequence. Refresh and retry.");
        return;
      }
      const res = await requestOv2SnakesOpenSession(roomId, participantId, { expectedRoomMatchSeq: Math.floor(ms) });
      if (!res.ok) {
  ...
  }, [roomId, participantId, canShellHostOpen, shellOpenDisabledReason, room, reloadContext]);
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="Snakes &amp; Ladders" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
          ...
          <div className="space-y-0 text-[11px] leading-relaxed text-zinc-400">
            <section className="border-b border-white/[0.05] py-2.5 first:pt-0 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                Classic 1–100 board. Roll 1–6, climb ladders and slide on snakes. Land exactly on 100 to win. Rolling a 6
                grants another roll unless three sixes in a row void the bonus chain. Overshooting 100 does not move.
              </p>
            </section>
            <section className="border-b border-white/[0.05] py-2.5 last:border-b-0 last:pb-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-500">Fair play</p>
              <p className="mt-1.5 text-zinc-400/95">
                No stake doubles or in-match raises in v1. Leaving during an active match uses the shared-room forfeit
                path. Any timer shown in the UI is display-only.
              </p>
            </section>
            <section className="py-2.5 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">Winner settlement is issued by the server; claim runs automatically when eligible.</p>
            </section>
          </div>
          ...
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                When the room is in play, the host can open Snakes &amp; Ladders here.
              </p>
              <Link
                href="/online-v2/rooms"
