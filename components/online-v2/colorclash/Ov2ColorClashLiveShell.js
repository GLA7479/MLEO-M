...
import { requestOv2ColorClashOpenSession } from "../../../lib/online-v2/colorclash/ov2ColorClashSessionAdapter";
import {
...
import Ov2ColorClashScreen from "./Ov2ColorClashScreen";
...
export default function Ov2ColorClashLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.COLOR_CLASH) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a Color Clash table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_cc_shell_room:${roomId}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_cc_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.COLOR_CLASH &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.COLOR_CLASH) return "";
    ...
    if (seatedCount < 2 || seatedCount > 4) return "Need 2 to 4 seated players.";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const res = await requestOv2ColorClashOpenSession(roomId, participantId, {
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="Color Clash" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
  ...
  return (
    ...
    >
      {roomId && loadError && !room ? (
        ...
        <div className="flex w-full min-h-min flex-col overflow-x-hidden">
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                When the room is active, the host can open Color Clash here.
              </p>
              <Link
                href="/online-v2/rooms"
          ...
          <div className="flex w-full min-h-min flex-col">
            <Ov2ColorClashScreen
              key={room?.active_session_id ? String(room.active_session_id) : "ov2-cc-no-session"}
