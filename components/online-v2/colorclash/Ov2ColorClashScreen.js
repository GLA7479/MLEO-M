...
export default function Ov2ColorClashScreen({ contextInput = null, onSessionRefresh }) {
  ...
  const onCardPress = useCallback(
    async card => {
      ...
      setErr("");
      if (myTurnPostDraw) {
        if (!vm.pendingDrawForYou || !ccCardsEqual(card, vm.pendingDrawForYou)) return;
    ...
    [busy, vaultClaimBusy, vm, myTurnPostDraw, myTurnPlaying, tryPlay, setErr]
  );
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
        ...
        {vm.phase === "playing" && myTurnPlaying ? (
          <div className="flex flex-wrap justify-center gap-2">
            <button type="button" disabled={busy || vm.stockCount <= 0} className={BTN_PRIMARY} onClick={() => void drawCard()}>
              Draw
            </button>
          </div>
        ) : null}
        ...
        <div className="rounded-xl border border-white/[0.08] bg-zinc-900/50 p-2 sm:p-3">
          ...
          <div className="flex max-h-[40vh] flex-wrap justify-center gap-1 overflow-y-auto sm:max-h-none sm:gap-1.5">
            {vm.myHand.map(card => {
              const key = ccStableCardKey(card);
              const highlightPostDraw = myTurnPostDraw && vm.pendingDrawForYou && ccCardsEqual(card, vm.pendingDrawForYou);
              const canTry =
                vm.phase === "playing" &&
                vm.mySeat != null &&
        ...
        <div className="absolute inset-0 z-20 flex items-end justify-center bg-black/70 p-3 sm:items-center">
          <div className="w-full max-w-sm rounded-xl border border-white/15 bg-zinc-950 p-4 shadow-xl">
            ...
            {vm.result?.prize != null && !isDraw ? (
              <p className="mt-2 text-center text-[11px] text-emerald-200/90">
                Pot: {Math.floor(Number(vm.result.prize) || 0).toLocaleString()} units
              </p>
            ) : null}
            {isDraw ? <p className="mt-2 text-center text-[11px] text-zinc-400">Entries were handled by settlement.</p> : null}
            <div className="mt-4 flex flex-col gap-2">
              ...
              <p className="text-center text-[10px] text-zinc-500">
                Rematch votes: {rematchCounts.ready}/{rematchCounts.seated} seated committed
              </p>
            </div>
            <button
              type="button"
              className="mt-3 w-full text-[11px] text-zinc-500 underline"
              onClick={() => {
