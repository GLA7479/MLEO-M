...
import { requestOv2FleetHuntOpenSession } from "../../../lib/online-v2/fleethunt/ov2FleetHuntSessionAdapter";
import {
...
import Ov2FleetHuntScreen from "./Ov2FleetHuntScreen";
...
export default function Ov2FleetHuntLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.FLEET_HUNT) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a Fleet Hunt table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_fh_shell_room:${roomId}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_fh_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.FLEET_HUNT &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.FLEET_HUNT) return "";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const res = await requestOv2FleetHuntOpenSession(roomId, participantId, {
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="Fleet Hunt" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
          ...
          <div className="space-y-0 text-[11px] leading-relaxed text-zinc-400">
            <section className="border-b border-white/[0.05] py-2.5 first:pt-0 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                Place five ships on a 10×10 grid, then take turns firing one shot per turn. Hits, misses, and sunk ships
                are tracked; you never see the opponent&apos;s ship layout. Sink every enemy ship cell to win the pot.
              </p>
            </section>
            <section className="border-b border-white/[0.05] py-2.5 last:border-b-0 last:pb-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-500">Timers & doubles</p>
              <p className="mt-1.5 text-zinc-400/95">
                About 30 seconds per placement action and per battle turn. Three missed battle turns in a row is a
                technical loss. Doubles run ×1→×16 with up to four accepted doubles; declining or timing out on a pending
                double forfeits at the current multiplier.
              </p>
            </section>
      ...
      {roomId && loadError && !room ? (
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                When the room is active, the host can open Fleet Hunt here.
              </p>
              <Link
                href="/online-v2/rooms"
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <Ov2FleetHuntScreen
              key={room?.active_session_id ? String(room.active_session_id) : "ov2-fh-no-session"}
