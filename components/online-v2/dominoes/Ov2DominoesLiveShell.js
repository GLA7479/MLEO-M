...
import { requestOv2DominoesOpenSession } from "../../../lib/online-v2/dominoes/ov2DominoesSessionAdapter";
import {
...
import Ov2DominoesScreen from "./Ov2DominoesScreen";
...
export default function Ov2DominoesLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.DOMINOES) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a Dominoes table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_dom_shell_room:${roomId}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_dom_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.DOMINOES &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.DOMINOES) return "";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const res = await requestOv2DominoesOpenSession(roomId, participantId, {
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="Dominoes" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
          ...
          <div className="space-y-0 text-[11px] leading-relaxed text-zinc-400">
            <section className="border-b border-white/[0.05] py-2.5 first:pt-0 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                Draw Dominoes, double-six set, seven tiles each, random first player. Draw from the stack when you have no
                legal play; pass only when the stack is empty. Optional stake increases (up to 16×) with accept or decline.
                Server holds hidden hands and stack order.
              </p>
            </section>
            <section className="border-b border-white/[0.05] py-2.5 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                Tap a tile to play. On the first move any tile is allowed; later, choose left or right when both ends match.
                Three missed turns in a row ends the round.
              </p>
            </section>
      ...
      {roomId && loadError && !room ? (
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                the room is active, the host can open Dominoes here.
              </p>
              <Link
                href="/online-v2/rooms"
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <Ov2DominoesScreen
              key={room?.active_session_id ? String(room.active_session_id) : "ov2-dom-no-session"}
