...
import { requestOv2FlipGridOpenSession } from "../../../lib/online-v2/flipgrid/ov2FlipGridSessionAdapter";
import {
...
import Ov2FlipGridScreen from "./Ov2FlipGridScreen";
...
export default function Ov2FlipGridLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.FLIPGRID) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a FlipGrid table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_fg_shell_room:${roomId}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_fg_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.FLIPGRID &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.FLIPGRID) return "";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const res = await requestOv2FlipGridOpenSession(roomId, participantId, {
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="FlipGrid" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
          ...
          <div className="space-y-0 text-[11px] leading-relaxed text-zinc-400">
            <section className="border-b border-white/[0.05] py-2.5 first:pt-0 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                8×8 grid, two sides. A move must flip at least one opposing disc along a line. If you have no legal move,
                the turn passes automatically (not a strike). When neither side can move, the round ends — more discs wins;
                a tie refunds entry. Table multiplier up to ×16 (accept or decline offers).
              </p>
            </section>
            <section className="border-b border-white/[0.05] py-2.5 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                ~30s per turn. Three missed turns in a row ends the round against you.
              </p>
            </section>
      ...
      {roomId && loadError && !room ? (
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                the room is active, the host can open FlipGrid here.
              </p>
              <Link
                href="/online-v2/rooms"
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <Ov2FlipGridScreen
              key={room?.active_session_id ? String(room.active_session_id) : "ov2-fg-no-session"}
