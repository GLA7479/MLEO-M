...
import { requestOv2FourLineOpenSession } from "../../../lib/online-v2/fourline/ov2FourLineSessionAdapter";
import {
...
import Ov2FourLineScreen from "./Ov2FourLineScreen";
...
export default function Ov2FourLineLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.FOURLINE) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a FourLine table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_fl_shell_room:${roomId}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_fl_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.FOURLINE &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.FOURLINE) return "";
  ...
  const onShellOpen = useCallback(async () => {
    ...
    setOpenErr("");
    try {
      const res = await requestOv2FourLineOpenSession(roomId, participantId, {
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="FourLine" showSubtitle={false} infoPanel={null} chromePreset="ov2_board">
          ...
          <div className="space-y-0 text-[11px] leading-relaxed text-zinc-400">
            <section className="border-b border-white/[0.05] py-2.5 first:pt-0 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                Seven columns, six rows. Pieces fall to the lowest open space in a column. Connect four in a row in any
                straight line to finish the round. Full grid with no line is a tie. Optional table multiplier up to 16×
                (accept or decline on offer).
              </p>
            </section>
            <section className="border-b border-white/[0.05] py-2.5 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1.5 text-zinc-400/95">
                Tap a column header on your turn. Three missed turns in a row ends the round against you.
              </p>
            </section>
      ...
      {roomId && loadError && !room ? (
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
          {showStakePhaseAfterRematchHint ? (
            <div className="flex min-h-[3.25rem] shrink-0 flex-col justify-center gap-1.5 border-b border-amber-500/20 bg-gradient-to-r from-amber-950/35 to-amber-950/10 px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-[3.5rem] sm:py-2">
              <p className="text-[10px] leading-snug text-amber-100/90 sm:text-[11px]">
                ...
                the room is active, the host can open FourLine here.
              </p>
              <Link
                href="/online-v2/rooms"
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <Ov2FourLineScreen
              key={room?.active_session_id ? String(room.active_session_id) : "ov2-fl-no-session"}
