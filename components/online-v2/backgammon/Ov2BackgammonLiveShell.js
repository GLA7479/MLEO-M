...
import { requestOv2BackgammonOpenSession } from "../../../lib/online-v2/backgammon/ov2BackgammonSessionAdapter";
import {
...
import Ov2BackgammonScreen from "./Ov2BackgammonScreen";
...
export default function Ov2BackgammonLiveShell() {
  ...
  const reloadContext = useCallback(async () => {
    ...
    try {
      ...
      if (r.product_game_id !== ONLINE_V2_GAME_IDS.BACKGAMMON) {
        setRoom(null);
        setMembers([]);
        setLoadError("This room is not a Backgammon table.");
        return;
      }
      setRoom(r);
      setMembers(m);
  ...
  useEffect(() => {
    ...
    const ch = supabaseMP
      .channel(`ov2_bg_live_presence:${roomId}`)
  ...
  const showStakePhaseAfterRematchHint = useMemo(
    () =>
      Boolean(
        room &&
          room.product_game_id === ONLINE_V2_GAME_IDS.BACKGAMMON &&
  ...
  const canShellHostOpenBg = useMemo(
    () =>
      Boolean(
        participantId &&
          isHost &&
  ...
  const shellOpenDisabledReason = useMemo(() => {
    if (!room || room.product_game_id !== ONLINE_V2_GAME_IDS.BACKGAMMON) return "";
  ...
  const onShellOpenBg = useCallback(async () => {
    if (!roomId || !participantId || !canShellHostOpenBg || shellOpenDisabledReason) return;
    setOpenBusy(true);
    setOpenErr("");
    try {
      const res = await requestOv2BackgammonOpenSession(roomId, participantId, {
  ...
  }, [roomId, participantId, canShellHostOpenBg, shellOpenDisabledReason, reloadContext]);
  ...
  if (!roomId) {
    return (
      <OnlineV2GamePageShell title="Backgammon" showSubtitle={false} infoPanel={null}>
        ...
        <>
          <div className="space-y-2 text-[11px] leading-snug text-zinc-300">
            <section>
              ...
              <p className="mt-0.5">
                Standard two-player backgammon: roll two dice, move checkers toward your home board, hit blots, re-enter from the
                bar, bear off all 15 to win. Moves and dice are enforced on the server.
              </p>
            </section>
            <section>
              <p className="font-semibold text-zinc-100">Turns</p>
              <p className="mt-0.5">
                Pick the die value to use, then choose from-bar or a point, then a destination (or bear off). If no legal play is
                possible, the server passes the turn.
              </p>
            </section>
            <section>
              <p className="font-semibold text-zinc-100">After the result</p>
              <p className="mt-0.5">
                Settlement runs automatically. Use <span className="text-zinc-200">Rematch</span> when ready; the host{" "}
                <span className="text-zinc-200">starts the next match</span>, then commit entry again in the room lobby.
              </p>
            </section>
          </div>
